...
function exceptionHandlerDevelop($exception)
{
    ...
    Application::getInstance()->headers->cleanHeaders()->setStatusCode($code)->sendHeaders();
...
{
    list($code, $message) = getCodeMessage($exception);
    Application::getInstance()->showException($code, $message);
}
...
{
    if (\Setup::$WRITE_EXCEPTIONS_TO_FILE && !($exception instanceof ExceptionPageNotFound)) {
        $type = get_class($exception);
        ...
        $text .= t("IP").":".($_SERVER['REMOTE_ADDR'] ?? '-')."\n";
        $text .= t("URL").":".($_SERVER['REQUEST_URI'] ?? '-')."\n";
...
{
    if ($exception instanceof ExceptionPageNotFound) {
        $code = 404;
