...
use Users\Model\Api;
use Users\Model\Notice\UserNeedRefreshPassword;
    ...
    */
    ...
    public static function login($login, $pass, $remember = false, $pass_encrypted = false, $no_set_current_user = false)
    {
        ...
        if ($user && $user['pass'] === $pass) {
            ...
            } elseif ($config['password_change_interval_days'] && $user['e_mail'] != '' && !$is_admin &&
                (time() - strtotime($user['last_change_password_date']) > $config['password_change_interval_days'] * 60*60*24)) {

                $notice = new UserNeedRefreshPassword();
                $notice->init($user);
                Manager::send($notice);

                self::$error = t('Вы давно не меняли свой пароль. Мы направили на Вашу почту письмо с инструкцией по активации вашего аккаунта.');
                return false;
    ...
    */
    public static function getCurrentUser()
    {
        ...
        if ($guest_hash === null) {
            ...
            $custom_session_id = \Setup::getCustomSessionId();
            if ($guest_hash === false || ($custom_session_id && $custom_session_id != $guest_hash)) {
    ...
    {
        // Необходимо для мобильного приложения для Android, потому что web view не сохраняет куки
        if (\Setup::getCustomSessionId()) {
            $guest_id = session_id();
        } else {
    ...
    public static function generateGuestId()
    {
        ...
        if (!$uniq_id) {
            $uniq_id = \Setup::getCustomSessionId() ?: md5(uniqid());
        }
        return $uniq_id;
    }
