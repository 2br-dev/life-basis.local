...
class Cms extends AbstractObject
{
    ...
    function _init()
    {
        $this->getPropertyIterator()->append([
            t('Основные'),
                    ...
                    'LOG_SETTINGS_DB_ADAPTER_MAX_FILE_SIZE' => new Type\Integer([
                        'description' => t('Размер файла лога запросов к БД'),
                        'checkboxView' => [1,0],
                        'visible' => false
                    ]),
                    'BULK_OPERATION_CHUNK_SIZE' => new Type\Integer([
                        'description' => t('Количество элементов для одной порции при массовых операциях'),
                        'hint' => t('Используется при массовом редактировании, удалении большого количества элементов. Уменьшите данное значение, если операция не укладывается в лимит max_execution_time на вашем сервере.')
                    ]),
                    'BULK_OPERATION_TIMEOUT' => new Type\Integer([
                        'description' => t('Таймаут в секундах для массовых операций'),
                        'hint' => t('Уменьшите значение, если желаете сократить время выполнения одной итерации массовой операции.')
                    ]),
            t('Кэш'),
    ...
    function _initDefaults()
    {
        ...
        $this['BULK_OPERATION_CHUNK_SIZE'] = \Setup::$BULK_OPERATION_CHUNK_SIZE;
        $this['BULK_OPERATION_TIMEOUT'] = \Setup::$BULK_OPERATION_TIMEOUT;
    }
