...
abstract class AbstractSetup
{
    public static 
        ...
        //Общие параметры
        $VERSION          = '6.1.158',  //Текущая версия ядра
        ...
        $RS_API_SERVER_DOMAIN = 'api.readyscript.ru', //Доменное имя API сервера ReadyScript
        ...
        $CACHE_BLOCKS_FOLDER = '/cache/blocks',  //Папка для хранения кэша настроек отрендеренных блоков
        ...
        $BULK_OPERATION_CHUNK_SIZE = 500, //Количество элементов, с которыми будут проводиться массовые операции в одной итерации
        $BULK_OPERATION_TIMEOUT = 12, //Таймаут в секундах для массовых операций
    ...
    */
    public static function init()
    {
        ...
        LanguageCore::init(); //Активируем языковые функции, объявляем функцию t
        ...
        EventManager::init(); //Инициализируем события
        LanguageCore::initThemeLang(); //Подключаем языковые файлы темы

        EventManager::fire('initialize'); //Вызываем событие инициализации
    }
    ...
    */
    protected static function initVars()
    {
        ...
        self::$CACHE_BLOCKS_FOLDER       = self::$PATH.self::$CACHE_BLOCKS_FOLDER;
    ...
    */
    ...
    /**
     * Возвращает переданный через GET параметр идентификатор сессии.
     *
     * @return string
     */
    public static function getCustomSessionId()
    {
        $user_session_name = $_GET[\Setup::getSessionName()] ?? '';
        if ($user_session_name && preg_match('/^[a-zA-Z0-9]{32}$/', $user_session_name)) {
