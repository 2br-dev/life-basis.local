...
abstract class Crud extends Front
{
    ...
    protected $update_chunk_size = BULK_OPERATION_CHUNK_SIZE;
    protected $timeout = BULK_OPERATION_TIMEOUT;
    ...
    public function actionDel()
    {
        ...
        try {
            $result = $this->runLimitedGroupTask(function ($ids) {
                if (!$this->api->multiDelete($ids)) {
                    $error = $this->api->getElement()->getErrorsStr()
                            .$this->api->getErrorsStr();
    ...
    /**
     * Перенаправляет пользователя на главную страницу контроллера или на страницу
     * указанного в параметре redirect действия
     */
    ...
    /**
     * Возвращает размер одной порции обновления элементов.
     * Деление на порции необходимо, чтобы уложиться в лимит max_execution_time
     *
     * @return integer
     */
    function getUpdateChunkSize()
    {
        return $this->update_chunk_size;
    }
     ...
     */
    function setUpdateChunkSize($chunk_size)
    {
        $this->update_chunk_size = $chunk_size;
    }
        ...
        if ($this->url->isPost() && !empty($ids)) {
            ...
            if (!empty($post)) {
                ...
                if ($obj->checkData($post, [], [], $doedit)
                    ...
                    //Производим порционное обновление элементов
                    try {
                        $result = $this->runLimitedGroupTask(function ($ids_chunk_one) use ($post, $doedit, $prototype) {
                            if ($this->sqlMultiUpdate) {
                                $this->api->clearFilter();
                                $this->api->setFilter($this->api->getIdField(), $ids_chunk_one, 'in');
                                $this->api->multiUpdate($post, $ids_chunk_one);
                            } else {
                                foreach ($ids_chunk_one as $id) {
                                    $prototype->load($id);
                                    $prototype->setCheckFields($doedit);
                                    $prototype->usePostKeys($doedit);
                                    $prototype->save($id, $post, [], []);
                                    if ($prototype->hasError()) {
                                        $prototype->addError(t("Во время обработки элемента %0 произошла ошибка", [$id]));
                                        throw new Exception($prototype->getErrors());
                                    }
                                }
                            }
                        },
     ...
     *
     * Разбивает список ids на части по chunk_size, и вызывает для каждой части callback.
     * Как только наступит timeout, возвращает следующее смещение offset.
     *
     * Ошибку, необходимо бросать через \RS\Exception внутри callback.
     *
     * @param callback $callback Функция, выполняемая для одного куска ids
     * @param array $ids Полный набор id элементов, к которым нужно выполнить callback
     * @param integer $chunk_size Размер одного куска ids
     * @param integer $offset Кол-во кусков, которые необходимо пропустить
     * @param integer $timeout Таймаут в секундах
     * @return bool|integer Возвращает true, в результате успешного исполнения callback для всех ids,
     * либо число - смещение offset, которое нужно передать в следующем вызове данного метода
     */
    protected function runLimitedGroupTask($callback, $ids, $chunk_size, $offset, $timeout, $session_key = null)
    {
        $session_queue_key = $session_key ?? 'queue_'.$this->getControllerName().$this->getAction();
        if (!$offset) {
            //Сохраняем в сессию первоначальную выборку ID, так как они далее будут удаляться
            $_SESSION[$session_queue_key] = $ids;
        } else {
            $ids = $_SESSION[$session_queue_key] ?? [];
        }

        $start = time();
        $ids_chunk = ($chunk_size > 0 && $timeout)
            ? array_chunk($ids, $chunk_size) : [$ids];

        for($n = $offset; $n < count($ids_chunk); $n++) {
            if (isset($ids_chunk[$n])) {
                $ids_chunk_one = $ids_chunk[$n];

                call_user_func($callback, $ids_chunk_one);

                if ($timeout && ($n + 1 < count($ids_chunk)) && (time() - $start) > $timeout) {
                    return $n + 1;
                }
            }
        }

        //Очищаем очередь из сессии по завершению выполнения
        unset($_SESSION[$session_queue_key]);
        return true;
    }
}
