...
abstract class Front extends AbstractClient
{
        ...
        if ($this->wrap_output && substr($this->act, 0, 4) != 'ajax' && $this->url->request('ajax', TYPE_INTEGER) != 1) {
            ...
            if ($this->user->isAdmin() && $this->user->checkSiteRights(SiteManager::getSiteId())) {
                ...
                if (ConfigLoader::getSystemConfig()->show_debug_header) {
                    $has_debug_right = defined('Main\Config\ModuleRights::RIGHT_DEBUG_MODE') ?
                        $this->user->checkModuleRight('main', ModuleRights::RIGHT_DEBUG_MODE) : true;

                    $has_cache_clean_right = defined('Main\Config\ModuleRights::RIGHT_CLEAN_CACHE') ?
                        $this->user->checkModuleRight('main', ModuleRights::RIGHT_CLEAN_CACHE) : true;

                    $debug_top_wrapper = new ViewEngine();
                    $debug_top_wrapper->assign([
                        'has_debug_right' => $has_debug_right,
                        'has_cache_clean_right' => $has_cache_clean_right,
                        'timing' => $timing,
                        'result_html' => $result
    ...
    */
    ...
    {
        if ($id == $item[$id_field] && $item[$id_field] != $item[$alias_field] && !empty($item[$alias_field])) {
