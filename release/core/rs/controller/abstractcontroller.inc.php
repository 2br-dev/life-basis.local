...
class AbstractController implements IController
{
    ...
    function wrapHtml($body, $html_template = null)
    {
        ...
        $wrapped_html = $html->fetch($html_template);

        $result = EventManager::fire('controller.afterwrap', [
            'controller' => $this,
            'html' => $wrapped_html,
        ]);

        return $result->getResult()['html'];
    }
    ...
    function exec($returnAsIs = false)
    {
        ...
        $result = EventManager::fire('controller.afteraction.' . $this->getUrlName(), [
            'controller' => $this,
            'action' => $this->act,
            'result' => $result
        ])->getResult()['result'];

        //Событие для совместимости со старыми версиями
