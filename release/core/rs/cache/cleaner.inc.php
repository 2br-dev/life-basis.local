...
class Cleaner
{
    const
        ...
        CACHE_TYPE_BLOCKS = 'blocks', //Кэш - настройки отрендеренных блоков
    ...
    */
    function clean($type = null)
    {
        if ($type === null || $type == self::CACHE_TYPE_FULL) {
            ...
            if (\RS\Module\Manager::staticModuleExists('designer')
                && \RS\Module\Manager::staticModuleEnabled('designer')){
                $block_api = new \Designer\Model\BlocksApi();
                $block_api->clearResourceCacheFolder();
            }

            //Проверка необходима для корректного обновления
            if (method_exists('\Autoload', 'cleanClassMap')) {
                \Autoload::cleanClassMap();
            }
            return true;
        } 
    ...
    */
    ...
    {
        Tools::deleteFolder(\Setup::$CACHE_BLOCKS_FOLDER);
    }
