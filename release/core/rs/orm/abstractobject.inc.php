...
abstract class AbstractObject extends AcceptBehavior implements \ArrayAccess, \Iterator
{
    ...
    */
    ...
    /**
     * Позволяет устанавливать свойства ORM объекта через синтаксис $orm->property = 'value'
     * Идентично $orm['property'] = 'value'
     * 
     * @param string $offset Имя свойства
     * @param mixed $value Значение
     * @return mixed
    ...
    /**
     * Возвращает значения свойства ORM Объекта через синтаксис $value = $orm->property
     * Идентично $value = $orm['property']
     *
     * @param string $offset Имя свойства
    ...
    */
    ...
    function getForm(array $tpl_vars = null, $switch = null, $is_multiedit = false, $template = null, $tpl_maker = null, $tpl_folder = null)
    {
        ...
        if ($template === null) {
            ...
            $template = $tpl_folder . '/form/' . $me . $filename . '_' . strtolower((string)$switch) . $hash . '.auto.tpl';
        }
