...
class PropertyIterator implements \ArrayAccess, \Iterator
{
    ...
    */
     ...
     *
     * @param string $key Ключ, который следует перенести
     * @param string $to_key Ключ, куда следует перенести поле $key
     * @param bool $after $after - Если true, то вставка будет после $to_key, если false, то перед ним
     */
    public function move($key, $to_key, $after = true)
    {
        if (isset($this->data[$key])) {
            ...
            if (isset($indexes[$to_key])) {
                $index_for_insert = $indexes[$to_key] + (int)$after;

                $this->data = array_slice($this->data, 0, $index_for_insert)
                    + [$key => $tmp]
                    + array_slice($this->data, $index_for_insert);
                ...
                foreach ($this->groups as $name => $items) {
                    ...
                    foreach ($items as $item_key => $value) {
                        ...
                        if ($to_key == $item_key) {
                            $group_name = $name;
                            $to_index = $i + (int)$after;
                        }
                        $i++;
                    }
                }

                $this->groups[$group_name] = array_slice($this->groups[$group_name], 0, $to_index)
                    + [$key => $tmp]
                    + array_slice($this->groups[$group_name], $to_index);
            }
        }
    }
