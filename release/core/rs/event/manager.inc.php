...
class Manager 
{
    ...
    */
    public static function init()
    {
        ...
        if (SiteManager::getSite() !== false) {
            self::$base = CacheManager::obj()
                            ->expire(0)
                            ->request([__CLASS__, 'loadBase'], $is_admin_zone, SiteManager::getSiteId());
    ...
    /**
     * Добавляет в список слушателей событий модули, которые
     * подписаны на событие cron и включены хотя бы на одном мультисайте.
     */
    public static function initCronListeners()
    {
        if (count(SiteManager::getSiteList()) > 1) {
            $base_before = self::$base;

            $module_manager = new ModuleManager();
            $all_modules = $module_manager->getActiveOnAnyMultisiteList();

            foreach ($all_modules as $module) {
                $module->initHandlers();
            }

            if (isset(self::$base['cron'])) {
                $base_before['cron'] = self::$base['cron'];
                uasort($base_before['cron'], [__CLASS__, 'cmpItems']);
            }

            self::$base['cron'] = $base_before['cron'];
        }
    }
    ...
    */
    ...
    {
        $module_manager = new ModuleManager();
        $active_modules = $module_manager->getActiveList(SiteManager::getSiteId());
