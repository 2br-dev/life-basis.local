...
class Tools
{
    ...
    */
    public static function generatePassword($len, $symb = null)
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            try {
                $random_value = random_int(1, $max);
            } catch (Exception) {
                $random_value = rand(1, $max);
            }
            $pass .= $symb[$random_value-1];
        }
        return $pass;
    }
    ...
    */
     ...
     *
     * %k на слово "сегодня, или вчера или 2 дня назад, или дата если больше месяца назад"
     * %v на месяц на русском языке в род. падеже. например: мая, июня
     * %dw на день недели из двух символов: пн,вт,ст...вс
     * %dW на день недели целиком: понедельник...воскресенье
     * %!Y будет отображен год, только если он не равен текущему году
     * %datetime на заменяет на ДД.ММ.ГГГГ ЧЧ:ММ:СС
     * @date заменяет на ДД.ММ.ГГГГ
     * @time заменяет на ЧЧ:ММ
     * @sec заменяет на секунды (два знака)
     *
     * @param string $format
    ...
    public static function dateExtend($format, $timestamp)
    {
        ...
        //%V будет заменен на месяц на русском языке в предложном. падеже. например: мае, июне
        if (strpos($format, '%V') !== false) {
            $months = [1 => t('январе'), t('феврале'), t('марте'), t('апреле'), t('мае'), t('июне'),
                t('июле'), t('августе'), t('сентябре'), t('октябре'), t('ноябре'), t('декабре')];
            $format = str_replace('%V', $months[date('n', $timestamp)], $format);
        }

        //%j будет заменен на месяц на русском языке в им. падеже. например: май, июнь
        if (strpos($format, '%j') !== false) {
            $months = [1 => t('январь'), t('февраль'), t('март'), t('апрель'), t('май'), t('июнь'),
                t('июль'), t('август'), t('сентябрь'), t('октябрь'), t('ноябрь'), t('декабрь')];
            $format = str_replace('%j', $months[date('n', $timestamp)], $format);
        }
        ...
        if (strpos($format, '@time') !== false) {
            $format = str_replace('@time', 'H:i', $format);
        }
        ...
        if (strpos($format, '%datetime') !== false) {
            $format = str_replace('%datetime', 'd.m.Y H:i:s', $format);
        }
    ...
    */
     ...
     * @param string $format форматирование записи валюты
     * @param string $format_decimals форматирование записи дробной части валюты
     * @return string
     */
                                         ...
                                         $format_decimals = '[plural:%0:копейка|копейки|копеек]')
    {
       ...
       $cur = t($format, [$price]); //Плюрал по рублям
       ...
       $string_kopeiki = t($format_decimals, [(int)$kopeiki]);
