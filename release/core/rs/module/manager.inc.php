...
class Manager
{
    ...
    */
     ...
     * @return mixed|Item[]
     */
    public function getActiveOnAnyMultisiteList($cache_enabled = true)
    {
        if (!\Setup::$INSTALLED) {
            //Во время установки - все модули активные
            return $this->getList($cache_enabled);
        ...
        if ($cache_enabled) {
            ...
            return CacheManager::obj()
                ->expire(0)
                ->tags(CACHE_TAG_MODULE)
                ->request([$this, __FUNCTION__], false, $is_admin_zone);
        } else {
            $modules = $this->getList();
            return array_filter($modules, function(ModuleItem $module) {
                return $module->isActiveOnAnyMultisite();
            });
        }
    }
