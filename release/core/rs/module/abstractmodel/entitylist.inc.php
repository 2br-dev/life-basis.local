...
class EntityList extends BaseModel
{
    ...
    function multiUpdate(array $data, $ids = [])
    {
        ...
        if (!empty($data)) {
            ...
            EventManager::fire('orm.multiupdate.'.$this->getElement()->getShortAlias(), [
                'ids' => $ids,
                'api' => $this
            ]);
        }
    ...
    {
        if (!isset($this->cache_alias[$alias_or_id])) {
            ...
            $this->cache_alias[$alias_or_id] = ($row = $res->fetchRow()) ? $row[$this->id_field] : false;
        }
        return $this->cache_alias[$alias_or_id];
    }
    ...
    {
        $q = clone $request;
        $q->select = 'DISTINCT '.$this->defAlias() . '.' . $this->id_field;
        $result = $q->limit(null)
          ->orderby(null)
          ->exec();
    ...
    /**
     * Обрабатывает установку произвольных свойств
     *
