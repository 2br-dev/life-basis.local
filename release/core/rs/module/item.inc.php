...
class Item
{
                    ...
                    if (preg_match('/(.*?)\.(.*)/', $file, $matches)) {
                        if ($matches[2] == \Setup::$CLASS_EXT) {
                            ...
                            if (class_exists($class_name) && is_subclass_of($class_name, '\RS\Controller\Block')) {
                                if ($class_name::isVisibleOnSiteConstructor()) {
                                    $ret[] = [
                                        'class' => $class_name,
    ...
    */
    public static function parseModuleXml($file)
    {
        $result = [];
        try {
            $config = @new \SimpleXMLElement($file, 0, true);
     ...
     * @return bool
     */
    public function isActiveOnAnyMultisite()
    {
        $site_list = \RS\Site\Manager::getSiteList();
        foreach($site_list as $site) {
            $config = $this->getConfig($site['id']);
            if ($config->isActive()) {
                return true;
            }
        }
