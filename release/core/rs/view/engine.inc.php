...
class Engine extends \Smarty
{
    ...
    function __construct()
    {
        ...
        $this->addPluginsDir(\Setup::$PATH.'/core/system/smarty/rsplugins/');
        $this->registerClass('ModuleManager', '\RS\Module\Manager');
        $this->registerClass('ConfigLoader', '\RS\Config\Loader');

        foreach($this->allow_modifiers_function as $name => $function) {
            if (is_array($function) && isset($function['callback'])) {
                $this->registerPlugin('modifier', $name, $function['callback']);
            } else {
