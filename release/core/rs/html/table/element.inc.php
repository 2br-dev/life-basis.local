    ...
    */
    ...
    {        
        ...
        if (is_array($data)) {
            ...
            if (is_callable($this->after_load_data_callback)) {
                call_user_func($this->after_load_data_callback, $this, $data);
            }
        }
    }

    /**
     * Устанавливает callback, который будет вызван после загрузки данных
     *
     * @param $callable
     * @return void
     */
    function setAfterLoadDataCallback($callable)
    {
        $this->after_load_data_callback = $callable;
    }
