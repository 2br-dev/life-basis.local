...
abstract class AbstractLog
{
     ...
     */
    protected function __construct($site_id = 0)
    {
        $this->setSiteId((int)$site_id);
    ...
    {
        $site_id = (int)($site_id ?? SiteManager::getSiteId());
        $self = get_called_class().$site_id;
        if (!isset(self::$instances[$self])) {
            self::$instances[$self] = new static($site_id);
    ...
    public final function getLogLevelList(): array
    {
        ...
        if (!isset($list[$self])) {
            $list[$self] = $this->selfLogLevelList();
        }
        return $list[$self];
    }
