...
class Core
{
    ...
    */
     ...
     *
     * @param mixed $origFileName имя оригинального файла
     * @param mixed $direction направление
     * @throws Exception
     */
    function flip($origFileName, $direction = 'horizontal')
    {
        $original_file = new File($this->getOriginalFilename($origFileName));
        $direction_int =  $direction == 'vertical' ? IMG_FLIP_VERTICAL : IMG_FLIP_HORIZONTAL;
        $flipped_success = imageflip($original_file->image_handler, $direction_int);
        imagejpeg($original_file->image_handler, $original_file->filename, 100);
        $this->removeFile($origFileName, true);
    }
}