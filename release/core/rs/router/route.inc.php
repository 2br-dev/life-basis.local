    ...
    */
    public function buildUrl($params = [], $absolute=false, $mask_key = null)
    {
        ...
        if (!$this->isAdmin()) { //Не дополняем пути в администраторской панели
            $site_folder = ($site && $site['folder'] != '') ? '/'.trim($site['folder'], '/') : '';
            $mask = $site_folder.$mask;
        }
        
        //Приписываем папку, в которой установлена система
        $folder = rtrim((string)\Setup::$FOLDER, '/');
        if ($folder !== '') {
            $mask = $folder.$mask;
        }       
        ...
        if ($absolute) {
            $uri = $site ? $site->getAbsoluteUrl($uri) : \RS\Http\Request::commonInstance()->getSelfAbsoluteHost().$uri;
        }        
        return $uri;
    }
