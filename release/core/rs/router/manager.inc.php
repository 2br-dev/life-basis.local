...
class Manager
{
    ...
    */
    ...
    {
        return SiteManager::getSite()->getRootUrl($absolute);
    }
    ...
    */
    ...
    /**
     * Возвращает URI, поэтапно очищенный от папки Setup::$FOLDER и подпапки мультисайта
     * Именно такой URL подается на мэтчинг в роутер
     *
     * @return string
     */
    public function getCleanUri()
    {
        ...
        $folder = str_replace('/','\\/', (string)\Setup::$FOLDER);
        $uri = preg_replace('/^'.$folder.'/', '', $uri);

        //Исключаем подпапку, которая задана у текущего мультисайта
        $site = SiteManager::getSite();
        if ($site && $site['folder'] != '') {
            $uri = preg_replace('/^\\/'.$site['folder'].'/', '', $uri);
        }
     ...
     * @return string
     */
    public function buildAbsoluteUrl($relative_url)
    {
