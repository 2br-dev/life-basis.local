...
{/if}
    ...
    <a {if $local_options.is_virtual}data-{/if}href="{$paginator->left}" class="prev {if !$local_options.is_virtual && !$local_options.no_ajax}call-update{/if} {if $paginator->isNoUpdateUrl()}no-update-hash{/if} zmdi zmdi-chevron-left" {if $paginator->getUpdateContainer()}data-update-container="{$paginator->getUpdateContainer()}"{/if} title="{t}предыдущая страница{/t}"></a>
    <input type="text" class="page" name="{$paginator->page_key}" value="{$paginator->page}" onfocus="$(this).select()">

    <a {if $local_options.is_virtual}data-{/if}href="{$paginator->right}" class="next {if !$local_options.is_virtual && !$local_options.no_ajax}call-update{/if} {if $paginator->isNoUpdateUrl()}no-update-hash{/if} zmdi zmdi-chevron-right" {if $paginator->getUpdateContainer()}data-update-container="{$paginator->getUpdateContainer()}"{/if} title="{t}следующая страница{/t}"></a>
