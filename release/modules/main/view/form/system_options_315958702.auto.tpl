        ...
        <form method="POST" action="{urlmake}" enctype="multipart/form-data" class="tab-content crud-form">
            <input type="submit" value="" style="display:none"/>
                        <div class="tab-pane active" id="rs-config-cms-tab0" role="tabpanel">
                                ...
                                <tr>
                                    <td class="otitle">{$elem.__BULK_OPERATION_CHUNK_SIZE->getTitle()}&nbsp;&nbsp;{if $elem.__BULK_OPERATION_CHUNK_SIZE->getHint() != ''}<a class="help-icon" data-placement="right" title="{$elem.__BULK_OPERATION_CHUNK_SIZE->getHint()|escape}">?</a>{/if}
                                    </td>
                                    <td>{include file=$elem.__BULK_OPERATION_CHUNK_SIZE->getRenderTemplate() field=$elem.__BULK_OPERATION_CHUNK_SIZE}</td>
                                </tr>
                                
                                                                                                                            
                                <tr>
                                    <td class="otitle">{$elem.__BULK_OPERATION_TIMEOUT->getTitle()}&nbsp;&nbsp;{if $elem.__BULK_OPERATION_TIMEOUT->getHint() != ''}<a class="help-icon" data-placement="right" title="{$elem.__BULK_OPERATION_TIMEOUT->getHint()|escape}">?</a>{/if}
                                    </td>
                                    <td>{include file=$elem.__BULK_OPERATION_TIMEOUT->getRenderTemplate() field=$elem.__BULK_OPERATION_TIMEOUT}</td>
                                </tr>
