...
<div class="export-csv">
    <form class="crud-form" method="POST" action="{adminUrl schema=$schema}" data-dialog-options='{ "width":"1000", "height":"600" }'>
        ...
        <div class="presets d-flex flex-between direction-column-xs gap-10">
            <div>
                <span class="m-r-5">{t}Шаблон{/t}</span>
                <select class="maps va-m-c">
            ...
            <div>
                <span class="m-r-5">{t}Формат{/t}
                <a class="help-icon"
                   title="{t}CSV - не требователен к ресурсам. XLS, XLSX, ODS - требуют большого объема памяти (Настройка PHP - memory_limit на хостинге).{/t}">?</a>
                </span>
                <select class="format va-m-c m-r-10" name="format">
