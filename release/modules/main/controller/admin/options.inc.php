...
*/
class Options extends ConfigEdit
{
    protected $orm;
    ...
    {
        $this->cms_config = Loader::getSystemConfig();
    ...
    */
    ...
    /**
     * Очищает информацию о количестве неудачных попыток авторизации для всех IP
     *
     * @return \RS\Controller\Result\Standard
     */
    function actionResetAuthTry()
    {
        $blocked_api = new BlockedIpApi();
        $count = $blocked_api->resetTryAuth();

        return $this->result
            ->addSection('noUpdate', true)
            ->addMessage(t('Сброшена информация о количестве попыток авторизации для %0 IP-адресов.', [$count]));
    }
}
