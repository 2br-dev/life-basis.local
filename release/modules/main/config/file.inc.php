...
class File extends ConfigObject
{
    function _init()
    {
        ...
        $this->getPropertyIterator()->append([
            t('Основные'),
                ...
                'microdata_organization' => new Type\Integer([
                    'description' => t('Добавить микроразметку организации на главную страницу'),
                    'hint' => t('На главной странице будет выводиться микроразметка данных об организации:<br>
                                        Название компании, ссылка на логотип, телефон, электронная почта, юридический адрес'),
                    'checkboxView' => [1, 0],
                ]),
            t('Изображения'),
            ...
            t('Сервисы Яндекса'),
                ...
                'yandex_js_api_suggest' =>  (new Type\Varchar())
                    ->setDescription(t('Ключ для "API Геосаджеста"')),
            t('Сервисы Google'),
    ...
    /**
     * Добавляет необходимый скрипт, отвечающий за показ карты в соответствии с настройками в амдинистративной панели
     *
     * @return void
     */
    ...
    /**
     * Добавляет глобальные переменные с ключами к JavaScript API Карт
     *
     * @return void
     */
    public function initMapJsVars()
    {
        $js_vars = [
            'mapParams' => [
        ...
        switch ($this['map_type']) {
            case 'yandex':
                $js_vars['mapParams']['yandexJsApiGeocoder'] = (string)$this['yandex_js_api_geocoder'];
                break;
            case 'google':
                $js_vars['mapParams']['googleApiKeyMap'] = (string)$this['google_api_key_map'];
                break;
        }
