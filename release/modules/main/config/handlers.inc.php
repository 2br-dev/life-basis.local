...
class Handlers extends HandlerAbstract
{
    ...
    {
        $routes[] = new Router\Route('main.manifest', '/manifest.json', [
            'controller' => 'main-front-manifest'
        ], t('Информация из файла manifest.json'), true);
    ...
    static function start()
    {
        ...
        $config->initMapJsVars();
    }

    /**
     * Выполняет перед добавлением секций head, body к полученному HTML
     *
     * @param $params
     */
    static function controllerBeforeWrap($params)
    {
        ...
        if ($route && !$route->getExtra(AbstractRelCanonical::ROUTE_EXTRA_DISABLE_REL_CANONICAL)) {
            AbstractRelCanonical::getRealCanonicalClass()->onControllerBeforeWrap();
        }

        if ($site_config['manifest_name'] && !$router->isAdminZone()) {
            $manifest_url = $router->getUrl('main.manifest');
            Application::getInstance()->addCss($manifest_url, 'manifest', BP_ROOT, true, [
                'rel' => 'manifest'
            ]);
        }
    }
