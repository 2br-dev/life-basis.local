  ...
  <ion-toolbar>
    <div class="container">
      <div class="head">
        ...
        <div class="head__title" *ngIf="!inLoading && order">{t}Заказ №{/t}{ { order.orderNum}} {t}от{/t} { { formatDate(order.dateof) } }</div>
        ...
        <ion-grid>
          <ion-row>
            ...
            <ion-col size-lg="5" size-md="6" offset-lg="1" size="12">
              <div class="margin-24-left-table">
                <div class="margin-32-bottom">
                  <div class="order-status order-status_payment" *ngIf="needPay()">
                    <div class="fw-bold">{t}Заказ ожидает оплаты{/t}</div>
                    <button
                  ...
                  <div class="order-status order-status_repeat margin-8-top" *ngIf="isPayed()">
                    <div class="fw-bold">{t}Заказ оплачен{/t}</div>
                    <button class="order-status__button" type="button" (click)="repeatOrder()">{t}Повторить{/t}</button>
                  </div>
                  {if $client_version >= 2.7}
                    <div class="order-status order-status_review margin-8-top" *ngIf="canReview && reviewConfig">
                      <div class="fw-bold">{t}Оставьте, пожалуйста, отзыв{/t}</div>
                      <button class="order-review__button" type="button" (click)="openOrderReview()">{t}Отзыв{/t}</button>
                    </div>
                  {/if}
                </div>
                    ...
                    <div class="margin-32-bottom">
                      <div class="order-product" *ngFor="let productItem of order.productItems; let i=index">
                        ...
                        <div class="order-product__price-wrap">
                          ...
                          <div class="order-product__price">{ { productItem.priceFormatted } }</div>
                        </div>
                        {if $client_version >= 2.7}
                          <div class="order-product__review-wrap margin-16-top" *ngIf="getProductReview(productItem.entityId)['message'] && reviewConfig">
                            <div class="order-product__review-message">
                                <span class="order-review__star">
                                  { { getProductReview(productItem.entityId).rate } }
                                </span>
                              <article>{ { getProductReview(productItem.entityId).message } }</article>
                            </div>
                          </div>
                        {/if}
                      </div>
                ...
                <div class="order-info">
                  {if $client_version >= 3.5}
                    <div class="order-info__item-status" *ngIf="order && order.getStatus()">
                      <div>{t}Статус:{/t}</div>
                      <div [ngStyle]="{ background: order.getStatus().bgcolor }" class="status">{ { order.getStatus().title } }</div>
                    </div>
                  {/if}
                    ...
                    <div *ngIf="order.getOrderDocs()">
                      <div class="order-info__item">
                        ...
                        <div class="c-dark">
                          <a class="order-item__receipt" *ngFor="let doc of order.getOrderDocs()" (click)="openDocument(doc.link)">{ { doc.title } }</a>
                        </div>
                      </div>
                    </div>
                  {if $client_version >= 2.7}
                    <div class="order-info__item" *ngIf="orderReview && orderReview.message && reviewConfig">
                      <div class="order-info__key">{t}Отзыв к заказу{/t}</div>
                      <div class="c-dark order-review__message">
                          <span class="order-review__star">
                            { { orderReview.rate } }
                          </span>
                        <article>{ { orderReview.message } }</article>
                      </div>
                    </div>
                  {/if}
                </div>
                <div class="margin-24-top margin-24-bottom" *ngIf="canShowChangePaymentButton()">
                  <button
                    type="button"
                    class="button button_default button_small w-100"
                    (click)="openChangePayment()"
                  >{t}Изменить способ оплаты{/t}</button>
                </div>
                  ...
                  <div *ngIf="order.otherItems && order.otherItems.length">
                    ...
                    >
                      ...
                      <div class="fw-semibold text-nowrap">
                        <span *ngIf="item.type === 'order_discount'">-</span>
                        { { item.priceFormatted } }
                      </div>
                    </div>
                  </div>
