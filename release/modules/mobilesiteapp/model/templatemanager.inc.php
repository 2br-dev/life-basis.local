...
class TemplateManager{
    ...
    */
     ...
     *
     * @param string $mobilesiteapp_version - версия мобильного приложения
     * @return array
     * @throws Exception
     */
    function getAllTemplatesPaths($mobilesiteapp_version = 'v1')
    {
        ...
        //Оставляем шаблоны текущей версии приложения
        foreach ($paths_array as $key => $path) {
            if (!strpos($path, $mobilesiteapp_version)) {
                unset($paths_array[$key]);
            }
        }
     ...
     */
    function getTemplatesJSONPrepared($view, $mobilesiteapp_version = 'v1')
    {
        //Получим все ключи шаблонов
        $paths_keys = $this->getAllTemplatesPaths($mobilesiteapp_version);
        $paths = [];
        ...
        if (!empty($paths_keys)){
            foreach ($paths_keys as $path_key){
                ...
                $html = $view->fetch("%MOBILEPATH%/view/pages/{$mobilesiteapp_version}/wrapper.tpl");
                $paths[str_replace($mobilesiteapp_version . '/', '', $path_key)] = $html;
            }
        }
        return $paths;
    }
    ...
    /**
     * Возвращает массив шаблонов для мобильного приложения
     *
     * @return array
     * @throws Exception
     * @throws \RS\Event\Exception
     * @throws \SmartyException
     */
    function getClientSiteAppTemplates($version)
    {
        $path_keys = $this->getAllTemplatesPaths('v' . $version);
        $view = new ViewEngine();
        $paths = [];
        ...
        if (isset($_GET['client_version'])){ //Если это в параметрах
            $client_version = $url->request('client_version', TYPE_FLOAT, 0);
            $client_name    = $url->request('client_name', TYPE_STRING, '');
        }else{ //Если нет, то смотрим в заголовках
            $client_version = $url->server('X-Client-Version', TYPE_FLOAT, 0);
            $client_name    = $url->server('X-Client-Name', TYPE_STRING, '');
        }
        ...
        if (!empty($path_keys)) {
            foreach ($path_keys as $path_key) {
                ...
                $wrapped_content = str_replace(['{ { ', ' } }', '{ ', ' }'], ['{{', '}}', '{', '}'],$view->fetch($template));
                $paths[str_replace('v' . $version . '/', '', $path_key)] = $wrapped_content;
            }
        }
        return $paths;
    }
