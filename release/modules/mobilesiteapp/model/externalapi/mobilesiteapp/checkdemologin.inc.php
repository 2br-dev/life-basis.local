...
class CheckDemoLogin extends \ExternalApi\Model\AbstractMethods\AbstractMethod
{
    ...
    protected $mobilesiteapp_config;

    public function __construct()
    {
        parent::__construct();
        $this->config = Loader::byModule('externalapi');
        $this->shop_config = Loader::byModule('shop');
        $this->catalog_config = Loader::byModule('catalog');
        $this->mobilesiteapp_config = Loader::byModule('mobilesiteapp');
    }
    ...
    */
    ...
    /**
     * Возвращает список настроек API
     * используется в первой версии приложения
     *
     * @return array
     */
    protected function getApiUrls()
    {
        ...
        $params['data']['category_list_default_order']           = $this->catalog_config['list_default_order'];
        $params['data']['category_list_default_order_direction'] = $this->catalog_config['list_default_order_direction'];
        ...
        $params['data']['mobilesiteapp_products_pagesize'] = $this->mobilesiteapp_config['products_pagesize'];
        ...
        $params['data']['mobile_phone']       = $this->mobilesiteapp_config['mobile_phone'];

        return $params;
    }

    /**
     * Возвращает информацию, необходимую для подключения к демо
     *
     * @return array
     */
    protected function getPlatformInfo()
    {
        $params['platform_data']['package'] = \Setup::$SCRIPT_TYPE;
        $params['platform_data']['externalapi_config'] = \ExternalApi\Model\Utils::extractOrm($this->config);
        $params['platform_data']['shop_config'] = \ExternalApi\Model\Utils::extractOrm($this->shop_config);
        $params['platform_data']['catalog_config'] = \ExternalApi\Model\Utils::extractOrm($this->catalog_config);
        $params['platform_data']['mobilesiteapp_config'] = \ExternalApi\Model\Utils::extractOrm($this->mobilesiteapp_config);
    ...
    {
        $params =
            $this->params && isset($this->params['v']) && $this->params['v'] >= '2'
