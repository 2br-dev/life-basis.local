...
class Gate extends \RS\Controller\Front
{
    ...
    */
    ...
    function actionTemplate()
    {
        ...
        $template = str_replace("pages/", "pages/{$this->config->mobilesiteapp_version}/", $template);
        ...
        if ($this->template_api->checkTemplateExists($template_full_path)){
            ...
            $this->result->setTemplate($this->template_api->getCurrentThemeTemplatesFolder()."/pages/{$this->config->mobilesiteapp_version}/".'wrapper.tpl');
        }else{
            $this->result->setHtml('');
        }
        return $this->result;
