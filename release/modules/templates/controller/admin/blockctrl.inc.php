...
class BlockCtrl extends \RS\Controller\Admin\Crud
{
    ...
    function actionAdd($primaryKeyValue = null, $returnOnSuccess = false, $helper = null)
    {
        ...
        $result = parent::actionAdd($primaryKeyValue, $returnOnSuccess, $helper);

        if ($this->url->isPost()) {
            BlockCacheManager::obj()->invalidateByTags(CACHE_TAG_BLOCK_PARAM);
        }
    ...
    function actionAddSection($primaryKeyValue = null)
    {
        ...
        if ($this->url->isPost() && !$primaryKeyValue && $elem['id'] && $section_id){
            //Если мы сохранили все удачно и нужно перестроить позицию относительно блока
