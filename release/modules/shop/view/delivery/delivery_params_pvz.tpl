{if $pvz_list}
    ...
    {$delivery_extra_value = json_decode(htmlspecialchars_decode(strval($delivery_extra.pvz_data)), true)}
            ...
            <div class="col-md">
                <select id="pvz_delivery" class="form-select rs-checkout_pvzInput rs-field-pvz-input rs-checkout_triggerUpdate" name="delivery_extra[pvz_data]">
                    {foreach $pvz_list as $pvz}
                        <option value='{$pvz->getDeliveryExtraJson()|escape}' data-pvz-code="{$pvz->getCode()}" {if !empty($delivery_extra_value.code) && ($delivery_extra_value.code == $pvz->getCode())}selected{/if}>
