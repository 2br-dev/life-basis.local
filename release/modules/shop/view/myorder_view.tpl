        ...
        <div class="row">
            <div class="col-md-7 mb-6 mb-md-0">
                ...
                <div class="row mb-5 g-4">
                    ...
                    {if !empty($url)}
                        <div>
                            <div class="fw-bold mb-2">{t}Ссылка для отслеживания заказа{/t}:</div>
                            <div>
                ...
                <div class="row row-cols-md-auto row-cols-1 g-3">
                    {hook name="shop-myorder_view:actions" title="{t}Просмотр заказа:действия над одним заказом{/t}"}
                        {if $order->canOnlinePay()}
                            <div>
                                <a href="{$order->getOnlinePayUrl()}" class="btn btn-primary col-12">{t}Оплатить заказ{/t}</a>
                            </div>
                        {/if}
                            <div>
                                <a href="{$router->getUrl('shop-front-cartpage', ['Act'=>'repeatOrder', 'order_num' => $order.order_num])}" rel="nofollow" class="btn btn-outline-primary col-12">{t}Повторить заказ{/t}</a>
                            </div>
                        {if $order->canChangePayment()}
                            <div>
                                <a href="{$router->getUrl('shop-front-myorderview', ['Act'=>'changePayment', 'order_id' => $order.order_num])}" rel="nofollow" class="btn btn-outline-primary col-12 rs-in-dialog">{t}Изменить оплату{/t}</a>
                            </div>
                        {/if}
                    {/hook}
                </div>
                    ...
                    <div class="collapse" id="orderItemsList" data-bs-target="orderItemsTitle">
                        <div class="pt-4">
                            <div class="order-items__list">
                                {hook name="shop-myorder_view:product-items" title="{t}Просмотр заказа:информация о товаре{/t}"}
                ...
                <div class="lk-order-total">
                    {foreach $order_data.other as $item}
                        <div class="lk-order-total__item">
                            ...
                            <div class="no-wrap fw-bold">{if $item.total_unformatted >0}{$item.total}{/if}</div>
                        </div>
                    {/foreach}
