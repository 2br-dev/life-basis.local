...
<div class="rs-order-shipment"
    ...
    <ul class="blank-uit-list-item hidden">
        <li class="uit-list-item" data-id="%gtin%serial">
            ...
            <input name="uit[%cart_item_uniq][%gtin%serial][other]" type="hidden" value="%other">
            <div class="uit-list-item-check" style="color:%status_color">
                <i class="f-16 zmdi zmdi-%status_icon" title="%status_text"></i>
            </div>
            <div class="uit-list-item-group">
            ...
            <div>
                <div class="uit-list-item-remove rs-icon-cross"></div>
            </div>
        </li>
    </ul>
                    ...
                    <div class="column-uit order-shipment-item-uit">
                        {if $product.marked_class}
                            <ul class="order-shipment-item-uit-list">
                                {foreach $cart_item->getUITs() as $uit}
                                    ...
                                    <li class="uit-list-item" data-id="{$uit_id}">
                                        ...
                                        <input name="uit[{$cart_item.uniq}][{$uit_id}][other]" type="hidden" value="{$uit.other}">

                                        {$uit_check_result = $uit->getCheckResult()}
                                        {$check_status = $uit_check_result->getCheckStatus()}
                                        <div class="uit-list-item-check" style="color: {$uit_check_result->getCheckStatusColor()}">
                                            <i class="f-16 zmdi zmdi-{$uit_check_result->getCheckStatusRsIcon()}" title="{$uit_check_result->getCheckText()}"></i>
                                        </div>
                                        <div class="uit-list-item-group">
                                            <span class="uit-list-item-field">
                                                <span class="uit-list-item-field-title">gtin:</span>
                            ...
                            <div class="order-shipment-item-uit-actions">
                                <input type="text" class="order-shipment-item-uit-input" data-app-scan="data_matrix"
                                       data-app-scan-filter="marking">
