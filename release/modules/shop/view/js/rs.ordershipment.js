class orderShipment {
    ...
    itemAddUIT(item, data) {
        ...
        let other = data[this.dataKeys.other].replace(/\"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
        ...
        html = html.replace(new RegExp('%other', 'g'), other);

        html = html.replace(new RegExp('%status_text', 'g'), data.checkResult.text);
        html = html.replace(new RegExp('%status_color', 'g'), data.checkResult.color);
        html = html.replace(new RegExp('%status_icon', 'g'), data.checkResult.icon);

        list.insertAdjacentHTML('beforeend', html);
        $(list).trigger('new-content');
        this.checkShipment();
    }
