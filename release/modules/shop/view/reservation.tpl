...
{block "body"}
    ...
    <form method="POST" action="{$router->getUrl('shop-front-reservation', ["product_id" => $product.id])}">
        <div class="modal-item mb-4">
            <div class="mb-2">{$product.title} {$product->getBarCode($reserve.offer_id)}</div>
                    ...
                    {if $product->isMultiOffersUse()}
                        ...
                        {foreach $offers_levels as $level}
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ms-1">
                                    <input name="multioffers[{$level.prop_id}]" value="{$reserve.multioffers[$level.prop_id]}" readonly type="hidden">
        ...
        <input type="hidden" name="product_barcode" value="{$product->getBarCode($reserve.offer_id)}">
