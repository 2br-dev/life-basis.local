...
<div id="rs-cart-page">
    {hook name="shop-cartpage:page" title="Корзина на отдельной странице"}
            ...
            <form class="row" method="POST" action="{$router->getUrl('shop-front-cartpage', ["Act" => "update"])}" id="rs-cart-form">
                <div class="col mb-6 mb-lg-0">
                    ...
                    <div class="fs-5 mt-4 d-flex justify-content-lg-end">
                        ...
                        {if $shop_config.exchange_cart_enable}
                            <a class="ms-3 fs-5 text-decoration-none link-dark" href="{$router->getUrl('shop-front-cartpage', ["Act" => "exportCart"])}" target="_blank">{t}Скачать в CSV{/t}</a>
                        {/if}
                    </div>
                </div>
    ...
    {else}
        <div class="text-center container col-lg-5 col-md-6 col-sm-8">
            <div class="mb-4">
            ...
            <div class="d-sm-flex gap-3 justify-content-center">
                <a href="{$router->getRootUrl()}" class="btn btn-primary rs-go-back mb-2">{t}Вернуться к покупкам{/t}</a>
                {if $shop_config.exchange_cart_enable}
