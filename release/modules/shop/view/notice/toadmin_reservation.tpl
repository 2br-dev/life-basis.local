...
{block name="content"}
    ...
    <p>{t url=$router->getAdminUrl('edit', ["id" => $data->reserve.id], 'shop-reservationctrl', true) order_num=$data->reserve.id}Номер предварительного заказа: <a href="%url"><strong>%order_num</strong></a> от{/t} <strong>{$data->reserve.dateof|date_format:"d.m.Y H:i:s"}</strong></p>
