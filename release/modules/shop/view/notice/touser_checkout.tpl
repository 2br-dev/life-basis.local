...
{block name="content"}
    ...
    <h1 style="line-height: normal;">{t url=$url->getDomainStr()}Вы сделали заказ в интернет-магазине %url.{/t}</h1>
    ...
    {hook name="shop-notice-tousercheckout:infoblock" title="{t}Заказ оформлен:блок информации{/t}" order=$data->order}
    ...
    <table cellpadding="5" border="1" bordercolor="#969696" style="border-collapse:collapse; border:1px solid #969696">
        ...
        <tbody>
            {hook name="shop-notice-tousercheckout:productblock" title="{t}Заказ оформлен:блок товара{/t}" order=$data->order}
        ...
        <tfoot>
            {hook name="shop-notice-tousercheckout:totalblock" title="{t}Заказ оформлен:блок итого{/t}" order=$data->order}
            <tr>
                <td colspan="4"></td>
                <td>
    ...
    <p>{t href=$router->buildAbsoluteUrl($router->getUrl('shop-front-myorders'))}Вы можете изменить свои данные и ознакомиться со статусом заказа в разделе <a href="%href">«Личный кабинет»</a>.{/t}</p>
{/block}