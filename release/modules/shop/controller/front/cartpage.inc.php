...
class CartPage extends Front
{
    ...
    protected function getCartStateData($cart_data)
    {
        return [
            ...
            'all_items_count' => $cart_data['all_items_count'], //Количество товаров с учетом выключенных
     ...
     * @return void
     */
    function actionExportCart()
    {
        $this->wrapOutput(false);
        if (!$this->getModuleConfig()->exchange_cart_enable) {
            $this->e404(t('Функция отключена'));
        }

        $cart_exchange = new CartExchange($this->cart);
        $filename = 'cart-'.date('Y-m-d H-i-s').'.csv';
        ...
        $cart_exchange->export('php://output');
    }

    /**
     * Отображает диалог импорта корзины из CSV
     *
     * @return Standard
     */
    function actionImportCart()
    {
        if (!$this->getModuleConfig()->exchange_cart_enable) {
            $this->e404(t('Функция отключена'));
        }
        ...
        if ($this->url->isPost()) {
            ...
            if ($file && !empty($file['tmp_name'])) {
                ...
                $error = $cart_exchange->checkPreImportError($file['tmp_name'], $file['name']);
                if (!$error) {
                    $this->view->assign([
                        'import_result' => $cart_exchange->import($file['tmp_name']),
                        'report' => $cart_exchange->getImportReport()
                    ]);

                    return $this->result->setTemplate('cart_import_report.tpl');
                }
            } else {
                $error = t('Файл не загружен');
            }
        }

        $this->view->assign([
            'charset' => $this->getModuleConfig()->exchange_cart_charset,
            'error' => $error ?? ''
        ]);

        return $this->result->setTemplate('cart_import_dialog.tpl');
    }
}