...
class OnlinePay extends Front
{
                    ...
                    switch ($payment_type->getRecurringPaymentsType()) {
                        case InterfaceRecurringPayments::RECURRING_TYPE_SAVE_METHOD:
                            ...
                            if (!empty($params['payment_method'])) {
                                $pay_transaction = $payment_type->recurringPayBalanceFounds($user, $cost, $saved_methods[$params['payment_method']]);

                                $redirect = $this->router->getUrl('shop-front-onlinepay', [
                                    'PaymentType' => $payment_type->getShortName(),
                                    'Act' => 'status',
                                    'transaction' => $pay_transaction['id'],
                                ]);
        ...
        if ($payment_type instanceof InterfaceRecurringPayments && $payment_type->isRecurringPaymentsActive()) {
            ...
            switch ($payment_type->getRecurringPaymentsType()) {
                case InterfaceRecurringPayments::RECURRING_TYPE_SAVE_METHOD:
                    ...
                    if (!empty($params['payment_method'])) {
                        $pay_transaction = $payment_type->recurringPayOrder($order, $saved_methods[$params['payment_method']]);
                ...
                case InterfaceRecurringPayments::RECURRING_TYPE_ONLY_SAVE_METHOD:
                    ...
                    if (!empty($params['payment_method'])) {
                        ...
                        $this->app->setBodyAttr('data-payment-method-selected', 1, true);
                    ...
                    return $this->result->setTemplate('%shop%/onlinepay/select_method.tpl');
                default:
                    throw new ShopException(t('Неизвестный тип рекуррентной оплаты'));
            }
        } else {
            $transaction = TransactionApi::getInstance()->createTransactionFromOrder($order['order_num']);
            TransactionChangeLog::new($transaction, t('Транзакция создана'));
    ...
    protected function authorizeByMobileClient()
    {
        ...
        if ($token['user_id']) {
            ...
            $this->app->redirect($this->url->replaceKey(['auth_token' => null, 'clientsiteapp' => null]));
        }
    }
    ...
    function actionSuccess()
    {
        ...
        $this->app->setBodyAttr('data-payment-ok', 1, true); //Добавим атрибут для проверки в мобильным приложением v2
    ...
    function actionFail()
    {
        ...
        $this->app->setBodyAttr('data-payment-fail', 1, true); //Добавим атрибут для проверки в мобильным приложением v2
    ...
    public function actionStatus()
    {
        ...
        $this->app->setBodyAttr('data-payment-status', 1, true); //Добавим атрибут для проверки мобильным приложением v2
        $this->app->setBodyAttr('data-transaction-id', $transaction->id, true); //Добавим атрибут для проверки мобильным приложением v2
