...
class Checkout extends StandartBlock
{
    ...
    public function actionUpdate()
    {
        ...
        $this->order->clearDeliveryCache();
        $this->order->clearPaymentCache();
    ...
    public function actionAddressAutocomplete(): string
    {
        ...
        if ($term) {
            ...
            /* Ограничение подсказок исключительно по городу */
            $location = [];
            if (!empty($address->getCity()['title'])) {
                $location['city'] = $address->getCity()['title'];
            }

            if ($location) {
                $extra_params['locations'] = [$location];
            }

            foreach (DaDataApi::getInstance()->getAddressSuggestion($query, $extra_params ?? []) as $item) {
                if (!empty($item['data']['street_with_type'])) {
                    ...
                    if (!empty($item['data']['settlement_with_type'])) {
                        $label_parts[] = $item['data']['settlement_with_type'];
                    }
    ...
    protected function correctOrderData()
    {
        ...
        if (!$this->order['warehouse']) {
            $this->order['warehouse'] = ($this->order->getStockAffiliateWarehouse())
                ? $this->order->getStockAffiliateWarehouse()['id'] : WareHouseApi::getDefaultWareHouse()['id'];
        }
    }
