...
class OrderShipmentCtrl extends Crud
{
    ...
    function helperIndex()
    {
        ...
        $helper->setTopToolbar(new Toolbar\Element([
            'Item' => new Toolbar\Button\Add($this->router->getAdminUrl('selectShipmentOrder'), t('Добавить отгрузку'), [
                'attr' => [
                    'class' => 'crud-sm-dialog btn-success'
                ]
            ])
        ]));
    ...
    /**
     * Открывает диалог выбора заказа
     */
    public function actionSelectShipmentOrder()
    {
        $helper = new CrudCollection($this);
        $helper->viewAsForm();
        $helper->setTopTitle(t('Выбор заказа для создания отгрузки'));

        $form_object = new FormObject(new PropertyIterator([
            'order_id' => (new SelectOrder())
                ->setChecker('chkEmpty', t('Необходимо выбрать заказ'))
                ->setDescription(t('Номер заказа'))
        ]));

        if ($this->url->isPost()) {
            if ($form_object->checkData()) {
                $url = $this->router->getAdminUrl('shipment', ['order_id' => $form_object['order_id']], 'shop-markingtools');
        ...
        $limit = 14;
        $api = new OrderApi();
        $last_orders = $api->getList(1, $limit);
        ...
        $helper->setForm($this->view->fetch('%shop%/form/shipment/shipment_select_order.tpl'));
        $helper->setBottomToolbar($this->buttons(['save', 'cancel'], [
            'save' => t('Выбрать заказ')
        ]));
