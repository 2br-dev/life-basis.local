...
class OrderCtrl extends Crud
{
    ...
    const SHOW_CHILDS_VAR = 'show_child';
    ...
    function __construct()
    {
        ...
        $is_order_action = $this->getAction() != 'treeEdit';

        if ($order_id && $is_order_action) {
        ...
        //Права доступа
        $this->rights = ModuleRights::getRightsData($this);
        $this->view->assign([
            'rights' => $this->rights
        ]);
    }
    ...
    function helperIndex()
    {
        ...
        $this->show_childs = $this->url->get(self::SHOW_CHILDS_VAR, TYPE_INTEGER, false);
        if ($this->show_childs === false) {
            $this->show_childs = $this->url->cookie(self::SHOW_CHILDS_VAR, TYPE_INTEGER, 0);
        } else {
            $this->app->headers->addCookie(self::SHOW_CHILDS_VAR, $this->show_childs, time() + (60 * 60 * 365 * 10));
        }

        if ($this->status > 0 && $this->getTreeApi()->getOneItem($this->status)) {
            $statuses = ($this->show_childs)
                ? $this->getTreeApi()->getChildsId($this->status)
                : [$this->status];
            $this->api->setFilter('status', $statuses, 'in');
        } else {
            $this->status = 0;
        }
        ...
        $helper
            ...
            ->setTable(new Table\Element([
                'Columns' => [
                    ...
                    new TableType\Usertpl('user_id', t('Покупатель'), '%shop%/admin/order_user_cell.tpl', ['Sortable' => SORTABLE_BOTH, 'href' => $edit_href]),
                    new TableType\Usertpl('id', t('Товары'), '%shop%/admin/order_user_products.tpl', ['hidden' => true]),
                    ...
                    new TableType\Datetime('shipment_date', t('Дата отгрузки'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true, 'parent_block' => 'info']),
                    new TableType\Usertpl('shipment_date', t('Таймер отгрузки'), '%shop%/admin/order_timer_shipment_cell.tpl', ['hidden' => true]),
                    new TableType\Usertpl('totalcost', t('Сумма'), '%shop%/admin/order_totalcost_cell.tpl', ['Sortable' => SORTABLE_BOTH, 'tdAttr' => ['class' => 'text-nowrap']]),
                    ...
                    new TableType\Userfunc('warehouse', t('Склад'), function($value, $_this) {
                        return $_this->getRow()->__warehouse->textView() ?: t('нет');
                    ...
                    new TableType\Text('creator_platform_id', t('Создатель заказа'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true]),
                    new TableType\Text('profit', t('Доходность'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true]),
        ...
        if (Rights::hasRight($this, ModuleRights::RIGHT_EXPORT_REPORT)) {
            $helper['topToolbar']->addItem(new ToolbarButton\Dropdown([
                ...
                [
                    'title' => t('Экспорт заказанных товаров'),
                    'attr' => [
            ...
            'Container' => new Filter\Container([
                'Lines' => [
                    new Filter\Line(['Items' => [
                        ...
                        new Filter\Type\DateRange('shipment_date', t('Дата отгрузки')),
                        new Filter\Type\Select('creator_platform_id', t('Создатель заказа'), OrderApi::getCreatorPlatformsListTitles([
                            '' => t('Любой')
                        ]))
                    ]]),
                ],
                'SecContainer' => new Filter\Seccontainer([
                    'Lines' => [
                        new Filter\Line(['Items' => [
                            ...
                            new Filter\Type\Select('warehouse', t('Склад'), WareHouseApi::staticSelectList(['' => t('Любой')]))
                        ]]),
                        new Filter\Line(['Items' => [
                            new Filter\Type\User('manager_user_id', t('Менеджер')),
                            new Filter\Type\MultipleSelect('status', t('Статус'), UserStatusApi::staticSelectList(), [
                                'attr' => [
                                    'size' => 7
                                ]
                            ]),
        ...
        $this->app->addJs('%shop%/rs.shipmenttimer.js');
        return $helper;
    }
        ...
        $tree = new Tree\Element([
            ...
            'headButtons' => [
                ...
                ($this->show_childs ?
                    [
                        'attr' => [
                            'title' => t('Включено отображение заказов в дочерних статусах. Нажмите, чтобы отключить'),
                            'href' => $this->url->replaceKey([self::SHOW_CHILDS_VAR => 0]),
                            'class' => 'showchilds-on call-update'
                        ]
                    ] : [
                        'attr' => [
                            'title' => t('Отключено отображение заказов в дочерних статусах. Нажмите, чтобы включить'),
     ...
     *
     * @return mixed|Standard|string|null
     * @throws RSException
    ...
    function actionEdit()
    {
        ...
        if ($this->url->isPost()) {
            ...
            $order->excludePostKeys($order->getExcludeKeysByRights($this->rights));
        ...
        $stocks = self::getAllStocks($order, $old_order);

        //Устанавливает ReadOnly для дополнительных полей блоков заказа без соответствующих прав доступа
        foreach ($order->getProperties() as $prop) {
            if ($prop->isVisible('user',false) && !$this->rights['customer_changing']) {
                $prop->setReadOnly(true);
            }
            if ($prop->isVisible('info',false) && !$this->rights['information_changing']) {
                $prop->setReadOnly(true);
            }
            if ($prop->isVisible('address',false) && !$this->rights['address_changing']) {
                $prop->setReadOnly(true);
            }
            if ($prop->isVisible('delivery',false) && !$this->rights['delivery_changing']) {
                $prop->setReadOnly(true);
            }
            if ($prop->isVisible('payment',false) && !$this->rights['pay_changing']) {
                $prop->setReadOnly(true);
            }
        }

        $public_status_api = new UserStatusApi();
        $public_status_api->setFilter('public', 1);
        ...
        $this->view->assign([
            ...
            'elem' => $order,
            'old_order' => $old_order,
            'old_order_stocks' => $stocks['old_order_stocks'],
            'order_available_stocks' => $stocks['order_available_stocks'],
            'user_id' => $order['user_id'],
            'warehouse_list' => $warehouses,
            'courier_list' => $couriers,
            'status_list' => $public_status_api->getTreeList(),
            'default_statuses' => $user_status_api->getStatusIdByType(),
    ...
    /**
     * Возращает остатки по товарам, для редактирования и клонирования заказа
     *
     * @param Order $order - объект заказа
     * @param Order|null $old_order - объект заказа - родителя
     * @return array
     * @throws RSException
     */
    protected static function getAllStocks(Order $order, $old_order = null)
    {
        $old_order_stocks = [];

        if ($old_order) {
            if (!$old_order instanceof Order)
            {
                $old_order = new Order($old_order);
            }
        ...
        $stocks['old_order_stocks'] = $old_order_stocks;
        $stocks['order_available_stocks'] = $order_available_stocks;
        return $stocks;
    }
     ...
     * @throws SmartyException|RSException
     */
    public function actionAdd($primaryKeyValue = null, $returnOnSuccess = false, $helper = null)
    {
        ...
        $stocks = self::getAllStocks($order, $from_order);
    ...
    function actionUserDialog()
    {
        ...
        //Если нет прав доступа на изменение
        if ($right_error = Rights::CheckRightError($this, ModuleRights::RIGHT_CUSTOMER_CHANGING)) {
            $this->api->addError($right_error);
    ...
    {
        //Если нет прав доступа на изменение
        if ($right_error = Rights::CheckRightError($this, ModuleRights::RIGHT_ADDRESS_CHANGING)) {
            $this->api->addError($right_error);
            return $this->result->setSuccess(false)
                ->setErrors($this->api->getDisplayErrors());
        }
        ...
        if ($this->url->isPost()) { //Если пришёл запрос
            $use_addr = $this->url->post('use_addr', TYPE_INTEGER); //Использовать адрес пользователя
            //Получим данные
            if ($error = Rights::CheckRightError($this, DefaultModuleRights::RIGHT_UPDATE)) {
                $this->api->addError($error);
                return $this->result->setSuccess(false)->setErrors($this->api->getDisplayErrors());
        ...
        if ($address_list || $order['use_addr']) {
            $bottom_toolbar_items['update_address'] = new ToolbarButton\SaveForm(null, t('сохранить и выбрать'), [
                'attr' => [
                    'data-url' => $this->router->getAdminUrl(null, $this->url->getSource(GET) + ['edit_type' => 'edit']),
                    'data-address-save' => true
                ],
            ]);
        }
        if ($user_id || !$order['use_addr']) {
            $bottom_toolbar_items['new_address'] = new ToolbarButton\SaveForm(null, t('создать новый адрес'), [
                'attr' => [
    ...
    {
        //Если нет прав доступа на изменение
        if ($right_error = Rights::CheckRightError($this, ModuleRights::RIGHT_DELIVERY_CHANGING)) {
            $this->api->addError($right_error);
            return $this->result->setSuccess(false)
                ->setErrors($this->api->getDisplayErrors());
        }
    ...
    {
        //Если нет прав доступа на изменение
        if ($right_error = Rights::CheckRightError($this, ModuleRights::RIGHT_PAY_CHANGING)) {
            $this->api->addError($right_error);
            return $this->result->setSuccess(false)
                ->setErrors($this->api->getDisplayErrors());
        }
    ...
    function actionGetOfferPrice()
    {
        ...
        foreach ($cost_list as $type_cost) {
            ...
            if ($shop_config['old_cost_delta_as_discount']) {
                $old_cost_id = $cost_list[$type_cost['old_cost']]['id'] ?? $catalog_config['old_cost'];
                if ($old_cost_id) {
    ...
    function actionDeleteAddress()
    {
        ...
        $address_id = $this->request('address_id', TYPE_INTEGER, 0);
        $address = new Address($address_id);
        if ($address['id']) {
            if ($address->delete()) {
                return $this->result->setSuccess(true);
            } else {
                return $this->result->setSuccess(false)->addEMessage($address->getErrorsStr());
            }
        } else {
            return $this->result->setSuccess(false)->addEMessage(t('Такого адреса не существует'));
        }
    }
    ...
    public function actionArchiveOrders()
    {
        ...
        $tmp_storage = new PersistentStateFile(\Setup::$TMP_DIR.'/archive_orders_ids.tmp');

        if ($offset) {
            $ids = $tmp_storage->get('ids', []);
        } else {
            $ids = $this->modifySelectAll($this->url->request('chk', TYPE_ARRAY, []));
            $tmp_storage->clean();
            $tmp_storage->set('ids', $ids);
        }
