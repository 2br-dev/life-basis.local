...
class Tools extends \RS\Controller\Admin\Front
{
    ...
    public function actionCdekUnsubscribeWebHooks()
    {
        try {
            ...
            //Если uuid не найден, значит веб-хук уже отключен
            if (str_contains($e->getMessage(), 'Entity is not found by uuid')) {
                $delivery = CdekApi::getDefaultCdekDelivery();

                /** @var Cdek2 $delivery_type */
                $delivery_type = $delivery->getTypeObject();
                $delivery_type->api->webHooksUnsubscribe(true);

                return $this->result->setSuccess(false)->addEMessage(t('Сайт уже отписан от получения веб-хуков'));
            }
    ...
    /**
     * Устанавливает пометку о том, что адрес удален для всех пользователей,
     * чтобы при следующем оформлении заказа пользователи не могли выбрать старый адрес.
     * Нужно при смене справочника населенных пунктов
     *
     * @return \RS\Controller\Result\Standard
     */
    function cleanUsersAddress()
    {
        $result = Request::make()
            ->update(new Address())
            ->set([
        ...
        return $this->result
            ->setSuccess(true)
            ->addMessage(t('Удалено %0 сохраненных адресов пользователей', [$result->affectedRows()]));
    }
}
