...
abstract class AbstractType
{
    ...
    private $cache_delivery_cost = []; //Кэш расчета стоимости
     ...
     */
    public function addToOrderAdminComment($order, $text)
    {
     ...
     * @param Order $order - объект заказа
     */
    public function removeFromOrderDeliveryErrorInfoLine($order)
    {
     ...
     * Результат возвращается из кэша при повторном вызове.
     * Для очистки кэша можно использовать ->cleanDeliveryCostCache()->getDeliveryFinalCost()
     *
     * @param Order $order - объект заказа
     * @param Address $address - Адрес доставки
    ...
    public function getDeliveryFinalCost(Order $order, ?Address $address = null, $use_currency = true)
    {
        ...
        $cache_key = md5(serialize($address->getValues())).'-'
                     .md5(serialize($order->getAffectingCostValues()));

        if (!isset($this->cache_delivery_cost[$cache_key])) {

            $cost = $this->getDeliveryCost($order, $address, $this->getDelivery(), false);
            $cost = $this->applyExtraChangeDiscount($this->getDelivery(), $cost, $order);

            $this->cache_delivery_cost[$cache_key] = $cost;
        }

        if ($use_currency) {
            $cost = $order->applyMyCurrency($this->cache_delivery_cost[$cache_key]);
        } else {
            $cost = $this->cache_delivery_cost[$cache_key];
        }
     ...
     * @return $this
     */
    public function cleanDeliveryCostCache()
    {
        $this->cache_delivery_cost = [];
        return $this;
    }
    ...
    protected function calcDeliveryPeriod(Order $order, ?Address $address = null, ?Delivery $delivery = null)
    {
        ...
        //Здесь у потомков нужно либо реализовать собственный расчет сроков доставки,
    ...
    public function getDeliveryPeriod(Order $order, ?Address $address = null, ?Delivery $delivery = null)
    {
        ...
        if ($period) {
            $delivery_delay_days = $this->getDeliveryDelayForCart($order);
            $period->setDeliveryDelay($delivery_delay_days);
            //var_dump($period);die;
        }
    ...
    /**
     * Возвращает количество дней, на которое нужно отодвинуть срок доставки.
     * Обычно это срок поставки товаров на склад.
     *
     * @param $order
     * @return int
     */
    public function  getDeliveryDelayForCart($order)
    {
        //Максимальный срок поставки корзины товара.
        //Равен максимальному сроку поставки в корзине
        $delay = 0;
        $warehouse_groups = WareHouseGroupApi::getInstance()->getAssocList('id');

        if ($warehouse_groups) {
            $product_data = $order->getCart()->getProductItems();
            foreach ($product_data as $item) {
                $product_delay = 0;
                $cartitem = $item[Cart::CART_ITEM_KEY];
                $product = $item[Cart::TYPE_PRODUCT];

                $amount = $cartitem['amount']; //Здесь сколько заказали товара

                $product_num_by_warehouse_groups = $product->getWarehouseGroupStocks($cartitem['offer']);
                foreach ($warehouse_groups as $group_id => $warehouse_group) {
                    if (isset($product_num_by_warehouse_groups[$group_id])) {
                        $product_delay = $warehouse_group['delivery_delay'];
                        if ($product_num_by_warehouse_groups[$group_id]['stock'] >= $amount) {
                            break;
                        } else {
                            $amount -= $product_num_by_warehouse_groups[$group_id]['stock'];
                        }
                    }
                }
    ...
    public function applyExtraChangeDiscount($delivery, $cost, ?Order $order = null)
    {
        ...
        if ($this->isDeliveryFree($delivery, $cost, $order)) {
            $cost = 0;
        } else {
     ...
     * @return bool
     */
    protected function isDeliveryFree($delivery, $cost, $order)
    {
        if ($cost == 0) return true;

        if ($order && ($cart = $order->getCart())) {
            if ($delivery['free_price'] && $cart->getTotalWithoutDelivery(false) >= $delivery['free_price']) {
                return true;
            }

            foreach ($cart->getCouponItems() as $coupon) {
                if (in_array($delivery['id'], (array)$coupon[Cart::TYPE_COUPON]['free_deliveries'])) {
                    return true;
                }
            }
        }
     ...
     * @return string
     */
    public function getClientSiteAppAdditionalHTML(Order $order, Delivery $delivery)
    {
        $html = '';
        ...
        if ($this->hasPvz()) {
            ...
            $html .= trim($view->fetch('%shop%/delivery/clientsiteapp/additional_html.tpl'));
        }
