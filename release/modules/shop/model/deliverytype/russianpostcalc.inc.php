...
class RussianPostCalc extends AbstractType
{
    ...
    const CALCULATE_URL =  'https://tariff.pochta.ru';
    const DELIVERY_PERIOD_URL = 'https://tariff.pochta.ru';
    ...
    */
    ...
    public function getSelectError(Order $order): string
    {
        ...
        $zipcode = $address['zipcode'] ?: $address->getCity()['zipcode'];
        if (!$zipcode) {
            return t('Нет индекса у города получателя');
        }
        return '';
    }
    ...
    protected function calcDeliveryPeriod(Order $order, Address $address = null, Delivery $delivery = null)
    {
        ...
        $weight = (empty($order['true_weight'])) ? $order->getWeight(\Catalog\Model\Api::WEIGHT_UNIT_G) : $order['true_weight'];
        ...
        if ($params['from'] && $params['to']) {
            $res = $this->apiRequest('get', self::DELIVERY_PERIOD_URL . '/v2/calculate/tariff/delivery', $params);
    ...
    function calculateCost($params)
    {
        ...
        $res = $this->apiRequest('get', self::CALCULATE_URL . '/v2/calculate/tariff', $request_params);
        return $res['response'];
    }
    /**
     * API запрос
