    ...
    private function applyRule(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address, $rule, $last_delivery_cost, &$is_matched)
    {
        ...
        if (isset($rule['zone'])){
            if($rule['zone'] != 'all') {
                ...
                if(!$address->region_id && !$address->country_id && !$address->city_id) {
                    return 0;
                }
        ...
        $order_price_without_delivery = $order->getCart()->getTotalWithoutDelivery(false);
        $examining_value = 0;
        ...
        if((!$rule['from'] || $examining_value >= $rule['from'])
            && (!$rule['to'] || $examining_value <= $rule['to'])) {
            $is_matched = true;
