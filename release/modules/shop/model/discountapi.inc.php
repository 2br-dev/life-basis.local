    ...
    public static function applyCouponPercentDiscountsToCart(Cart $cart)
    {
        ...
        foreach ($coupon_items as $key => $coupon_item) {
            ...
            if ($coupon['discount_type'] == Discount::DISCOUNT_TYPE_PERCENT) {
                ...
                //Проверим наличие обязательных товаров в корзине
                $need_product_has_error = $coupon['need_products']
                    && (!$coupon->getNeedProductsUniq($in_basket, $sum) || $sum < $coupon['need_products_sum']);

                if (($min_order_price > $cart_total || $need_product_has_error
                        || $date_expire || !$coupon['active']) && $cart->getMode() == CART::MODE_SESSION) {
                    $cart->removeItem($key);
                } else {
                    $coupon_applied = false;

                    $linked_products = $coupon->getLinkedProductsUniq($in_basket);
                    foreach($linked_products as $uniq) {
        ...
        foreach ($coupon_items as $key => $coupon_item) {
            ...
            if ($coupon['discount_type'] == Discount::DISCOUNT_TYPE_BASE_CURRENCY) {
                ...
                if (($min_order_price > $cart_total || $need_product_has_error
                    ...
                    $linked_remaining_discount_limit = $coupon->getLinkedProductsUniq($in_basket);

                    //Добавляем информацию о максимально возможной скидке, которую можно применить
                    foreach($linked_remaining_discount_limit as $uniq => $uniq) {
