...
use RS\Helper\Tools;
use RS\Orm\Request;
use Shop\Model\Notice\CheckoutUser;
    ...
    */
    public static function SendNoticeReceipt($site_id)
    {
        ...
        $reservations = \RS\Orm\Request::make()
            ...
            ->join(new Offer, 'R.offer_id = O.id', 'O')
    ...
    function createOrderFromReservation(Orm\Reservation $reservation)
    {
        ...
        $offer = new Offer($reservation['offer_id']);
        
        //Попробуем загрузить сам товар
        $product     = new Product($reservation['product_id']);
        $symb = array_merge(range('a', 'z'), range('0', '9')); //Символя для генерации уникального индекса
        $uniq = Tools::generatePassword(10, $symb); //Уникальный индекс товара
        $products_arr[$uniq] = [
              'uniq'          => $uniq,
              ...
              'offer'         => $offer['id'],
              'multioffers'   => $reservation['multioffers'],
        ...
        if (\RS\Module\Manager::staticModuleExists('statistic')
            && \RS\Module\Manager::staticModuleEnabled('statistic')
            && $reservation['source_id'])
        {
            $order['source_id'] = $reservation['source_id'];
        }
        $reservation->update();//изменяем статус предварительного заказа
