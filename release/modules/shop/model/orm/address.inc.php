...
class Address extends OrmObject
{
    ...
    function _init()
    {
        ...
        $this->addIndex(['site_id', 'user_id', 'deleted', 'city_id'], self::INDEX_KEY);
    }
    ...
    {
        $this['extra'] = @json_decode((string)$this['_extra'], true) ?? [];
    }
    ...
    function updateAddressTitles()
    {
        ...
        // Откорректируем название страны
        $country = $this['country_id'] ? $regionApi->getOneItem($this['country_id']) : null;
        $this['country'] = $country ? $country['title'] : null;

        // Откорректируем область
        $region = $regionApi->getOneItem($this['region_id']);
        if ($this['region_id'] && $region && $country && $region['parent_id'] == $country['id']) {
            $this['region'] = $region['title'];
        } else {
            $this['region_id'] = 0;
        }

        //Заполним название города
        if ($this['city'] == '' && $this['city_id']) {
            $city = $regionApi->getOneItem($this['city_id']);
