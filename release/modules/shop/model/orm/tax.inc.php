 ...
 * @property integer $nds_settlement_rate Использовать расчетную ставку N / (100 + N)
 * @property integer $nds_shipment_settlement_rate Использовать расчетную ставку для отгрузки N / (100 + N)
 * @property integer $sortn Порядок применения
 * @property array $rates 
 * @property array $region_id_list Регионы для отображения в древовидном списке
 * --\--
 */
class Tax extends OrmObject
{
    ...
    function _init()
    {
        parent::_init()->append([
            t('Основные'),
                ...
                'nds_settlement_rate' => new Type\Integer([
                    'description' => t('Использовать расчетную ставку N / (100 + N)'),
                    'hint' => t('Используется в чеках, например, если для НДС 20% установить данный флажок, то в чек будет передана ставка 20/120'),
                    'checkboxView' => [1,0],
                ]),
                'nds_shipment_settlement_rate' => new Type\Integer([
                    'description' => t('Использовать расчетную ставку для отгрузки N / (100 + N)'),
                    'hint' => t('Используется в чеках отгрузки(Полный расчет), например, если для НДС 20% установить данный флажок, то в чек будет передана ставка 20/120'),
                    'checkboxView' => [1,0],
                ]),
            ...
            t('Ставки налога'),
                ...
                'region_id_list' => new Type\ArrayList([
                    'description' => t('Регионы для отображения в древовидном списке'),
    ...
    */
    ...
    /**
     * Возвращает полное название региона (Страна > Область > Город) по id региона
     *
     * @param $region_id
     * @return string
     */
    function getFullTitleRegionById($region_id) {
        $region = new Region($region_id);
        $full_title = $region->title;
        while ($region->parent_id > 0 && ($parent_region = $region->getParent()) && $region->getParent()->id) {
            $full_title = $parent_region->title . ' > ' . $full_title;
