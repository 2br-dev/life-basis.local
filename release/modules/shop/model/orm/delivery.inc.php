...
class Delivery extends OrmObject
{
    ...
    public function beforeWrite($flag)
    {
        ...
        $type_params = $this->getTypeObject()->getFormObject();
        if ($type_params) {
            $data = $this['data'];
            foreach($type_params->getPropertyIterator() as $key => $property) {
                if (!isset($data[$key]) && $property->getCheckboxParam()) {
                    $data[$key] = $property->getCheckboxParam('off');
                }
            }
            $this['data'] = $data;
        }
    ...
    public function afterObjectLoad()
    {
        ...
        $this['delivery_periods'] = @unserialize((string)$this['_delivery_periods']);
        $this['tax_ids'] = @unserialize((string)$this['_tax_ids']);
    }
    ...
    public function getDeliveryExtraText(Order $order, Address $address = null)
    {
        ...
        if (empty($text) && !empty($this['delivery_periods'])) {
            ...
            foreach ($this['delivery_periods'] as $delivery_period) {
                if (($delivery_period['zone'] == 0) || in_array($delivery_period['zone'], $zones)) { //Если зона все
                    $period->setDayMin($delivery_period['days_min']);
                    $period->setDayMax($delivery_period['days_max']);
                    $period->setPeriodAsText($delivery_period['text']);
                }

                $text = $period->getPeriodAsText();
