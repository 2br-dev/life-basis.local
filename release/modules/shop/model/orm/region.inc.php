...
class Region extends OrmObject
{
    ...
    */
    ...
    /**
     * Возвращает исключительно название региона, без его типа
     *
     * @return string
     */
    function getCleanTitle()
    {
        $list = ['автономный округ', 'автономная область', 'республика', 'округ', 'сельское поселение', 'село',
            'аул', 'поселок', 'станица', 'дачный поселок', 'поселок городского типа', 'деревня',
            'область', 'край', 'город', 'г.', 'обл.', 'респ.'];

        $before = array_map(function($value) { return $value.' '; }, $list);
        $after = array_map(function($value) { return ' '.$value; }, $list);

        $title = str_ireplace($before, '', $this['title'] ?? '');
        $title = str_ireplace($after, '', $title);
        return trim($title);
    }
}
