...
class Order extends OrmObject
{
    ...
    function _init()
    {
        $properties = parent::_init()
            ...
            //Подтверждение
                ...
                'shipment_date' => new Type\Datetime([
                    'description' => t('Дата отгрузки'),
                    'meVisible' => false,
                ]),
                'creator_platform_id' => new Type\Varchar([
                    'maxLength' => t('100'),
                    'description' => t('Платформа, на которой был создан заказ'),
                    'list' => [['\Shop\Model\OrderApi', 'getCreatorPlatformsListTitles']],
                    'allowEmpty' => false,
                    'index' => true,
                    'meVisible' => false,
                ]),
                ...
                'is_exported' => new Type\Integer([
                    'maxLength' => '1',
                    'default' => 0,
                    'description' => t('Выгружен ли заказ в 1С'),
                    'hint' => t('Если сбросить данный флаг, то заказ автоматически попадет в следующую выгрузку в 1С'),
                    'checkboxView' => [1, 0],
                    'visible' => false,
                    'meVisible' => true,
                ]),
        ...
        $this->addIndex(['site_id', 'status'],self::INDEX_KEY);
        ...
        //Шаг 2.2 - только при самовывозе
            'warehouse' => new Type\Integer([
                ...
                'Checker' => ['chkEmpty', t('Не настроен склад отгрузки')],
    ...
    function beforeWrite($flag)
    {
        ...
        $config = ConfigLoader::byModule($this, $this['__site_id']->get());

        //Проверка уникального идентификатора номера заказа
        if (empty($this['order_num'])
            && $config['order_num_generate_type'] == ShopConfig::ORDER_NUM_GENERATE_TYPE_RANDOM) {
            try {
                $api = new OrderApi();
        ...
        if ($flag == self::INSERT_FLAG) {
            ...
            if (empty($this['dateof'])) {
                $this['dateof'] = date('Y-m-d H:i:s');
            }
            if (empty($this['creator_platform_id'])) {
                $this['creator_platform_id'] = self::CREATOR_PLATFORM_SITE;
            }
    ...
    function afterWrite($flag)
    {
        ...
        if ($flag == self::INSERT_FLAG) { //При вставке
            ...
            if (empty($this['order_num'])
                ...
                $this->afterGenerateOrderNum($config['order_num_generate_type']);
            }
        }
        ...
        if ($flag == self::UPDATE_FLAG) {
            ...
            //Посмотрим, если появился трек номер, то отправим его по SMS
        ...
        if (!$address['user_id'] && !$address['order_id']) {
            //Если это заказ без регистрации, то привязываем адрес к заказу
            $address['order_id'] = $this['id'];
            $address->update();
        }
        ...
        $catalog_config = ConfigLoader::byModule('catalog', $this['__site_id']->get());

        if (($catalog_config['inventory_control_enable'] || $config['check_quantity'])
            && !$this->getSpecialParam(self::SPECIAL_DISABLE_UPDATE_PRODUCTS_STOCK)) {
    ...
    {
        $config = ConfigLoader::byModule('catalog', $this['__site_id']->get());
    ...
    */
    ...
    {
        $shop_config = ConfigLoader::byModule('shop', $this['__site_id']->get());
        /** @var UsersConfig $users_config */
        $users_config = ConfigLoader::byModule('users', $this['__site_id']->get());

        $result = User::checkFioField($this->getUser(), $this['user_fio']);
        if ($result !== true) {
            $this->addError($result, 'user_fio');
        }
    ...
    */
    function getFieldsManager()
    {
        if (!$this->order_fields_manager) {
            $this->order_fields_manager = ConfigLoader::byModule($this, $this['__site_id']->get())->getUserFieldsManager();
    ...
    */
    ...
    function getAddress($cache = true)
    {
        if ($this->address === null || !$cache) {
            if ($this['use_addr']) {
                $this->address = new Address($this['use_addr'], false);
            } else {
     ...
     * @return Delivery
     */
    function getDelivery($cache = true)
    {
        if (!$cache || $this->cache_delivery === null || $this->cache_delivery['id'] != $this['delivery']) {
            $this->cache_delivery = new Delivery($this['delivery']);
        }
     ...
     * @return Payment
     */
    function getPayment($cache = true)
    {
        if (!$cache || $this->cache_payment === null || $this->cache_payment['id'] != $this['payment']) {
            $this->cache_payment = new Payment($this['payment'], true, $this);
        }
    ...
    function getUser()
    {
        ...
        preg_match('/^([^\s]+)\s*([^\s]+)?\s*(.+)?$/u', trim((string)$this['user_fio']), $match);
    ...
    function getCart()
    {
        if ($this['id']) {
            ...
            if ($this->session_cart === null) {
                $this->session_cart = @unserialize((string)$this['basket']);
            }
            if ($this->session_cart){
    ...
    */
    ...
    /**
     * Возвращает последнюю транзакцию заказа
     *
    ...
    public function getOrderTransaction()
    {
        $transactrions_api = new TransactionApi();
        $transactrions_api->setFilter('order_id', $this['id']);
        $transactrions_api->setFilter('entity', null);
        $transactrions_api->setOrder('dateof DESC');

        /** @var Transaction $transactrion */
        $transactrion = $transactrions_api->getFirst();
    ...
    */
     ...
     * @param array $extra_params - Дополнительные параметры, которые необходимо добавить к URL
     * @return string
     */
    function getOnlinePayUrl($absolute = false, $extra_params = [])
    {
    ...
    */
    ...
    {
        return new User($this['manager_user_id']);
    }
    ...
    public function setUserId(int $user_id): void
    {
        ...
        if ($this['user_id']) {
            ...
            if (ConfigLoader::byModule('shop', $this['__site_id']->get())['check_captcha'] && !$reg_phone_field->isEnabledVerification()) {
    ...
    /**
     * Возвращает массив значений текущего заказа, влияющих на стоимость данного заказа
     * Из этих значений можно высчитывать хэш заказа, имеющего уникальную стоимость.
     *
     * @return array
     */
    function getAffectingCostValues()
    {
        $cart = $this->getCart();
        $cart_items = $cart ? $cart->getItems() : [];
        $data = [
            'payment' => $this['payment'],
            'delivery' => $this['delivery'],
            'address' => $this->getAddress()->getLineView(),
            'user_id' => $this['user_id'],
            'cart' => md5(serialize($cart_items))
        ];

        $event_result = EventManager::fire('order.getAffectingCostValues', [
            'data' => $data,
            'order' => $this
        ]);

        return $event_result->getResult()['data'];
    }

    /**
     * Проверка изменений и соответствующих прав при изменении в блоках заказа
     *
     * @return bool
     * @throws RSException
     */
    private function checkRightsChanges()
    {
        $allow = true;
        $old_order = $this->this_before_write;

        //Проверка изменения Статуса заказа
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_STATUS_CHANGING)) {
            if ($this['status'] != $old_order['status']) {
                $this->addError($rights_error, t('Статус заказа'));
                $allow = false;
            }
        }

        //Проверка изменения Покупателя
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_CUSTOMER_CHANGING)) {
            if ($this['user_id'] != $old_order['user_id']) {
                $this->addError($rights_error, t('Покупатель'));
                $allow = false;
            }
        }

        //Проверка изменения Информации о заказе
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_INFORMATION_CHANGING)) {
        $changed = $this['shipment_date'] != $old_order['shipment_date']
            || $this['comments'] != $old_order['comments']
            || $this['manager_user_id'] != $old_order['manager_user_id'];
            if ($changed) {
                $this->addError($rights_error, t('Информация о заказе'));
                $allow = false;
            }
        }

        //Проверка изменения Адреса
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_ADDRESS_CHANGING)) {
            if ($this['contact_person'] != $old_order['contact_person']) {
                $this->addError($rights_error, t('Адрес'));
                $allow = false;
            }
        }

        //Проверка изменения Доставки
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_DELIVERY_CHANGING)) {
        $changed = $this['warehouse'] != $old_order['warehouse']
            || $this['track_number'] != $old_order['track_number']
            || $this['courier_id'] != $old_order['courier_id'];
            if ($changed) {
                $this->addError($rights_error, t('Доставка'));
                $allow = false;
            }
        }

        //Проверка изменения Оплаты
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_PAY_CHANGING)) {
            if ($this['is_payed'] != $old_order['is_payed']) {
                $this->addError($rights_error, t('Оплата'));
                $allow = false;
            }
        }

        $old_cart = $old_order->order_cart->getCartItemsByType('product');
        $now_cart = $this->order_cart->getCartItemsByType('product');

        //Проверка изменения элементов Состава заказа
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_PRODUCTS_CHANGING)) {
            foreach ($old_cart as $index => $old) {
                if ($now = $now_cart[$index]) {
                    if (
                        $old['entity_id'] != $now['entity_id']
                        || $old['amount'] != $now['amount']
                        || $old['model'] != $now['model']
                        || $old['price'] != $now['price']
                        || $old['offer'] != $now['offer']
                        || $old['single_cost'] != $now['single_cost']
                    ) {
                        $this->addError($rights_error, t('Состав заказа: Изменение'));
                        $allow = false;
                        break;
                    }
                }
        ...
        //Проверка добавление элементов Состава заказа
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_PRODUCTS_ADD)) {
            $added = array_diff_key($now_cart, $old_cart);
            if ($added) {
                $this->addError($rights_error, t('Состав заказа: Добавление'));
                $allow = false;
            }
        }

        //Проверка удаления элементов Состава заказа
        if ($rights_error = Rights::CheckRightError($this, ModuleRights::RIGHT_PRODUCTS_DELETE)) {
            $deleted = array_diff_key($old_cart, $now_cart);
            if ($deleted) {
                $this->addError($rights_error, t('Состав заказа: Добавление'));
                $allow = false;
            }
    ...
    /**
     * Возвращает наиболее подходящий на текущий момент
     * ID типа цены заказа в сравнении с тем, что был при оформлении заказа
     *
     * Это либо тип цен пользователя, который оформлял заказ,
     * либо Тип цен филиала, с которым был оформлен заказ
     * либо цена по умолчанию
     *
     * @return integer
     */
    public function getOrderCostId()
    {
        $cost_id = $this->getUser()->getUserTypeCostId();

        if (!$cost_id
            && ModuleManager::staticModuleEnabled('affiliate')
            && ($data = $this->getExtraInfoLine('affiliate'))
            && isset($data['data']['id'])) {

            $affiliate = new Affiliate($data['data']['id']);
            if ($affiliate['id'] && $affiliate['cost_id']) {
                $cost_id = $affiliate['cost_id'];
            }
        }

        if (!$cost_id) {
            $cost_id = CostApi::getDefaultCostId();
        }
    ...
    /**
     * Возвращает список ключей, которые следует игнорировать при сохранении заказа
     * с учетом текущих настроек прав пользователя
     *
     * @return array
     */
    public function getExcludeKeysByRights(array $rights)
    {
        $keys = [];
        $exclude_rules = [
            ModuleRights::RIGHT_CUSTOMER_EMAIL_READING => 'user_email',
            ModuleRights::RIGHT_CUSTOMER_PHONE_READING => 'user_phone',
            ModuleRights::RIGHT_CUSTOMER_FULLNAME_READING => 'user_fio',
            ModuleRights::RIGHT_IS_PAY_READING => 'is_payed',
            ModuleRights::RIGHT_USER_COMMENT_READING => 'comments',
            ModuleRights::RIGHT_SHIPMENT_DATE_READING => 'shipment_date',
            ModuleRights::RIGHT_MANAGER_READING => 'manager_id',
            ModuleRights::RIGHT_COURIER_READING => 'courier_id',
            ModuleRights::RIGHT_CONTACT_PERSON_READING => 'contact_person',
            ModuleRights::RIGHT_WAREHOUSE_READING => 'warehouse',
            ModuleRights::RIGHT_USERFIELDS_READING => 'userfields_arr',
        ];

        foreach($exclude_rules as $right => $key) {
            if (!$rights[$right]) {
                $keys[] = $key;
            }
        }
    ...
    /**
     * Автоматически генерирует инкрементный номер заказа
     *
     * @param string $type Тип генерации номера заказа
     * @param integer $try Попытка
     * @return void
     */
    private function afterGenerateOrderNum($type, $try = 0)
    {
        if ($type == ShopConfig::ORDER_NUM_GENERATE_TYPE_INCREMENT) {
            $now_config = new ShopConfig();
            $now_config->load($this['__site_id']->get());
            $order_num = $now_config['generated_ordernum_start_number'] ?: 1;
            $now_config['generated_ordernum_start_number'] = $order_num + 1;
            $now_config->update();

            $order_num = str_pad((string)$order_num,
                (int)$now_config['generated_ordernum_numbers'],
                "0",
                STR_PAD_LEFT);
            $order_num = str_replace("{n}", $order_num, $now_config['generated_ordernum_mask']);
        } else {
            $order_num = $this['id'];
        }
        ...
        if (isset($order_num)) {
            try {
                ...
                if ($try < 5) {
                    $this->afterGenerateOrderNum($type, $try + 1);
                }
            }
        }
    }
