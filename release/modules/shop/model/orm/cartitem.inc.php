 ...
 * @property integer $cart_id Идентификатор корзины
 * @property integer $enabled Включен для заказа
    ...
    public function _init()
    {
        ...
        $this->getPropertyIterator()->append([
            ...
            'cart_id' => new Type\Integer([
                ...
                'hint' => t('Может применяться во внешних модулях, реализующих мультикорзины. 0 - корзина по умолчанию.'),
                'allowEmpty' => false
            ]),
            'enabled' => new Type\Integer([
                'description' => t('Включен для заказа'),
                'hint' => t('Включенный товар доступен для оформления, выключенный - будет оставаться в корзине без дальнейшего оформления.'),
                'default' => 1,
                'allowEmpty' => false
            ]),
        ...
        $this->addIndex(['site_id', 'user_id', 'cart_id'], self::INDEX_KEY);
        $this->addIndex(['site_id', 'session_id', 'cart_id'], self::INDEX_KEY);
    }
