 ...
 * @property string $shipment_warehouse_name Склад отгрузки товара
 * @property string $shipment_warehouse_id ID склада отгрузки
 * --\--
 */
class OrderItem extends AbstractCartItem
{
    ...
    function _init()
    {
        ...
        $this->addIndex(['type', 'entity_id']);
        $this->addIndex(['order_id', 'barcode']);
    }
    ...
    {
        if ($this['unit_id']) {
            $unit = Unit::loadSingle($this['unit_id']);
        } else {
    ...
    function getProfit()
    {
        ...
        if ($this['type'] == self::TYPE_DELIVERY && $config['include_delivery_to_profit']) {
