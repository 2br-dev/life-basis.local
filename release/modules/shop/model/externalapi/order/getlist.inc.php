...
class GetList extends AbstractGetList
{
    ...
    */
    ...
    protected function makeFilterFullSearch($key, $value, $filters, $filter_settings)
    {
        ...
        $q = $dao->queryObj();
        $q->select('A.*');
        if (!$q->issetTable(new OrderItem())) {
            $q->leftjoin(new OrderItem(), 'A.id=PRODUCT.order_id', 'PRODUCT');
    ...
    */
    public function setFilter($dao, $filter)
    {
        ...
        }elseif ($this->token->getApp()->getId() == 'store-management') {
            //Для админского приложения отдаем все заказы
        }else {
    ...
    /**
     * Добавляет к заказу секцию со способами оплаты
     *
     * @param Order $order
     * @return void
     * @throws \Exception
     */
    private function addOrderPayment(Order $order)
    {
        $payment = null;
        if ($payment_id = $order['payment']) {
        ...
        $order['payment'] = $payment ? Utils::extractOrm($payment) :  null;
    }
    ...
    public function getResultList($dao, $page, $pageSize)
    {
        ...
        $dao->getElement()->getPropertyIterator()->append([
            ...
            'dateof_date' => new Type\MixedType([
                'description' => t('Дата создания в dd.mm.YYYY'),
                'appVisible' => true
            ]),
            'dateof_datetime' => new Type\MixedType([
                'description' => t('Дата создания в dd.mm.YYYY HH:ii'),
                'appVisible' => true
            ]),
            'can_online_pay' => new Type\Integer([
                'description' => t('Возможна ли оплата online'),
                'appVisible' => true
            ]),
            'total' => new Type\Varchar([
                'description' => t('Общая отформатированная цена'),
                'appVisible' => true
            ]),
            'totalcost_formatted' => new Type\Varchar([
                'description' => t('Общая отформатированная цена'),
                'appVisible' => true
            ]),
            'pay_url' => new Type\Varchar([
                'description' => t('Ссылка на оплату'),
                'appVisible' => true
            ]),
            ...
            'payment' => new Type\MixedType([
                'description' => t('Способ оплаты'),
                'appVisible' => true
            ]),
            'preview_data' => new Type\MixedType([
                'description' => t('Выборка данных, отображаемых в списке'),
                'appVisible' => true
            ])
        ]);
        ...
        foreach($this->list_orders as $order) {
            ...
            $order['total']           = \RS\Helper\CustomView::cost($order['totalcost'])." ".$order['currency_stitle'];
            $order['totalcost_formatted'] = \RS\Helper\CustomView::cost($order['totalcost'])." ".$order['currency_stitle'];
            $order['pay_url']           = '';
            if ($order['can_online_pay']) {
                $order['pay_url'] = $order->getOnlinePayUrl(true, ['rs_mobile_app' => 1]);
            }

            ApiUtils::addOrderItems($order);
            $this->addOrderPayment($order);
            $this->addOrderPreviewData($order);
        }
        
        return Utils::extractOrmList( $this->list_orders );
    }
     ...
     * @return void
     */
    protected function addOrderPreviewData(Order $order)
    {
        ...
        if ($user['is_company']) {
            if ($user['company']) {
                $preview_data->addTextRow(t('Компания'), Tools::unEntityString($user['company']));
            }
            if ($user['company_inn']) {
                $preview_data->addTextRow(t('ИНН'), $user['company_inn']);
            }
        }

        if ($order['use_addr']) {
            $address = $order->getAddress();
            $address_short = trim(implode(', ', [$address['city'], $address->getLineView(false)]),', ');
            $preview_data->addTextRow(t('Адрес'), $address_short);
        }

        $order['preview_data'] = $preview_data->getPreviewData();
    }
    ...
    * @param string $fulltext_filter Полнотекстовый фильтр (используется для совместимости со старыми приложениями, в будущем будет удалено. Необходимо использовать вместо него filter[query])
    * 
    ...
    */
    ...
    {
        if ($fulltext_filter != '') {
            //Для совместимости с предыдущим приложением ReadyScript Mobile Manager App
            $filter['query'] = $fulltext_filter;
        }
        ...
        if (in_array('user', $sections)) {
            ...
            $response['response']['user'] = Utils::extractOrmList($users, 'id');
        }
        ...
        if (in_array('status', $sections)) {
            ...
            $response['response']['status'] = Utils::extractOrmList($statuses, 'id');
        }        
        ...
        if (in_array('address', $sections)) {
            ...
            $response['response']['address'] = Utils::extractOrmList($addresses, 'id');
        }        
