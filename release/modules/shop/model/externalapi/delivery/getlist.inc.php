...
class GetList extends \ExternalApi\Model\AbstractMethods\AbstractGetList
{
    ...
    */
    ...
    {
        return [
            self::RIGHT_LOAD => t('Доступ к публичным доставкам или доставкам текущего пользователя'),
            self::RIGHT_FULL_ACCESS => t('Доступ ко всем доставкам')
        ];
    }
    ...
    */
    private function getDeliveryListByCurrentOrder($sortn)
    {
        ...
        $response['response'] = ApiUtils::getOrderDeliveryListSection($this->token, $this->order, $sortn);
        return $response;
    }
 
    /**
    * Возвращает список доставок, доступных при оформлении заказа
    * ---
    * Если указан параметр filter_by_current_order=1, то в ответ будет получен 
    * массив для текущего оформляемого заказа из сессии, т.е. секция $filter в этом случае работать не будет,
    ...
    */
    protected function process($token = null, $filter = [], $sort = 'sortn', $page = "1", $pageSize = "20", $filter_by_current_order = 0)
    {
