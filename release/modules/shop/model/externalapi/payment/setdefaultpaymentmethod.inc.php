...
*/
class SetDefaultPaymentMethod extends AbstractAuthorizedMethod
{
    const RIGHT_SET = 1;
    ...
    {
        return [
            self::RIGHT_SET => t('Установка платежного метода по умолчанию.')
        ];
    }

    /**
     * Устанавливает способ платежа пользователя (из сохраненных карт) по-умолчанию
     *
     * @param string $token Авторизационный токен
     * @param string $saved_method_id ID привязанного платежного метода
     *
     * @example GET /api/methods/payment.setDefaultPaymentMethod?token=123123123123123123123&payment_id=1
     * Ответ:
     * <pre>
     * {
     *      "response": {
     ...
     */
    protected function process($token, $saved_method_id)
    {
        $response = [];
