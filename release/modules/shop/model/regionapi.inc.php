...
class RegionApi extends TreeList
{
    ...
    */
    ...
    /**
     * Добавляет фильтр по связанной с филиалом зоной
     *
     * @param $region_api
     * @return mixed
     */
    static function addAffiliateFilter($region_api)
    {
        if (ModuleManager::staticModuleExists('affiliate') && ModuleManager::staticModuleEnabled('affiliate')) {
            ...
            if ($current_affiliate && isset($current_affiliate['xzone']) && $current_affiliate['xzone'] != 0) {
                $regions = Request::make()
                    ->select('R.id,R.is_city,R.parent_id,XR.region_id,XR.zone_id')
                    ->from(new Region(), 'R')
                    ->join(new Xregion(), 'XR.region_id=R.id', 'XR')
                    ->where([
                        'XR.zone_id' => $current_affiliate['xzone']
                    ])->exec()->fetchSelected('is_city', null, true);
                ...
                if ($regions) {
                    ...
                    foreach ($regions as $is_city => $region) {
                        if (!$is_city) {
                            foreach ($region as $item) {
                                if ($item['parent_id'] != 0) {
                                    $regions_arr[] = $item['region_id'];
                                }else {
                                    $is_country = true;
                                }
                            }
                            if ($regions_arr) {
                                $filter_arr['parent_id:in'] = implode(',', HelperTools::arrayQuote($regions_arr));
                            }
                        }else {
                            foreach ($region as $item) {
                                $cities_arr[] = $item['region_id'];
                            }
                            if ($cities_arr) {
                                $query_key_title = $regions_arr ? '|id:in' : 'id:in';
                                $filter_arr[$query_key_title] = implode(',', HelperTools::arrayQuote($cities_arr));
                            }
                        }
                    }

                    if (!$is_country && $filter_arr) {
                        $region_api->setFilter(['&' => $filter_arr]);
                    }
                }
            }
        }
