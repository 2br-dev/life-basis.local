...
class PaymentApi extends EntityList
{
    ...
    public function getCheckoutPaymentList($user, $order)
    {
        ...
        if ($cart) {
            $cartdata = $cart->getCartData(false, false);
        ...
        foreach ($payment_list as $k => $pay_item) {  //Перевод оплат
            ...
            //Проверим условие, если выбран тип доставки Самовывоз, вернуть только доступные способы оплаты для выбранного склада
            if ($delivery && $delivery->getTypeObject()->isMyselfDelivery()) {
                //Получаем id склада, выбранного в выпадающем списке
                //Если не выбран старый Тип оформления заказа (Оформление в 4 шага), то использовать ПВЗ для получения кода ПВЗ/id склада
                if ($config['checkout_type'] != $config::CHECKOUT_TYPE_FOUR_STEP) {
                    if ($pvz = $order->getSelectedPvz()) {
                        $pvz_code = $pvz->getCode();
                    } else {
                        $pvz_list = $delivery->getTypeObject()->getPvzByAddress($order->getAddress());
                        if ($pvz_first = reset($pvz_list)) {
                            $pvz_code = $pvz_first->getCode();
                        }
                    }
                } else {
                    $pvz_code = $order->getWarehouse()->id;
                }

                if (!empty($pvz_code)
                    && !empty($pay_item['show_on_warehouses'])
                    && !in_array(0, $pay_item['show_on_warehouses'])
                    && !in_array($pvz_code, $pay_item['show_on_warehouses'])) {
                    unset($payment_list[$k]);
                }
            }
        }
    ...
    public static function getQrCodeUrl($order_or_transaction, $width = 200, $height = 200, $absolute = false)
    {
        if ($order_or_transaction instanceof Order) {
            ...
            //Некоторые банки некорректно интерпретируют длинные адреса, поэтому исключаем
    ...
    {
        return mb_substr(str_replace('|', '', htmlspecialchars_decode((string)$string)), 0, $max_length);
    }
}
