...
abstract class AbstractType
{
    ...
    protected function addReceiptItemsData(array $receipt, array $items, $operation_type)
    {
        ...
        foreach ($items as $item) {
            if ($item instanceof OrderItem) {
                ...
                if ($operation_type == self::OPERATION_SELL || $operation_type == self::OPERATION_SELL_REFUND) {
                    //Если это чек предоплаты или возврата предоплаты, то Признак предмета расчета нужно указывать "Платеж"
            ...
            if (!$shop_config['exclude_zero_cost_products'] || $item_data['sum'] > 0) {
    ...
    protected function getItemDataFromShipmentItem(ShipmentItem $shipment_item)
    {
        ...
        if ($item_entity instanceof Product && $item_entity['marked_class'] && isset($marked_classes[$item_entity['marked_class']])) {
            $result['nomenclature_code'] = str_replace(chr(29), '',
                $marked_classes[$item_entity['marked_class']]->getNomenclatureCode($shipment_item->getUit()));
        }
    ...
    protected function getItemDataFromOrderItem(OrderItem $order_item, $transaction_type = '')
    {
        ...
        switch ($order_item['type']) {
            case OrderItem::TYPE_PRODUCT:
                    ...
                    $result['payment_object'] = $product['payment_subject'] ?: CashRegisterApi::PAYMENT_SUBJECT_COMMODITY;
     ...
     * @return string
     */
    protected function getRightTaxForProduct(Order $order, Product $product, $transaction_type = '')
    {
        ...
        return $this->fetchVatTax($taxes, $address, $transaction_type);
    }
     ...
     * @return string
     */
    protected function getRightTaxForDelivery(Order $order, Delivery $delivery, $transaction_type = '')
    {
        $address = $order->getAddress();
     ...
     * @return string
     */
    protected function fetchVatTax($taxes, Address $address, $transaction_type = '')
    {
        $tax = new Tax();
        ...
        if ($tax['is_nds']) {
            $nds_settlement_field = ($transaction_type == Transaction::ENTITY_SHIPMENT)
                ? 'nds_shipment_settlement_rate' : 'nds_settlement_rate';

            $tax_rate = $tax->getRate($address);
            switch (floatval($tax_rate)) {
                case "5":
                    $tax_id = ($tax[$nds_settlement_field]) ? static::TAX_VAT105 : static::TAX_VAT5;
                    break;
                case "7":
                    $tax_id = ($tax[$nds_settlement_field]) ? static::TAX_VAT107 : static::TAX_VAT7;
                    break;
                case "10":
                    $tax_id = ($tax[$nds_settlement_field]) ? static::TAX_VAT110 : static::TAX_VAT10;
                    break;
                case "18":
                    $tax_id = ($tax[$nds_settlement_field]) ? static::TAX_VAT118 : static::TAX_VAT18;
                    break;
                case "20":
                    $tax_id = ($tax[$nds_settlement_field]) ? static::TAX_VAT120 : static::TAX_VAT20;
                    break;
                case "0":
