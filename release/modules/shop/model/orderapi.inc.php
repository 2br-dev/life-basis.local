...
class OrderApi extends AbstractModel\EntityList implements HasMeterInterface
{
    ...
    {
        $config = ConfigLoader::byModule('shop', $order['__site_id']->get());
        $mask = $config['generated_ordernum_mask'];
        $numbers = $config['generated_ordernum_numbers'];
        $try_count = 0;
        do {
            if ($try_count == 1000) {
                throw new \RS\Exception(t('За 1000 попыток не удалось сгенерировать уникальный идентификатор заказа. Увеличьте число цифр в маске номера заказа.'));
            }
            $try_count++;
    ...
    public function repeatOrder($order_id)
    {
        ...
        foreach ($order_cart['items'] as $uniq_id => $item) {
            ...
            $items[$uniq_id]['multioffers'] = unserialize((string)$items[$uniq_id]['multioffers']);
            foreach ($cartitem->getDiscounts() as $discount) {
                if ($discount->getSource() == Cart::DISCOUNT_SOURCE_OLD_COST) {
                    $items[$uniq_id]['discount_from_old_cost'] = $discount->getDiscount();
                }
            }
        }
        ...
        return Manager::fire('shop.repeatOrder', [
            'old_order' => $old_order,
            'new_order' => $new_order
        ])->getResult()['new_order'];
    }
     ...
     * @return array
     */
    static function getCreatorPlatformsListTitles($first = [])
    {
        $list = [
            Order::CREATOR_PLATFORM_SITE => t('Сайт'),
            Order::CREATOR_PLATFORM_API => t('API')
        ];

        $event_result = EventManager::fire('shop.creatorPlatforms.getList', $list);
     ...
     * @return array
     */
    public function getSellStatuses()
    {
        ...
        foreach($counts as $id => $count) {
            $data[] = [
                'label' => (isset($statuses[$id]) ? $statuses[$id]['title'] : t('статус удален')),
                'data' => (int)$count,
                'color' => isset($statuses[$id]) ? $statuses[$id]['bgcolor'] : '',
                'border_color' => isset($statuses[$id]) ? $statuses[$id]['bgcolor'].'DD' : '',
            ];
