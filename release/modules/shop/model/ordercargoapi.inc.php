...
class OrderCargoApi extends EntityList
{
    ...
    function validateCargos($order_id, $cargos)
    {
        ...
        if ($error = Rights::CheckRightError($this, ModuleRights::RIGHT_CARGO_CHANGING)) {
    ...
    /**
     * Возвращает список товаров для грузовых мест, а также информацию по максимальному количеству
     * В случае с маркированными товарами, каждый товар будет представлен отдельной позицией
     *
     * @param Order $order
     * @return array
     */
    public function getCargoOrderItems($order)
    {
        $result = [];
        $cart = $order->getCart();
        $shipped_uits = $this->getShippedUits($order);
        foreach($cart->getProductItems() as $key => $item) {
            $order_item = $item[Cart::CART_ITEM_KEY];
            $product = $item[Cart::TYPE_PRODUCT];
            $editable_amount = $order_item['amount'];
            if ($product['marked_class'] && isset($shipped_uits[$key])) {
                foreach($shipped_uits[$key] as $uit) {
                    $result[] = $item + [
                        'uit' => $uit,
                ...
                $editable_amount = $order_item['amount'] - count($shipped_uits[$key]);
            }

            if ($editable_amount > 0) {
                $result[] = $item + [
                    'max_amount' => $editable_amount
                ];
            }
        }
        return $result;
