...
class CashRegisterApi
{
    ...
    public static function getStaticOFDList()
    {
        return [
            ...
            self::PLATFORM_KALUGA_ASTRAL => t('Калуга Астрал'),
            self::PLATFORM_MAGNIT       => t('АО Тандер'),
            self::PLATFORM_INITPRO      => t('ИнитПро ОФД'),
            self::PLATFORM_ELEMENT      => t('ОФД Группа Элемент'),
            self::PLATFORM_BEELINE      => t('ПАО Вымпел коммуникации'),
            self::PLATFORM_KONTUR_NTT   => t('Контур НТТ'),
            self::PLATFORM_OFD_ONLINE   => t('ОФД Онлайн'),
            self::PLATFORM_INFORM_CENTER   => t('АО Информационный центр')
        ];
    }
    ...
    public static function getStaticPaymentSubjects($first = [])
    {
        $result = [
            ...
            self::PAYMENT_SUBJECT_WORK => t('Работа'),
            self::PAYMENT_SUBJECT_SERVICE => t('Услуга'),
            self::PAYMENT_SUBJECT_PAYMENT => t('Платеж (для ФФД 1.2)'),
    ...
    public static function getOFDReceiptUrlMask($ofd_type)
    {
        switch ($ofd_type) {
            ...
            case self::PLATFORM_KALUGA_ASTRAL:
                $url = "https://ofd.astralnalog.ru";
                break;
            case self::PLATFORM_MAGNIT:
                $url = "https://ofd-magnit.ru";
                break;
            case self::PLATFORM_INITPRO:
                $url = "https://ofd-initpro.ru";
                break;
            case self::PLATFORM_ELEMENT:
                $url = "https://e-ofd.ru";
                break;
            case self::PLATFORM_BEELINE:
                $url = "https://ofd.beeline.ru";
                break;
            case self::PLATFORM_KONTUR_NTT:
                $url = "https://konturntt.ru";
                break;
            case self::PLATFORM_OFD_ONLINE:
                $url = "https://ofd-online.ru";
                break;
            case self::PLATFORM_INFORM_CENTER:
                $url = "https://ofd.informcenter.ru";
                break;
            default:
                $url = "";
                break;
    ...
    function getReceiptUrl(Receipt $receipt)
    {
        ...
        if ($info) {
            $url = str_replace("%fn_number", $info->getFnNumber(), $url);
            $url = str_replace("%fiscal_document_number", $info->getFiscalDocumentNumber(), $url);
            $url = str_replace("%fiscal_document_attribute", $info->getFiscalDocumentAttribute(), $url);
        }
