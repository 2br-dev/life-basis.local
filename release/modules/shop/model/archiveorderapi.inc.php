...
class ArchiveOrderApi extends EntityList
{
    ...
    /**
     * Перемещает в архив заказы, созданные раннее, чем $old_days дней назад
     *
     * @param integer $old_days Количество дней
     */
    public function moveToArchiveOldOrders($old_days)
    {
        if ($old_days > 0) {
            $ids = Request::make()
                ->select('id')
                ->from(new Order())
                ->where("dateof < '#date'", [
                    'date' => date('Y-m-d H:i:s', strtotime('-' . $old_days . ' days'))
                ])
                ->where([
