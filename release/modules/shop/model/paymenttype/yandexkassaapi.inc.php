...
class YandexKassaApi extends AbstractType implements InterfaceRecurringPayments
{
    ...
    public function getFormObject()
    {
        $properties = [
            ...
            'is_holding' => (new Type\Integer())
                ...
                ->setDefault(0)
                ->setCheckboxView(1, 0),
            'payment_method_type' => (new Type\Varchar())
                ->setDescription(t('Способ оплаты'))
                ->setListFromArray([
                    '' => t('Все (Пользователь выбирает сам)'),
                    'bank_card' => t('Банковская карта'),
                    'yoo_money' => t('ЮМани'),
                    'sberbank' => t('Сбербанк'),
                    'tinkoff_bank' => t('Т-Банк'),
                    'sbp' => t('СБП'),
                    'sber_loan' => t('Покупки в кредит от сбербанка'),
                    'b2b_sberbank' => t('СберБанк Бизнес Онлайн'),
                    'mobile_balance' => t('Баланс телефона'),
                    'cash' => t('Наличные'),
                    'electronic_certificate' => t('Электронный сертификат')
                ])
        ];
    ...
    public function onResult(Transaction $transaction, HttpRequest $request)
    {
        ...
        if ($payment_id) {
            ...
            if (isset($response['income_amount']['value']) && isset($response['amount']['value'])) {
                $transaction['comission'] = $response['amount']['value'] - $response['income_amount']['value'];
                $transaction->update();
            }
     ...
     * @throws ShopException
     */
    public function recurringPayOrder(Order $order, SavedPaymentMethod $saved_payment_method): Transaction
    {
     ...
     * @throws ShopException
     */
    public function recurringPayBalanceFounds(User $user, float $cost, SavedPaymentMethod $saved_payment_method): Transaction
    {
    ...
    public function createPayment(Transaction $transaction)
    {
        ...
        if ($this->getOption('payment_method_type')) {
            $params['payment_method_data'] = [
                'type' => $this->getOption('payment_method_type')
            ];
        }
    ...
    protected function getFZ54CheckCustomerData(Transaction $transaction)
    {
        ...
        $customer = [];
        if ($full_name) {
            $customer['full_name'] = mb_substr($full_name, 0,256);
        }
        ...
        if ($transaction['order_id'] && !$transaction['entity']) {
            ...
            if ($cart = $order->getCart()) {
                foreach ($cart->getProductItems() as $item) {
                    ...
                    $cost = sprintf('%0.2f', round($single_cost, 2));
                    if ($cost > 0) {
                        $items[] = [
                ...
                foreach ($cart->getCartItemsByType(Cart::TYPE_DELIVERY) as $delivery_item) {
                    ...
                    $cost = sprintf('%0.2f', round($delivery_item['price'] - $delivery_item['discount'], 2));
                    if ($cost > 0) {
                        $items[] = [
                            'description' => mb_substr($title, 0, 128),
                            'quantity' => "1",
    ...
    protected function changeTransactionFromResponse(Transaction $transaction, array $response, bool $is_notice = false): ChangeTransaction
    {
        ...
        if ($changelog) {
            $changelog = (($is_notice) ? t('Уведомление: ') : '').$changelog;
    ...
    protected static function handbookNds()
    {
        static $nds = [
            ...
            TaxApi::TAX_NDS_5 => 7,
            TaxApi::TAX_NDS_7 => 8,
            TaxApi::TAX_NDS_105 => 9,
            TaxApi::TAX_NDS_107 => 10,
        ];
        return $nds;
    }
     ...
     * @return array
     */
    public function getTelegramProviderData(Transaction $transaction)
    {
        $receipt = $this->getParamsForFZ54Check($transaction);

        if (isset($receipt['customer']['email'])) {
            $receipt['email'] = $receipt['customer']['email'];
        }

        if (isset($receipt['customer']['phone'])) {
            $receipt['phone'] = $receipt['customer']['phone'];
        }
        unset($receipt['customer']);
