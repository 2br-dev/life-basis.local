...
class Patches extends AbstractPatches
{
    ...
    /**
     * Патч, добавляет возможность потенциальной реализации флажков "Включен" в корзине
     *
     * @return void
     */
    public function afterUpdate60181()
    {
        $cart_item = new CartItem();
        $cart_item->getPropertyIterator()->append([
            'enabled' => new Type\Integer([
                'description' => t('Включен для заказа'),
                'default' => 1,
                'allowEmpty' => false
            ]),
        ]);
    ...
    /**
     * Патч, переносит значение настройки генерации номера заказа
     */
    public function afterUpdate60180()
    {
        foreach(Manager::getSiteList() as $site) {
            $config = File::config($site['id']);
            $config->getPropertyIterator()->append([
            ...
            if ($config['use_generated_order_num']) {
                $config['order_num_generate_type'] = 'order_num_random';
                $config->update();
            }
        }
    }
    ...
    /**
     * Патч, добавляет возможность потенциальной реализации мультикорзин
     */
    public function afterUpdate60107()
    {
        $cart_item = new CartItem();
        $cart_item->getPropertyIterator()->append([
            'cart_id' => new Type\Integer([
                'description' => t('Идентификатор корзины'),
                'allowEmpty' => false
            ]),
        ]);

        $cart_item->addIndex(['site_id', 'user_id', 'cart_id'], AbstractObject::INDEX_KEY);
        $cart_item->addIndex(['site_id', 'session_id', 'cart_id'], AbstractObject::INDEX_KEY);
        $cart_item->dbUpdate();
    }

    /**
     * Патч обновляют значения в базе в связи с созданием единого поля Дата отгрузки
     * Патч необходим для переноса значений из старых полей модулей OZON, Yandex Market в новые
     */
    public function afterUpdate6050()
    {
        $order = new Order();
        ...
        try {
            Request::make()
                ->update(new Order())
                ->set("creator_platform_id = 'site'")
                ->where("creator_platform_id = ''")
                ->exec();
        } catch (Exception $e) {}
        ...
        try {
            Request::make()
                ->update(new Order())
                ->set("creator_platform_id = 'yandex_market'")
                ->where('id_yandex_market_cpa_order > 0')
                ->exec();
        } catch (Exception $e) {}
        ...
        try {
            Request::make()
                ->update(new Order())
                ->set("shipment_date = yandex_shipment_date")
                ->where('id_yandex_market_cpa_order > 0 AND yandex_shipment_date > 0')
                ->exec();
        } catch (Exception $e) {}
        ...
        try {
            Request::make()
                ->update(new Order())
                ->set("creator_platform_id = 'ozon_seller'")
                ->where('ozon_order_id > 0')
                ->exec();
        } catch (Exception $e) {}
    }
