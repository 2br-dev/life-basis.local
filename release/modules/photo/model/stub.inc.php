...
class Stub extends \Photo\Model\Orm\Image
{
    ...
    */
    ...
    {
        if ($force_create) {
            $theme_full_path = \Setup::$SM_RELATIVE_TEMPLATE_PATH . '/' .
                $this->scale . \Setup::$IMG_PATH . \Setup::$NOPHOTO_THEME_PATH;

            if (file_exists(\Setup::$PATH . $theme_full_path . '/' . $this->pic_id)) {
                $srcFolder = $theme_full_path;
            } else {
                $srcFolder = dirname(\Setup::$NOPHOTO_IMAGE);
            }
        } else {
            $srcFolder = '';
        }
        ...
        $cimg = new \RS\Img\Core(\Setup::$ROOT, $srcFolder, \Setup::$FOLDER . '/storage/photo/stub/resized');
        $url = $cimg->getImageUrl($this->theme_image_file, $width, $height, $theme, $absolute);

        if ($force_create) {
            $cimg->buildImage($this->theme_image_file, $width, $height, \Setup::$STUB_SCALE, $theme);
        }
