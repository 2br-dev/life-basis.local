...
class PhotoApi extends EntityList
{
    ...
    public function __construct()
    {
        ...
        //Если включена настройка показ фотографий в обратном порядке
     ...
     * Например, photo-catalogproduct, будет сконвертирован в имя класса \Photo\Model\PhotoType\CatalogProduct
     * т.е. первый минус конвертируется в \Model\PhotoType\, остальные минусы конвертируются в \
     *
     * @return AbstractType
     * @throws Exception
     */
    public static function getTypeClassInstance($type_class_short_name)
    {
        if (!preg_match('/^([^\-]+?)\-(.*)$/', $type_class_short_name, $match)) {
            throw new PhotoType\Exception(t('Неверно указан идентификатор типа связи'));
        } else {
            $type_class_name = str_replace('-', '\\', "-{$match[1]}-model-phototype-{$match[2]}");
        }

        if (class_exists($type_class_name)) {
            $link_type = new $type_class_name;
            if (!$link_type instanceof PhotoType\AbstractType) {
                throw new PhotoType\Exception(t('Класс связи должен быть потомком \Photos\Model\PhotoType\AbstractType'));
            }
            return $link_type;
        } else {
            throw new PhotoType\Exception(t('Не найден класс типа связи %0', [$type_class_name]));
        }
    }
    ...
    public function deletePreviewPhotos()
    {
        ...
        //Удаляем также миниатюры других модулей
        $result = FileTools::deleteFolder(\Setup::$PATH . '/storage/system/resized') && $result;
        $result = FileTools::deleteFolder(\Setup::$PATH . '/storage/banners/resized') && $result;
        return $result;
    }
