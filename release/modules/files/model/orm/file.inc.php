 ...
 * @property string $link_id ID связанного объекта
 ...
 * @property integer $user_id Пользователь, загрузивший файл
 * @property string $ip IP-адрес
 * @property string $date_of_upload Дата загрузки файла
 * --\--
 */
class File extends OrmObject
{
    ...
    function _init()
    {
        parent::_init()->append([
            ...
            'user_id' => new Type\User([
                'description' => t('Пользователь, загрузивший файл'),
                'visible' => false
            ]),
            'ip' => new Type\Varchar([
            ...
            'date_of_upload' => new Type\Datetime([
                'description' => t('Дата загрузки файла')
            ])
        ]);
    ...
    */
    ...
    {
        return RouterManager::obj()->getUrl('files-front-download', [
            'uniq_name' => $this['uniq_name']
        ], $absolute);
    }

    /**
     * Возвращает ссылку на скачивание файла со скрытым именем.
     * Необходмо использовать данный метод в публичной части для скачивания, если речь идет о контенте,
     * который загружается пользователями.
     *
     * @param bool $absolute
     * @return string
     */
    function getHashedUrl($absolute = false)
    {
        list($name, $ext) = \RS\File\Tools::parseFileName($this['name'], true);
        return RouterManager::obj()->getUrl('files-front-download', [
            'uniq' => $this['uniq'],
            'extension' => $ext
        ], $absolute);
    ...
    */
    ...
    {
        return RouterManager::obj()->getAdminUrl(false, ['uniq' => $this['uniq']], 'files-download', $absolute);
    }
    ...
    */
    ...
    {
        return FileApi::getStoragePath().'/'.$this['servername'];
    }
