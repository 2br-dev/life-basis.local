...
class Api
{
    ...
    function __construct($site_id = null, $map_type = null, $gzip = false)
    {
        ...
        //Если это филиал с поддоменом
        if (Manager::staticModuleEnabled('affiliate')) {
            $affiliate = AffiliateApi::getCurrentAffiliate();
            if ($affiliate['domain']) {
                $folder = \Setup::$FOLDER ? '/' . trim(\Setup::$FOLDER, '/') : '';
                $suffix = $affiliate->getDomain(false).$folder;
                $filename .= '_'.Transliteration::str2url($suffix);
            }
        }

        //Если есть партнёрские сайты и мы на нём, то добавим в нему ещё идентификатор
        if (Manager::staticModuleExists('partnership') && Manager::staticModuleEnabled('partnership')) {
    ...
    */
    function sitemapToOutput()
    {
        ...
        if (file_exists($this->full_filename)) {
            ...
            $content_type = $this->gzip ? 'application/x-gzip' : 'application/xml';
    ...
    */
    function generateSitemap()
    {
        ...
        $default_page = [
            'priority' => number_format((float)$config['priority'], 1, '.', '')
        ];
    ...
    public function createSitemapItemsFile($pagelist, $default_page, $chunk = null)
    {
        ...
        if (Manager::staticModuleEnabled('affiliate')) {
            ...
            if ($affiliate['domain']) {
                $folder = \Setup::$FOLDER ? '/' . trim(\Setup::$FOLDER, '/') : '';
                $base_url = $affiliate->getDomain(true).$folder;
            }
        }
