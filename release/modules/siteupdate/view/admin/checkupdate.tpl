<div class="viewport">
    {if $canUpdate && $isUpdateExpire}
        <p class="notice-box notice-bg">
            ...
            <a href="{$expireSaleBuyUrl}" target="_blank" class="u-link">{t}Продлите подписку на обновления{/t}</a>{if $expireSale}{t expireSaleDays=$expireSaleDays expireSale={$expireSale|dateformat:"@date @time"}} со <b>скидкой 30%</b> до %expireSale (осталось %expireSaleDays дней){/t}{else}.{/if}
        </p>
    {/if}
</div>
