    ...
    {foreach $items as $item}
        <li {if $item->getChildsCount()} class="sm-node rs-meter-group"{/if}>
            <a {if isset($sel_id) && $sel_id==$item.fields.id}class="active"{/if} {if $item->getChildsCount()}data-url="{$item.fields.link}"{else}href="{$item.fields.link}"{/if}>
                <i class="rs-icon rs-icon-{$item.fields.alias}" {if $item.fields.iconstyle}style="{$item.fields.iconstyle}"{/if}>
            ...
            {if $item->getChildsCount()}
                <div class="sm">
                    ...
                    <div class="sm-body">
                        <ul>
                            {include file="%menu%/admin/adminmenu_branch.tpl" list=$item.child is_second_level=true}
                        </ul>
                    </div>
                </div>
            {/if}
