...
class Handlers extends HandlerAbstract
{
    ...
    /**
     * Действия перед записью заказа. Установим источник
     *
    ...
    {
        $order->addIndex(['dateof', 'profit'], $order::INDEX_KEY);
        $order->addIndex(['dateof', 'totalcost'], $order::INDEX_KEY);
    ...
    {
        $oneclick->getPropertyIterator()->append([
            t('Статистика'),
                'source_id' => new Type\Integer([
                    'description' => t('Источник перехода пользователя'),
                    'default' => 0,
