...
{
    protected $schema;
    protected $dont_check_extra_keys = false;
    ...
    */
    function validate($param_name, $param_value, $full_data)
    {
        if (!is_array($param_value)) {
            throw new ApiException(t('Параметр %0 должен быть массивом или объектом', [$param_name]), ApiException::ERROR_WRONG_PARAM_VALUE);
        }
    ...
    * @return array Возвращает $param_value с приведенными к нужным типам значения
    */
    protected function recursiveValidation($schema, $param_name, $param_value, $full_data, $path = [])
    {
        ...
        foreach($schema as $key => $value) {
            ...
            if (!$path && $key == '@validate_callback') {
                ...
                if ($validate_result === false) {
                    throw new ApiException(t('Недопустимое значение в переменной %0', [$param_name]), ApiException::ERROR_WRONG_PARAM_VALUE);
                }
            ...
            $var_path = $cur_path ? $cur_path.' > '.$key : $key;
        ...
        if (is_array($param_value) && !$this->dont_check_extra_keys) {
            $allowable_keys = [];
    ...
    */
    ...
    /**
     * Отменяет проверку на наличие неописанных в схеме ключей
     *
     * @param bool $bool
     * @return void
     */
    public function setDontCheckExtraKeys($bool)
    {
        $this->dont_check_extra_keys = $bool;
    }
}
