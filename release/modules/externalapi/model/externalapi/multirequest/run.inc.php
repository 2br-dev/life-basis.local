    ...
    */
    protected function parseMethodParam($request)
    {
        ...
        if (isset($request['method'])) {
            ...
            //Удаляем подпапку сайта
            $site = Manager::getSite();
            $remove_prefix = \Setup::$FOLDER.($site['folder'] ? '/'.$site['folder'] : '');
            if ($remove_prefix) {
                $remove_prefix = rtrim($remove_prefix, '/');
                $info['path'] = preg_replace("/^".preg_quote($remove_prefix, '/')."/", '', $info['path']);
            }
