...
class Form extends DesignAtoms\AbstractAtom {
     ...
     * @param FormFieldItem $field - данные с параметрами поля
     *
     * @return array
     */
     ...
     * @param FormFieldItem $field - объект поля формы
     *
     * @return array
     */
     ...
     *
     * @param FormItem $form - объект формы
     * @param FormFieldItem $field - данные с параметрами поля
     * @return string
     */
    ...
    {
        $child = $children_params[FormFieldItem::SHOW_TYPE_STRING];
    ...
    {
        $child = $children_params[FormFieldItem::SHOW_TYPE_TEXT];
        $child['attrs']['name']['value'] = $field['alias'];
        return $child;
    }
    ...
    {
        $child = $children_params[FormFieldItem::SHOW_TYPE_FILE];
    ...
    private static function fillFieldsChild($children_params, $form, $field, $attrs)
    {
        ...
        if ($field['show_type'] == FormFieldItem::SHOW_TYPE_CAPTCHA){
            return [];
        }

        //Получит заголовок
        if ($field['show_type'] != FormFieldItem::SHOW_TYPE_YESNO && $attrs['show_field_header']['value']){
    ...
    public static function getChildParamsData()
    {
        ...
        if (!$childs){
            ...
            $childs[FormFieldItem::SHOW_TYPE_STRING] = self::getChildParamsDataForInput();
            $childs[FormFieldItem::SHOW_TYPE_TEXT]   = self::getChildParamsDataForTextArea();
            $childs[FormFieldItem::SHOW_TYPE_FILE]   = self::getChildParamsDataForFile();
    ...
    public static function getFillChildsDataForRender($data)
    {
        ...
        if ($data['form_id']){ //Если форма назначена
            $form = new FormItem($data['form_id']);
            if (!$form['id']){
            ...
            //Создадим поле checkbox для согласия на обработку персональных данных
            $field = new FormFieldItem();
            $field->alias = 'agree_with_data_policy';
            $field->show_type = FormFieldItem::SHOW_TYPE_YESNO;
