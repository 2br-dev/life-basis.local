...
class UploadApi extends BaseModel
{
    ...
    {
        $this->files_dir  = \Setup::$ROOT.\Setup::$STORAGE_DIR.self::FILES_DIR;
        $this->images_dir = \Setup::$ROOT.\Setup::$STORAGE_DIR.self::IMAGES_DIR;
    ...
    {
        return str_replace(\Setup::$ROOT, "", $file);
    }
    ...
    public static function getSVGImages()
    {
        static $images;
        if (!$images){
            foreach (glob(\Setup::$PATH.\Setup::$MODULE_FOLDER."/designer".\Setup::$MODULE_TPL_FOLDER."/img/iconsset/*", GLOB_ONLYDIR) as $folder){
                $category = basename($folder);
                foreach (glob($folder."/*") as $file){
                    $title = str_replace([".svg", "_"], ["", " "], basename($file));
                    $src   = \Setup::$FOLDER."/modules/designer/view/img/iconsset/".str_replace(\Setup::$PATH.\Setup::$MODULE_FOLDER."/designer".\Setup::$MODULE_TPL_FOLDER."/img/iconsset/", "", $file);
                    $images[] = [
                        'category' => $category,
                        'title' => $title,
                        'src' => $src,
