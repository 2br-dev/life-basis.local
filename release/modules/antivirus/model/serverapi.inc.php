...
class ServerApi extends BaseModel
{
    ...
    {
        if (class_exists('RS\Log\AbstractLog')) {
            $this->log = LogAntivirus::getInstance();
            $this->log->write(t("Создание объекта ServerApi"), LogAntivirus::LEVEL_SERVER_API);
        }
    }
    ...
    function requester($params)
    {
        ...
        if ($this->log) {
            $this->log->write("Request Params: " . json_encode($params, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT), LogAntivirus::LEVEL_SERVER_API);
            $this->log->write("Response:".$result, LogAntivirus::LEVEL_SERVER_API);
        }
    ...
    public function downloadOneFileContent($file)
    {
        ...
        while(($response = $this->downloadRecoverFiles([$file], $limit, $offset)) !== false)
        {
    ...
    */
    ...
    public function isServerFilesVersionsCompatible($files)
    {
        ...
        foreach($versions as $key => $value)
        {
            ...
            {
                return $this->addError(t('Версия модуля \'%0\' %1 не является последней (%2)', [$key, $value, $server_versions[$key]]));
            }
        }
