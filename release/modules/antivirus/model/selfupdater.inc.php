...
class SelfUpdater extends BaseModel
{
    ...
    public function __construct()
    {
        ...
        //Проверяем обновлено ли ядро, есть ли там новая
        //подсистема логирования, так как модуль может обновиться раньше
    ...
    public function log($msg)
    {
        ...
        Utils::log($out);
        if ($this->log) {
            $this->log->write($msg, LogAntivirus::LEVEL_SELF_UPDATE);
        }
    }
        ...
        if($state->file_id)
        {
            ...
            if($res === false)
            {
                ...
                if ($result = $module->install()) {
                    ...
                    self::log(t('Во время переустановки модуля возникли ошибки: %0', [implode(',', $result)]));
                }
            }
            else
            {
                ...
                self::log(t('Загружена очередная порция архива, offset: %0', [$state->offset]));
            }
        }
        return true;
    }
