...
class File extends ConfigObject
{
    ...
    public function _init()
    {
        $property_iterator  = parent::_init()->append([
            ...
            'zero_cost_sale_status' => new Type\Varchar([
                'description' => t('Переключать состояние продаж товара при нулевой цене?'),
                'list' => [['Catalog\Model\Orm\Product', 'getSaleStatusTitles']]
            ]),
            ...
            'search_fields' => new Type\ArrayList([
                'description' => t('Поля, которые должны войти в поисковый индекс товара (помимо названия, артикула).'),
            ...
            'search_barcode_like' => new Type\Integer([
                'description' => t('Разрешить неточный поиск по артикулу'),
                'hint' => t('Если установлен флаг, поиск товаров по артикулу будет выполняться по части строки (тип поиска Like).<br/> 
                Иначе по точному совпадению (Полнотекстовый тип поиска).'),
                'checkboxview' => [1, 0],
            ]),
            ...
            'show_filters_old_logic' => new Type\Integer([
                'description' => t('Включить старую логику отображения фильтров'),
                'hint' => t('Настройки отображения фильтра на сайте распространяются на дочерние категории, независимо от установленного флага, задающего персональное значения.'),
                'checkboxview' => [1, 0],
            ]),
            ...
            t('Обмен данными в CSV'),
                ...
                'csv_file_upload_access' => new Type\Varchar([
                    'description' => t('Уровень доступа к файлу по умолчанию'),
                    'list' => [function() {
                        $result = [];
                        foreach(CatalogProduct::getAccessTypes() as $id => $data) {
                            $result[$id] = is_array($data) ? $data['title'] : $data;
                        }
                        return $result;
                    }],
                ]),
            ...
            t('Склады'),
                'warehouse_sticks' => new Type\Varchar([
                    ...
                    которое будет соответствовать 1, 2, 3-м и т.д. "делениям" наличия данного товара на складе')
                ]),
            t('Складской учет'),
            ...
            t('Настройки импорта YML'),
                ...
                'yml_available_tag_stock' => new Type\Integer([
                    'description' => t('Устанавливать следующий остаток товара, если атрибут offer.available имеет значение true'),
                    'hint' => t('Значение 0 - будет означать, что данный тег использоваться не будет')
                ]),
            t('Параметры товаров'),
            ...
            t('Микроразметка'),
                'microdata_product_list_active' => new Type\Integer([
                'description' => t('Включить микроразметку ItemList в каталоге товаров'),
                'hint' => t('Если данный флаг установлен, дополнительно будет выводится микроразметка в виде массива товаров на странице'),
                'CheckboxView' => [1, 0],
            ]),
        ]);
        ...
        return parent::getDefaultValues() + [
            ...
            'tools' => [
                ...
                [
                    ...
                    'title' => t('Заархивировать документы складского учета'),
                ...
                [
                    ...
                    'title' => t('Разархивировать документы складского учета'),
