...
{addjs file="%catalog%/selectproduct.js?v=1.0" basepath="root"}
            ...
            {foreach from=$productArr.group item=item}
                <li class="group" val="{$item}"><a class="remove" title="{t}удалить из списка{/t}">&#215;</a>
                    ...
                    {if isset($plugin_options.additionalItemHtml)}
                        {str_replace(array('%field_name%', '%item_id%', '%type%'), array($fieldName, $item, 'groups'), $plugin_options.additionalItemHtml)}
                    {/if}
                </li>
            {/foreach}
        </ul>
        <ul class="product-block">
            {foreach from=$productArr.product item=item}
                <li class="product" val="{$item}">
                    ...
                    {if isset($plugin_options.additionalItemHtml)}
                        {str_replace(array('%field_name%', '%item_id%', '%type%'), array($fieldName, $item, 'products'), $plugin_options.additionalItemHtml)}
                    {/if}
                </li>
            {/foreach}
        </ul>
