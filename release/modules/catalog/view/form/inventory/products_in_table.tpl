...
{foreach $products as $document_product}
    ...
    <tr data-uniq="{$document_product.uniq}" class="product-row admin-style" style="border-bottom: solid 1px #dddddd">
        <td class="chk"><input class="m_delete" data-uniq="{$document_product.uniq}" type="checkbox"></td>
        ...
        {/if}
        <td>{if !$disable_edit}<a class="remove zmdi zmdi-delete c-red f-18" data-uniq="{$document_product.uniq}"></a>{/if}</td>
    </tr>
{/foreach}