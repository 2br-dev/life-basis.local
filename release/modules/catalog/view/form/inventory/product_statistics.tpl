<form  data-dialog-options='{ "width":"900"}'>
    ...
    {else}
        {$url_map = [
            'arrival' => 'catalog-inventoryarrivalctrl',
            'write_off' => 'catalog-inventorywriteoffctrl',
            'reserve' => 'catalog-inventoryreservationctrl',
            'waiting' => 'catalog-inventorywaitingsctrl'
        ]}
                                    ...
                                    {foreach $data.docs as $doc}
                                        {if $doc.archived != 1}
                                            <tr>
                                                ...
                                                <td>
                                                    {if isset($url_map[$doc.type])}
                                                        <a href="{$router->getAdminUrl('edit', ['id' => $doc.id], $url_map[$doc.type])}" class="crud-edit">{$doc.id}</a>
                                                    {else}
                                                        {$doc.id}
                                                    {/if}
                                                </td>
