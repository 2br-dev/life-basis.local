...
<div class="document" data-id="{$elem->id}">
    <div class="table-mobile-wrapper">
        <div class="order-beforetable-tools" style="margin: 20px 0">
            ...
            {/if}
            <input class="barcode-scanner" data-app-scan="ean_13,ean_8,upc_a,upc_e,code_39" data-app-scan-options='{ "autoPressEvent": "keypress" }' data-href="{$router->getAdminUrl('getProductBySku', array(), 'catalog-ctrl')}" type="text" placeholder="Добавить по штрихкоду">
        ...
        <div class="table-mobile-wrapper">
            <table class="pr-table rs-table" data-getnum="{$router->getAdminUrl('GetNum', null, 'catalog-inventorizationctrl')}">
            <thead>
            <tr>
                <th class="chk" style="width: 50px;">
                ...
                {if $is_inventory}
                    <th>{t}Расчетное кол-во{/t}</th>
                    <th>{t}Фактическое кол-во{/t}</th>
                    <th>{t}Разница{/t}</th>
                {else}
                <th>{t}Кол-во{/t}</th>
                {/if}
