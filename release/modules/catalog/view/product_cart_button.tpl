...
{hook name="catalog-product_cart_button:button" title="{t}Кнопка в корзину:кнопка{/t}" product=$product shop_config=$shop_config offers_data=$offers_data}
{if $shop_config}
    {if !$product.disallow_manually_add_to_cart}
        ...
        {if $has_any_offers || !$THEME_SETTINGS.button_as_amount}
            ...
            <div class="item-product-cart-action" {if $THEME_SETTINGS.show_offers_in_list}data-sol{/if}>
                ...
                <div class="item-card__not-available btn btn-outline-danger rs-unobtainable">{t}Нет в наличии{/t}</div>
                <div class="item-card__not-available rs-bad-offer-error"></div>
            </div>
        {else}
            {* Кнопка с количеством. Может испольоваться только, если нет комплектаций *}
            {$sale_status = $product->getSaleStatus()}
            {if $product->shouldReserve()
                || ($product->canBeReserved() && $sale_status == 'no_cost')
                || $sale_status == 'on_request'}
            ...
            {else}
                {if ($shop_config->check_quantity && $product->getNum() <= 0)
                    || in_array($sale_status, ['discontinued', 'no_cost'])}
                    <div class="item-product-cart-action">
                        <div class="item-card__not-available btn btn-outline-danger rs-unobtainable">{t}Нет в наличии{/t}</div>
                    </div>
                {else}
