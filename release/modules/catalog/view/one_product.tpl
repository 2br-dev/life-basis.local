...
<div class="item-card rs-product-item
                ...
                {$product->getDebugAttributes()} data-sale-status="{$product->getSaleStatus()}" data-id="{$product.id}">
    <div class="item-card__inner">
        <div class="position-relative mb-2">
            ...
            {/if}
            {hook name="catalog-list_products:blockview-image" title="{t}Просмотр категории продукции:изображение товара, блочный вид{/t}" product=$product}
            <a href="{$product->getUrl()}" class="item-product-img rs-to-product">
                <canvas width="268" height="268"></canvas>
                <img src="{$product->getMainImage()->getUrl(268, 268)}" srcset="{$product->getMainImage()->getUrl(536, 536)} 2x" loading="lazy" alt="{$product.title}" class="rs-image">
            </a>
            {/hook}
        </div>
        {if $THEME_SETTINGS.show_rating}
            <a href="{$product->getUrl()}" class="item-product-reviews mb-2 rs-to-product">
        ...
        {/if}
        {hook name="catalog-list_products:blockview-title" title="{t}Просмотр категории продукции:название товара, блочный вид{/t}" product=$product}
            <a href="{$product->getUrl()}" class="item-card__title rs-to-product">{$product.title}</a>
        {/hook}
        ...
        <div class="item-product-price item-product-price_card rs-price-block">
            ...
            <div class="item-product-price__new-price">
                <span class="rs-price-new">{$cur_cost}</span> {$product->getCurrency()}
                {if $catalog_config.use_offer_unit}
                    {$unit = $product->getUnit()}
                    {if ($offers_data && $offers_data.offers) || $unit }
                        <span class="rs-unit-block">/ <span class="rs-unit">{$offers_data.offers[0].unit|default:$unit->stitle}</span></span>
                    {/if}
                {/if}
            </div>
            {if $old_cost && $old_cost != $cur_cost}
                <div class="item-product-price__old-price"><span class="rs-price-old">{$old_cost}</span> {$product->getCurrency()}</div>
            {/if}
        </div>
