...
{$offers_data = $product->getOffersJson(['images' => [], 'disableCheckOffers' => true, 'showStockNum' => $THEME_SETTINGS.show_product_stock], true)}

<div class="product-variant-{$THEME_SETTINGS.product_card_view_type}
            product-variant-tab-{$THEME_SETTINGS.product_tabs_view_type}
            rs-product
            {if $THEME_SETTINGS.product_zoom_variant == 'double_click_zoom'}rs-zoom{/if}
            {if $THEME_SETTINGS.product_zoom_variant == 'lightbox_zoom'}rs-lightbox{/if}
            {if !$product->isAvailable()} rs-not-avaliable{/if}
            {if $product->canBeReserved()} rs-can-be-reserved{/if}
            {if $product->isReservationForced()} rs-forced-reserve{/if}"
            data-sale-status="{$product->getSaleStatus()}"
            data-id="{$product.id}">
        ...
        <div class="container">
            <div class="row g-3">
                ...
                <div class="col order-lg-first">
                    {hook name="catalog-product:title" title="{t}Карточка товара:название товара{/t}"}
                    ...
                    <div class="row align-items-center row-cols-auto g-3 gx-5">
                        ...
                        {/if}
                        {hook name="catalog-product:labels" title="{t}Карточка товара:метки спецкатегорий{/t}"}
                    ...
                    {hook name="catalog-product:images" title="{t}Карточка товара:изображения{/t}"}
                        <div class="product-gallery">
                                    ...
                                    {foreach $images as $image}
                                        <div class="swiper-slide" data-image-id="{$image.id}">
                                            <a class="d-block swiper-zoom-container"
                                               {if $THEME_SETTINGS.product_zoom_variant == 'lightbox_zoom'}href="{$image->getUrl(1200, 900)}"{/if}>
                            ...
                            <div class="product-gallery-thumbs-wrap">
                                {if $product->getVideoPlayerEmbedUrl()}
                                ...
                                {hook name="catalog-product:information" title="{t}Карточка товара:краткая информация{/t}"}
                                    ...
                                    {if $THEME_SETTINGS.enable_short_description_in_product_card}
                                        <div class="d-none d-lg-block mt-4 fs-5 text-gray">
                                            <div>{$product.short_description|nl2br}</div>
                                        </div>
                                    {/if}
                                {/hook}
                            </div>
                            ...
                            {hook name="catalog-product:buyblock" title="{t}Карточка товара:Блок покупки{/t}"}
                                ...
                                <div class="product-controls">
                                    ...
                                    <div class="mb-xl-5 mb-md-4 mb-3">
                                        <div class="item-product-sale-status rs-sale-status">{$product->getSubstitutePriceText()}</div>
                ...
                <div class="col-xxl-8 col-xl-9 mt-6 mt-xl-0">
                    ...
                    {if !empty($tabs)}
                        <div class="tab-content" id="pills-tabContent">
                            {if $tabs.description && (!$tab || $act_tab == 'description')}
                                                                    ...
                                                                    <div class="col-sm-7 col-6">{$property.title} {if $property.unit}({$property.unit}){/if}
                                                                        {if $property.description}
                                                                            <a class="btn-popover"
                                                                               ...
                                                                               data-bs-html="true"
                                                                               tabindex="0"
                                                                               data-bs-content="{$property.description|escape}"> ? </a>
                                                                        {/if}
                                                                    </div>
                            ...
                            <div class="tab-pane fade {if $act_tab == 'stock'}show active{/if}" id="tab-stock">
                                {hook name="catalog-product:stock" title="{t}Карточка товара:остатки{/t}"}
                                    ...
                                    <div>
                                        {$main_offer = $offers_data.offers[0]}
                                        {foreach $stick_info.warehouses as $warehouse}
                                            {$sticks = $main_offer.sticks[$warehouse.id]}
                                            {$stock = $main_offer.stock_num[$warehouse.id]}
                                            <div class="product-availability-item rs-warehouse-row{if !$sticks} rs-warehouse-empty{/if}" data-warehouse-id="{$warehouse.id}">
                                                <div class="row g-2 g-lg-4 align-items-lg-center gx-3">
                                                    ...
                                                    <div class="col-auto col-lg-2 order-1 order-lg-2">
                                                        {if $THEME_SETTINGS.show_product_stock}
                                                            {if $stock < 0}
                                                                {$stock = 0}
                                                            {/if}
                                                            <span class="rs-stock-number">{$stock|format_price}</span>
                                                            <span class="rs-stock-unit">{$main_offer.unit}</span>
                                                        {else}
...
{if $video_embed_url = $product->getVideoPlayerEmbedUrl()}
    <div class="modal fade" id="modal-video">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 p-0 bg-transparent ratio ratio-16x9">
                <iframe class="youtube-player" width="100%" height="100%" src="{$video_embed_url}"
