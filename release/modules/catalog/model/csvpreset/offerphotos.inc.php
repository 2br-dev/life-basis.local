...
class OfferPhotos extends \RS\Csv\Preset\AbstractPreset
{
    ...
    */
     ...
     * @param string $field
     */
    function setExportForeignField($field)
    {
        $this->export_foreign_field = $field;
    }
     ...
     * @return string
     */
    function getExportForeignField()
    {
        return $this->export_foreign_field;
    }
    ...
    */
    ...
    {
        return [
            $this->id.'-photos_named_arr' => [
                'key' => 'photos_named_arr',
                'title' => $this->title
            ]
        ];
    }
    ...
    */
    function getColumnsData($n)
    {
        ...
        foreach($this->getColumns() as $id => $column) {
            $photos_ids = $this->schema->rows[$n][$this->getExportForeignField()];
            $values_arr = [];
            if (count((array)$photos_ids)){ //Если есть назначенные фото
               ...
               foreach($photos as $photo){
                  $values_arr[] = basename($photo['servername']);
               }     
            }
