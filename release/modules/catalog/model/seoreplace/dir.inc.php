    ...
    */
    function __construct(array $real_replace = [])
    {
        $this->struct = [         //Структурный массив автозамены
            ...
            'parent_name' => t('Название родительской категории'),
            'parent2_name' => t('Название родительской категории 2-го уровня'),
        ];
    ...
    function beforeReplace($text, $real_struct)
    {
        ...
        //Добавим родительскую категорию
        $text = preg_replace_callback('/\{parent(\d*)_name\}/si', function($match) use ($dir) {
            $index = $match[1] ?: 1;
            if ($dir['parent'] > 0) {
                for($i=0; $i<$index; $i++) {
