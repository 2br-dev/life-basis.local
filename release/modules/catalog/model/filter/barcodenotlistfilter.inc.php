 ...
 */
class BarcodeNotListFilter extends AbstractType
{
    public
    ...
    function modificateQuery(\RS\Orm\Request $q)
    {
        if ($this->getValue() != '') {
            ...
            $exclude_product_ids = \RS\Orm\Request::make()
                ->select('product_id')
                ->from(new Offer())
                ->whereIn('barcode', $barcode_list)
                ->exec()->fetchSelected(null, 'product_id');

            if ($exclude_product_ids) {
                $q->whereIn('A.id', $exclude_product_ids, 'AND', true);
            }

            $q->whereIn('A.barcode', $barcode_list, 'AND', true);
