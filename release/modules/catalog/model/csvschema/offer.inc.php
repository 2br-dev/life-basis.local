...
class Offer extends \RS\Csv\AbstractSchema
{
    ...
    function __construct()
    {
        ...
        $presets = [
            ...
            new \Catalog\Model\CsvPreset\StockCost([
                'linkPresetId' => 0,
                'linkIdField' => 'id',
                'sortnField' => 'sortn',
            ...
            new \Catalog\Model\CsvPreset\OfferPhotos([
                ...
                'linkForeignField' => 'photos_named_arr',
                'exportForeignField' => 'photos_arr',
                'title' => t('Фотографии'),
            ])
        ];
        ...
        parent::__construct(new \Catalog\Model\CsvPreset\Offer([
            ...
            'beforeRowImport' => function ($_this) {
                ...
                if (isset($_this->row['propsdata'])) {
                    $_this->row['propsdata_arr'] = unserialize((string)$_this->row['propsdata']);
                }
                return null;
            },
        ...
        if ($preset->row['product_id'] && empty($preset->row['sortn'])) {
            ...
            if (isset($preset->row['pricedata_arr'])) {
                ...
                if (isset($pricedata['price'])) {
                    ...
                    $product_update_data['excost'] = $excost;
                }
            }

            if (isset($preset->row['barcode'])) {
                $product_update_data['barcode'] = $preset->row['barcode'];
            }

            if ($product_update_data) {
                $product = new \Catalog\Model\Orm\Product($preset->row['product_id']);
                $product->getFromArray($product_update_data);
                $product->update();
            }
