...
class GetList extends \ExternalApi\Model\AbstractMethods\AbstractAuthorizedMethod
{
    ...
    function __construct()
    {
        ...
        $this->product_api->setAffiliateRestrictions(true);
    }
    ...
    * ---
    * Для незарегестрированных пользователей передавать токен не нужно, а для присутствующих в системе обязательно.
    ...
    * @return array
    */
    function process($token = null, $page = 1, $pageSize = 20, $sections = ['image', 'cost', 'property', 'current_currency', 'unit'])
    {
        ...
        $product->getPropertyIterator()->append([
            ...
            'offers_list_info' => new \RS\Orm\Type\ArrayList([
                'appVisible' => true
            ]),
            'in_cart' => new \RS\Orm\Type\Varchar([
                'appVisible' => true
            ]),
            'amount_in_cart' => new \RS\Orm\Type\Varchar([
                'appVisible' => true
            ])
        ]);

        foreach($list as $product) {
            $offers_info = \Catalog\Model\ApiUtils::fillOffersFromJSON($product);
            $product['offers_list_info'] = $offers_info ?: \Catalog\Model\ApiUtils::getOffersInfo($product, ['stock', 'costs']);

            if (\RS\Module\Manager::staticModuleExists('shop') && \RS\Module\Manager::staticModuleEnabled('shop')) {
                $product['in_cart'] = $product->inCart();
                $product['amount_in_cart'] = $product->getAmountInCart();
            }
        }
