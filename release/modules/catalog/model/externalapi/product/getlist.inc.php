...
use Catalog\Model\SearchType\ProductSearchType;
...
class GetList extends AbstractGetList
{
    const
        ...
        FILTER_TYPE_ALREADY_BUY = 'alreadybuy'; //Фильтр директорий
    ...
    function getDaoObject()
    {
        if ($this->dao == null){
            ...
            $this->dao->setAffiliateRestrictions(true);
    ...
    public function getAllowableFilterKeys()
    {
        return [
            'query' => [
                'title' => t('Универсальный поиск по различным полям. Использует стандартные настройки поиска в системе'),
                'type' => 'string',
                'func' => 'fullSearch'
            ],
            'id' => [
            ...
            'already_buy' => [
                'title' => t('Уже покупали'),
                'type' => 'string',
                'func' => self::FILTER_TYPE_ALREADY_BUY,
                'values' => [1,0]
            ]
        ];
    }
    ...
    protected function makeFilterFullSearch($key, $value, $filters, $filter_settings)
    {
        ...
        $search->setSearchType(new ProductSearchType());
    ...
    public function setFilter($dao, $filter)
    {
        ...
        $config = Loader::byModule('catalog');
     ...
     * @return array
     */
    protected function makeFilterAlreadyBuy($key, $value, $filters, $filter_settings)
    {
        if (!empty($value) && $value == 1){
            $this->dao->applyAlreadyBuyFilter();
        }
        return [];
    }
    ...
    public function getResultList($dao, $page, $pageSize)
    {
        ...
        $this->list_products = $dao->addProductsDynamicNum($this->list_products);
        ...
        foreach ($list as $key => &$product){
            $product['short_description'] = nl2br((string)$product['short_description']);
            $product['description'] = Utils::prepareHTML($product['description']);
            $product['num'] = $this->list_products[$key]->getNum();

            if (\RS\Module\Manager::staticModuleExists('shop') && \RS\Module\Manager::staticModuleEnabled('shop')) {
                $product['in_cart'] = $this->list_products[$key]->inCart();
                $product['amount_in_cart'] = $this->list_products[$key]->getAmountInCart();
            }

            $offers_info = \Catalog\Model\ApiUtils::fillOffersFromJSON($this->list_products[$key]);
            $product['offers_list_info'] = $offers_info ?: \Catalog\Model\ApiUtils::getOffersInfo($this->list_products[$key], ['stock', 'costs']);
        }
     ...
     * @param string $fulltext_filter Текстовый фильтр (используется для совместимости со старыми приложениями, в будущем будет удалено. Необходимо использовать вместо него filter[query])
     *
     * Возможные значения:
     ...
     */
    function process($token = null, $filter = [], $sort = 'dateof desc', $page = 1, $pageSize = 20, $sections = ['image', 'cost', 'property', 'concomitant', 'unit', 'current_currency', 'filters'], $fulltext_filter = '')
    {
        if ($fulltext_filter != '') {
        ...
        if (!empty($this->list_products)){ //Если товары найдены
            ...
            if ((isset($filter['dir']) && ((is_array($filter['dir']) && count($filter['dir'])==1) || !is_array($filter['dir'])) && in_array('filters', $sections))
                ...
                if (!$category['is_virtual']){
                    ...
                    $brands_filters_list = $this->dao->getAllowableBrandsValues($this->filter_cache, false);
                    $brands = [];
