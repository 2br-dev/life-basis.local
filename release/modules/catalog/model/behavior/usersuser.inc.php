...
class UsersUser extends BehaviorAbstract
{
    ...
    public function getUserTypeCostId($site_id = null, $cache = true)
    {
        ...
        $user = $this->owner;
        $cache_cost_id = $user->cache_cost_id;
        if (!isset($cache_cost_id[$site_id]) || !$cache) {
            /** @var User $user */
            $user_cost = @unserialize((string)$user['cost_id']);
            ...
            if (!$cost_id) {
                ...
                foreach ($sorted_groups as $group) {
                    $user_cost = @unserialize((string)$group['cost_id']);
            ...
            $cache_cost_id[$site_id] = $cost_id;
            $user->cache_cost_id = $cache_cost_id;
        }

        return $cache_cost_id[$site_id];
    }
}
