...
class PropertyApi extends EntityList
{
    ...
    */
    ...
    {
        $site_id = SiteManager::getSiteId();
        if ($obj_id !== null) { //Создать связь с категориями можно только если есть $obj_id
    ...
    */
    ...
    function getGroupProperty($group_id, $include_parent = true, $public = null)
    {
        ...
        if ($this->use_static_cache && !isset($cache[$hash])) {
            ...
            $public_where = ($public !== null && $config['show_filters_old_logic']) ? "public='".(int)$public."' " : null;
            ...
            foreach ($res as $prop_id => $prop_links) {
                $first_link = reset($prop_links); //Link самой верхней категория по дереву
                $last_link = end($prop_links); //Link самой крайней категории по дереву
                ...
                foreach ($prop_links as $link) {
                    if ($link['group_id'] == $last_link['group_id']) {
                        ...
                        if ($link['public']) {
                            $item['public'] = true;
                        } elseif ($public && !$config['show_filters_old_logic']) {
                            $item['public'] = false;
                        }
    ...
    */
    protected function listFilter($pn, $prop, $value, \RS\Orm\Request $q)
    {
        ...
        $this->last_filtered_props[$prop['id']] = [
            'property' => $prop,
            'values' => ItemValue::getValueDataById($value)
        ];
    }
    ...
    */
    ...
    function cleanUnusedProperty()
    {
        ...
        \RS\Orm\Request::make()
            ...
            ->where("V.site_id = '#site_id' AND I.id IS NULL", ['site_id' => $site_id])
            ->exec();
    ...
    */
     ...
     *
     * @param Api $product_api - Объект выборки товаров
     * @param $filters - фильтры примененные к категории
     * @param array $base_filters
     * @param integer $dir_id - id категории
     * @param array $brand_id_allowed
     * @return array - массив фильтров которые нужно выключить
     */
    function getSwitchedOffFilters(Api $product_api,
                                           $filters,
                                           $base_filters,
                                           $dir_id = null,
                                           $brand_id_allowed = [],
                                           $filters_allowed = [])
    {
        ...
        if ($prop_ids) {
            ...
            //Если в категории есть публичные фильтры по характеристикам,
            //то рассчитываем доступность для выбора значений брендов и фильтров
            $result_filters_allowed = [
                'brand' => array_fill_keys($brand_id_allowed, false)
            ];

            foreach ($filters_allowed as $prop_id => $item) {
                if (isset($item['allowed_values'])) {
                    foreach ($item['allowed_values'] as $value_id => $value) {
                        //По умолчанию выключаем все фильтры.
                        //Если они встретятся в выборке товаров ниже, то включим
                        $result_filters_allowed[$prop_id][$value_id] = false;
                    }
                }
            }

            $product_request = clone $product_api->queryObj();
            $product_request->select = 'L.prop_id, L.val_list_id, A.id as product_id, A.brand_id';
            $product_request->leftjoin(new Orm\Property\Link(), 'L.product_id = A.id AND L.prop_id IN(' . implode(',', $prop_ids) . ')', 'L');
            $product_request->where('(brand_id > 0 OR val_list_id IS NOT NULL)');

            $product_request->orderby = null;
            $product_request->groupby = null;
            $resource = $product_request->exec();

            $sorted_props_prop = [];
            $sorted_props_prod = [];

            $default_empty_props   = array_fill_keys($prop_ids, []);
            while ($prop = $resource->fetchRow()) {
                if ($prop['brand_id']) {
                    ...
                    $result_filters_allowed['brand'][$prop['brand_id']] = true;
                }
                if ($prop['val_list_id']) {
                    ...
                    $result_filters_allowed[$prop['prop_id']][$prop['val_list_id']] = true;
                }
            }
            ...
            foreach ($filters_new as $prop_id => $val_list_id) {
                    ...
                    foreach ($val_list_id as $val_id) {
                        if (isset($sorted_props_prop[$prop_id][$val_id]))
                            foreach ($sorted_props_prop[$prop_id][$val_id] as $product_id) {
                                //Перебираем все товары, которые принадлежат одному фильтру и смотрим,
                                //как у этого товара обстоят дела с другими характеристиками

                                if (isset($sorted_props_prod[$product_id]))
                                    foreach ($sorted_props_prod[$product_id] + $default_empty_props as $key => $item) {
                                        if ($key != $prop_id) { //$key - это "brand" или ID характеристики
                                            // Собираем массив свойств которые должны быть выключены
                                            $turn_off_values = array_diff(array_keys($result_filters_allowed[$key]  ?? []), $item);
                                            if (!isset($array_prop_diff[$key])) {
                                                // Находим значения фильтров, которых нет в списке у товара
                                                $array_prop_diff[$key] = $turn_off_values;
                                            } else {
                                                // Расширяем пересечением список выключенных значений.
                                                // Выключаем только те значения, которых нет абсолютно у всех товаров.
                                                $array_prop_diff[$key] = array_intersect($array_prop_diff[$key], $turn_off_values);
                                            }
                                        }
                                    }
                ...
                foreach ($array_prop_diff as $prop_key => $prop_val) {
                    foreach ($prop_val as $val_id_1) {
                        $result_filters_allowed[$prop_key][$val_id_1] = false;
                    }
                }
            }

        } else {
            //Если в категории нет фильтров по характеристикам, то бренды не блокируем
            $result_filters_allowed['brand'] = array_fill_keys($brand_id_allowed ?? [], true);
        }

        return $result_filters_allowed;
    }
}
