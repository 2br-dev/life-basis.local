...
class Offer extends OrmObject
{
    ...
    function afterObjectLoad()
    {
        ...
        if (!empty($this['photos']) && !$this->isModified('photos_arr')) {
            $photos = @unserialize((string)$this['photos']);
        ...
        if (!empty($this['pricedata'])) {
            $this['pricedata_arr'] = @unserialize((string)$this['pricedata']) ?: [];
        } else {
        ...
        if (!empty($this['propsdata'])) {
            $this['propsdata_arr'] = @unserialize((string)$this['propsdata']) ?: [];
        }
    ...
    function beforeWrite($flag)
    {
        ...
        if ($this['pricedata_arr'] == null) {
            //По умолчанию устанавливаем +0% от начальной цены товара для комплектации
            $this['pricedata_arr'] = [
                'oneprice' => [
                    'use' => 1,
                    'znak' => '+',
        ...
        if ($flag != self::UPDATE_FLAG && !$this->isModified('sortn')) {
            ...
            if ($this['xml_id'] && $this['use_exchange_xml_id_logic']) { //Если это подкомплектация и идет обмен с 1С
    ...
    {
        $unit_id = $this['unit']
                    ?: $this->getProduct()->unit
                    ?: ConfigLoader::byModule($this)->default_unit;
     ...
     * @return Product
     */
    function getProduct($cache = true)
    {
        $this->cache_product;
        if (!isset($this->cache_product) || $this->cache_product['id'] != $this['product_id'] || !$cache) {
            $this->cache_product = new Product($this['product_id']);
        }
        return $this->cache_product;
    }
