...
class Item extends OrmObject
{
    ...
    function afterWrite($flag)
    {
        ...
        if (isset($this->before_this)) {
            if ($this['type'] != $this->before_this->type) {
                ...
                //Очищаем хэш импорта у связанных товаров, чтобы при следующем обмене с 1С данные товара были обновлены
                $this->cleanLinkedProductsImportHash();
            }
        }
    }

    /**
     * Очищает хэш импорта у связаанных с данной характеристикой товаров
     */
    function cleanLinkedProductsImportHash()
    {
        Request::make()
            ->update(new Product())->asAlias('P')
            ->join(new Link(), 'L.product_id = P.id', 'L')
            ->where([
                'L.prop_id' => $this['id']
            ])
            ->set([
    ...
    function isListType()
    {
        if ($this->_cache_is_list === null) {
            ...
            $this->_cache_is_list = $types[(string)$this['type']]['is_list'] ?? false;
        }
