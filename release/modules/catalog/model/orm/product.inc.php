 ...
 * @property string $date_of_update Дата изменения
 * @property string $sale_status Статус продаж товара
 ...
 * @property string $video_link Ссылка на видео (YouTube, RuTube, VK Видео)
 * --\--
 */
class Product extends OrmObject
{
        ...
        parent::_init()->append([
            t('Основные'),
                ...
                'weight' => new Type\Real([
                    'description' => t('Вес'),
                    'hint' => t('Вес товара. Единица измерения устанавливается в настройках модуля каталог. У всех товаров вес должен быть указан в одних единицах измерения'),
                ...
                'date_of_update' => new Type\Datetime([
                    ...
                    'hint' => t('Дата устанавливается автоматически при любом изменении товара'),
                    'readOnly' => true
                ]),
                ...
                'sale_status' => new Type\Varchar([
                    'description' => t('Статус продаж товара'),
                    'list' => [[__CLASS__, 'getSaleStatusTitles'], [self::SALE_STATUS_DEFAULT => t('По умолчанию')]],
                    'hint' => t('По умолчанию - означает, что при положительной цене она будет отображаться, а при нулевой отображение будет зависеть от значения опции в настройках модуля Каталог товаров. При статусе, отличном от "Показывать цену", добавить товар в корзину будет невозможно'),
                    'default' => self::SALE_STATUS_DEFAULT,
                    'allowEmpty' => false
                ]),
            ...
            t('Видео'),
                'video_link' => new Type\Varchar([
                    'description' => t('Ссылка на видео (YouTube, RuTube, VK Видео)'),
                    'hint' => t('Ссылка должна иметь вид:<br> https://www.youtube.com/watch?v=ID<br>https://rutube.ru/video/38029c64a...aa9c92e85503/<br>https://vk.com/video-12342343_8921713')
                ]),
        ]);
    ...
    protected function buildOffersStockStars($offer_items) {
        ...
        $warehouses_info = WareHouseApi::getWarehouseStickInfo();
        $warehouses_ids = [];
    ...
    /**
     * Добавляет к объекту запроса комплектаций условие для выборки
     ...
     * После вызова данного метода fillOffers() будет также добавлять dynamic_num к каждой комплектации
    ...
    function calculateUserCost()
    {
        if ($this->calculate_user_cost === null) {
            ...
            //Тут сохраним цены без обработки сторонними модулями
            $this['xcost_original'] = $this['xcost'];
    ...
    public function getBaseCost($cost_id = null, $offer_id = null)
    {
        if ( $this->getUserCost() !== null ) {
            ...
            if ($this->isMainOffer($offer_id)) {
                ...
                $xcost = $this->getOfferCost($offer_id, $this['xcost_original']);
            }
    ...
    public function getOfferCost($offer_key, $xcost)
    {
        if (!isset($this->offer_xcost[$offer_key])) {
            $this->fillOffers();
            if (!$this->isMainOffer($offer_key) && isset($this['offers']['items'][$offer_key])) {
                $offer = $this['offers']['items'][$offer_key]['pricedata_arr'];

                foreach ($xcost as $cost_id => $base) {
                    $xcost[$cost_id] = CostApi::calculateCostFromPriceData($offer, $cost_id, $base);
                }
     ...
     * @param bool $hide_self Если true, то исключит из списка текущий товар
     * @return Product[]
     */
    function getRecommended($return_hidden = false, $add_dir_recommended = true, $hide_self = true)
    {
        $list = [];
        if (!empty($this['recommended_arr']['product'])) {
            ...
            $api->setOrder('FIELD(id, '.implode(',', \RS\Helper\Tools::arrayQuote($ids)).')');
     ...
     * Без учета статуса продаж
     *
     * @return bool
     */
    function shouldReserve()
    {
        if ($this->isReservationForced()) {
            return true;
        }
        elseif ($this['reservation'] == 'default') {
            if ($shop_config = ConfigLoader::byModule('shop')) {
                return ($shop_config['reservation'] && $shop_config['check_quantity'] && $this->getNum() <= 0);
            }
        }
    ...
    /**
     * Возвращает true, если товар (в основной комплектации) доступен только через предзаказ
     * Без учета статуса продаж
     *
     * @return bool
     */
    function isReservationForced()
    {
        return $this['reservation'] == 'forced';
    }
    ...
    function canBeReserved()
    {
        ...
        return ($shop_config && $shop_config['reservation'] && $this['reservation'] != 'throughout') || $this->isReservationForced();
    }
    ...
    function isAvailable($only_main_offer = true)
    {
        ...
        if ($only_main_offer) {
            ...
            if ($this->fast_mark_offers_use === false) {
                return $this->getNum() > 0; //Если комплектаций нет, то товар доступен, если его общий остаток больше нуля
            }

            $offers_data = $this->getOffersJson(['images' => [], 'disableCheckOffers' => true], true);
    ...
    /**
     * Возвращает ссылку для воспроизведения видео в iFrame для сервисов YouTube и RuTube
     *
     * @return string|null
     */
    public function getVideoPlayerEmbedUrl()
    {
        if ($id = $this->getYoutubeVideoId()) {
            return "https://www.youtube.com/embed/{$id}?enablejsapi=1&rel=0";
        }
        elseif ($id = $this->getRutubeVideoId()) {
            return "https://rutube.ru/play/embed/{$id}/";
        }
        elseif ($params = $this->getVkVideoParams()) {
            return "https://vk.com/video_ext.php?".http_build_query($params + ['js_api' => 1]);
        }
    }

    /**
     * Возвращает параметры видео из VK
     *
     * @return array
     */
    public function getVkVideoParams()
    {
        if ($this['video_link']) {
            if (preg_match('/video-(\d+)_(\d+)/', $this['video_link'], $match)) {
                return [
                    'oid' => '-'.$match[1],
                    'id' => $match[2]
                ];
    ...
    /**
     * Возвращает идентификатор видео на RuTube
     *
     * @return string
     */
    public function getRutubeVideoId()
    {
        if ($this['video_link']) {
            if (preg_match('/video\/([^\/]+)/', $this['video_link'], $match)) {
                return $match[1];
            }
        }
    ...
    {
        if ($this['video_link']) {
            if (preg_match('/watch\?v=([^&]+)/', $this['video_link'], $match)) {
                return $match[1];
            }

            if (preg_match('/shorts\/([^&]+)/', $this['video_link'], $match)) {
                return $match[1];
            }
        }
    ...
    public function getYoutubeVideoType()
    {
        if(preg_match('/shorts\/([^&]+)/', $this['video_link'])) {
            $type = self::VIDEO_TYPE_SHORTS;
        } else {
            $type = self::VIDEO_TYPE_DEFAULT;
        }
     ...
     *   'showStockNum' => true, //Если true, то у комплектаций будут оставаться сведения о конкретном количестве на складах. По умолчанию - false
     *   'images' => [
     *       'url' => [
     *           'width' => 268,
     *           'height' => 268,
    ...
    public function getOffersJson(array $options = [], $as_array = false, $cache = true, $update_db = true)
    {
        ...
        if (!empty($options['disableCheckOffers']) || ($this->isMultiOffersUse()
            ...
            $this->updateOffersJson($data, $update_db);
            $data = $this->fixDynamicAfter($data, $dynamic_params, $options);
            $data = $this->prepareOffersImagesUrl($data, $options);
            $data = $this->prepareOffersPropertyImagesUrl($data, $options);
    ...
    protected function fixDynamicBefore($data, $dynamic_params)
    {
        ...
        $event_result = EventManager::fire('product.fixDynamicBefore.start', [
            'product' => $this,
            'data' => $data
        ]);
     ...
     * @return array
     */
    protected function fixDynamicAfter($data, $dynamic_params, $options)
    {
        ...
        foreach($data['offers'] as &$offer_data) {
            ...
            if (empty($options['showStockNum'])) {
                unset($offer_data['stock_num']); //Скрываем конкретные цифры остатков на складах, если не задано иное
            }

            $offer_data['price'] = $offer_data['prices'][$dynamic_params[self::DP_COST_ID]];
            $offer_data['oldPrice'] = $offer_data['prices'][$dynamic_params[self::DP_OLD_COST_ID]] ?? null;

            if (empty($options['showAllCosts'])) {
                unset($offer_data['prices']); //Скрываем все цены, если не задано иное
            }

            $offer_data['sale_status'] = $this->getOfferSaleStatus($offer_data['price']);
            $offer_data['substitute_price_text'] = $this->getSubstitutePriceTextBySaleStatus($offer_data['sale_status']);
        }
        ...
        if ($this['multioffers']['use']) {
            ...
            foreach($this['multioffers']['levels'] as $prop_id => $level) {
                ...
                foreach($level['values'] as $value) {
                    ...
                    if ($value['image']) {
                        $value_arr['property_image'] = $value['image'];
                    }
                    if (isset($level['values_photos'][$value['val_str']])) {
                        $value_arr['property_image'] = $level['values_photos'][$value['val_str']]->getValues();
                    }
    ...
    protected function getOffersJsonOffersSection(array $data, $dynamic_params)
    {
        ...
        if (isset($data['levels']) || count($this['offers']['items'])) {
            ...
            foreach ($this['offers']['items'] as $offer_id => $offer) {
                ...
                $all_costs = CostApi::getFullCostList();
                $prices = [];
                foreach ($all_costs as $cost) {
                    $prices[(string)$cost->id] = $this->getCost($cost->id, $offer_id);
                }

                $offer_data['prices'] = $prices;
        ...
        if (isset($data['images']) && $options['images']) {
            foreach ($data['images'] as $id => $image_array) {
                ...
                foreach ($options['images'] as $key => $size_pack) {
                    $data['images'][$id][$key] = $img_object->getUrl($size_pack['width'], $size_pack['height'], $size_pack['scale'], $size_pack['absolute']);
                }
            }
        } else {
            unset($data['images']);
     ...
     * @return array
     */
    protected function prepareOffersPropertyImagesUrl($data, array $options)
    {
        $options += [
            'property_images' => [
                'url' => [
        ...
        if (isset($data['levels'])) {
            foreach ($data['levels'] as $levels_key => $level) {
                if (isset($level['values'])) {
                    foreach ($level['values'] as $values_key => $value) {
                        //Пост-обработка
                        if (isset($value['property_image']) && $options['property_images']) {
                            if (is_array($value['property_image'])) {
                                if (isset($value['property_image']['id'])) {
                                    $img_object = new PhotoImage();
                                } else {
                                    $img_object = new PhotoStub();
                                }
                                $img_object->getFromArray($value['property_image']);
                            }else {
                                $img_object = new Type\Image();
                                $img_object->set($value['property_image']);
                            }

                            $images = [];
                            foreach ($options['property_images'] as $key => $size_pack) {
                                $images[$key] = $img_object->getUrl($size_pack['width'], $size_pack['height'], $size_pack['scale'], $size_pack['absolute']);
                            }
                            $data['levels'][$levels_key]['values'][$values_key]['image'] = $images;
                        }else {
                            unset($value['property_image']);
                        }
                    }
                }
            }
     ...
     *
     * Это влияет на то, что при создании отгрузки система будет понимать что на все количество товара
     * будет один код маркировки
     *
     * @return bool
     */
    public function isBulk()
    {
        ...
        //Проверяем код ОКЕИ на единицы массы
        if (in_array($unit->code, [160, 161, 162, 163, 164, 166, 168, 170, 173, 181, 185, 206])) {
            return true;
        }
     ...
     * @return array
     */
    public static function getSaleStatusTitles(array $first = [])
    {
        return $first + [
                self::SALE_STATUS_SHOW_COST => t('Показывать цену'),
                self::SALE_STATUS_ON_REQUEST => t('Цена по запросу'),
                self::SALE_STATUS_DISCONTINUED => t('Снято с продаж'),
                self::SALE_STATUS_NO_COST => t('Нет цены'),
            ];
    }
     ...
     *
     * @param float|\Closure $offer_price
     * @return string
     */
    protected function getOfferSaleStatus($offer_price)
    {
        if (!$this['sale_status'] || $this['sale_status'] == self::SALE_STATUS_DEFAULT) {
            $config = Loader::byModule($this);
            if ($config['zero_cost_sale_status'] != self::SALE_STATUS_SHOW_COST) {
                if ($offer_price instanceof \Closure) {
                    $offer_price = $offer_price($this);
                }
                if ($offer_price <= 0) {
                    return $config['zero_cost_sale_status'];
                }
            }
        } else {
        ...
        return self::SALE_STATUS_SHOW_COST;
    }
     ...
     * @return false|mixed
     */
    public function getSubstitutePriceTextBySaleStatus($sale_status)
    {
        if ($sale_status == self::SALE_STATUS_SHOW_COST) {
            return false;
        } else {
            $statuses = self::getSaleStatusTitles();
            return $statuses[$sale_status];
        }
    }


    /**
     * Возвращает замещающий цену текст или false, если текста нет.
     * Например, "цена по запросу" или "снято с продаж". Этот текст нужно показывать вместо цены, если он есть
     *
     * @param integer|null $offer_id ID Комплектации, если не передано, то основная
     * @return string | bool(false)
     */
    public function getSubstitutePriceText($offer_id = null, $cost_id = null)
    {
        $sale_status = $this->getSaleStatus($offer_id, $cost_id);
        return $this->getSubstitutePriceTextBySaleStatus($sale_status);
    }
     ...
     *
     * @param integer|null $offer_id ID Комплектации, если не передано, то основная
     * @param integer|null $cost_id ID Типа цен, на основании которого будет приниматься решение о статусе.
     * Если null, то текущий тип цен.
     * @return string
     */
    public function getSaleStatus($offer_id = null, $cost_id = null)
    {
        return $this->getOfferSaleStatus(function($_this) use ($offer_id, $cost_id) {
            return $_this->getCost($cost_id, $offer_id, false, true);
        });
    }
}