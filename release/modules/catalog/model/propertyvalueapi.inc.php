 ...
 */
class PropertyValueApi extends EntityList
{
    protected $filter_parts;
    ...
    function addSomeValues($multi_value_form)
    {
        ...
        if ($values) {
            ...
            $current_values = OrmRequest::make()
                ->select('id, value')
    ...
    private function convertFromList($property, $new_field)
    {
        ...
        foreach (['product_id', 'group_id'] as $link_field) {
            ...
            OrmRequest::make()
                ...
                ->update("($sub_request)")
                ->asAlias('T')
                ->set(['L.val_list_id' => null])
                ->where("L.prop_id = T.prop_id AND L.{$link_field} = T.{$link_field} AND L.val_list_id = T.val_list_id")
                ->exec();
        }
    ...
    function addTranslitAliases()
    {
        ...
        while ($row = $res->fetchRow()) {
            ...
            $ok = false;
            while (!$ok && $i < 15) {
                $property_value[$this->alias_field] = Transliteration::str2url(Tools::unEntityString($property_value['value'])) . (($i > 0) ? "-$i" : '');
