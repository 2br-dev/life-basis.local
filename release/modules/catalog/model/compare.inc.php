...
class Compare
{
    ...
    */
    function addProduct($id)
    {
        $api = new Api();
        $api->setFilter('id', $id);
        $api->setFilter('site_id', Manager::getSiteId());
        if ($api->getListCount()) {
            $id = (int)$id;
            $this->items[$id] = $id;
            return true;
    ...
    * @return array
    */
    function getCompareData(array $products_id_filter = null)
    {
        $result = [];
        $groups = [];
        ...
        if ($products_id_filter !== null) {
            $items = array_filter($items, function($item) use ($products_id_filter) {
                return in_array($item['id'], $products_id_filter);
            });
        }
