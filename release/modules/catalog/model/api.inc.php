...
class Api extends EntityList
{
    ...
    const SHORT_FILTER_STOCK_VALUES = [
        ...
        self::FILTER_STOCK_AVAILABLE => 'in',
        self::FILTER_STOCK_BY_WAREHOUSES => 'part'
    ];

    protected $dynamic_num_warehouses_id;
    protected $having_count_select;
    ...
    */
    ...
    public function getMultiDirCount()
    {
        ...
        if ($q->having) {
            ...
            $q->select = $this->getHavingCountSelect() ?: $this->defAlias().'.'.$this->getIdField();
    ...
    public function applyBaseFilters($bfilters = null)
    {
        ...
        foreach ($keys as $n => $key) {
            ...
            if (method_exists($this, $func))
                $this->$func($bfilters[$key], $bfilters);
        }
        return true;
    }
     ...
     * @return void
     */
    public function applyAlreadyBuyFilter()
    {
        ...
        if ($user && $user['id'] > 0) {
            ...
            $items = Request::make()
                ...
                ->groupby('entity_id')
                ->exec()->fetchSelected(null, 'entity_id');
     ...
     * @param array $base_filters - массив базовых фильтров
     * @throws RSException
     */
    protected function isNumFilter($filter, $base_filters)
    {
        if ($filter != "") {
            $config = ConfigLoader::byModule($this);
            ...
            if (ModuleManager::staticModuleExists('affiliate')
                ...
                switch ($filter) {
                    case 2: //До вызова данного метода должен быть вызван setAffiliateRestrictions(true)
                        if (!empty($base_filters['warehouses'])) {
                            $this->queryObj()
                                ->whereIn('XST.warehouse_id', $base_filters['warehouses'])
                                ->having('SUM(XST.stock) > 0');
                            break;
                        }
                        //Переходим к обычной фильтрации по наличию, если не заданы склады
                    case 1: {
                ...
                switch ($filter) {
                    case 2: //Фильтр по наличию в конкретных магазинах
                        if (!empty($base_filters['warehouses'])) {
                            $warehouses_id = implode(',', Tools::arrayQuote($base_filters['warehouses']));
                            $this->queryObj()
                                ->join(new Xstock(), 'FXST.product_id = A.id AND warehouse_id IN (' . $warehouses_id . ')', 'FXST')
                                ->where('FXST.stock > 0');
                            break;
                        }
                        //Переходим к обычной фильтрации по наличию, если не заданы склады
                    case 1: //Фильтр по общему наличию
                        $q = $this->queryObj();
                        $q->where('num > 0');
                        break;

                    case 0: //Фильтр по общему отсутствию
                        $q = $this->queryObj();
                        $q->where('num <= 0');
                }
            }
    ...
    {  
        ...
        if (isset($data['simage'])) {
            ...
            if (isset($_FILES['simagefile'])) {
                $photo_api->addFromUrl($_FILES['simagefile']['tmp_name'], 'catalog', $ids, true);
            }
            unset($data['simage']);
        }
        ...
        //Обновляем товары
        $data['offers_json'] = ''; //Сбросим кэш комплектаций
    ...
    */
    ...
    function getMinMaxProductsCost()
    {
        ...
        $money_array['interval_from'] = str_replace('.00', '', (string)$money_array['interval_from']);
        $money_array['interval_to'] = str_replace('.00', '', (string)$money_array['interval_to']);
                ...
                switch ($key) {
                    case 'brand':
                        ...
                        foreach ($values as $value) {
                            if (isset($brands[$value])) {
                                ...
                                $values_arr[] = [
                                    'value' => $value,
                                    'title' => $brands[$value]['title']
                                ];
                            }
                        }
                        $part = [
                            'title' => t('Бренд: ') . implode(', ', $brand_titles),
                            'property_title' => t('Бренд'),
                            'selected_values' => $values_arr
                        ];
                        break;
                    ...
                    case 'cost':
                        $part = [
                            ...
                            'property_title' => t('Цена')
                        ];
                        break;

                    case 'isnum':
                        if ($values != "") {
                            if ($values == self::FILTER_STOCK_EMPTY) {
                                $part = [
                                    'title' => t('Нет в наличии')
                                ];
                            } elseif ($values == self::FILTER_STOCK_AVAILABLE) {
                                $part = [
                                    'title' => t('В наличии')
                                ];
                            } elseif ($values == self::FILTER_STOCK_BY_WAREHOUSES) {
                                $part = [
                                    'title' => t('В наличии в выбранных магазинах')
                                ];
                            }
                        }
                        break;
    ...
    protected function getPropertyFiltersParts($prop_filters)
    {
        ...
        foreach($prop_filters as $prop_id => $data) {
            if ($data['property']->isListType()) {
                $values_arr = [];
                $values_text_arr = [];
                foreach($data['values'] as $key => $value_data) {
                    $value_item = [
                        'value' => $key,
                        'title' => $value_data['value'],
                    ];

                    if ($data['property']['type'] == Item::TYPE_COLOR) {
                        $value_item['color'] = $value_data['color'];
                    }

                    $values_arr[] = $value_item;
                    $values_text_arr[] = $value_data['value'];
                }
                $part = [
                    'title' => $data['property']['title'].': '.implode(', ', $values_text_arr),
                    'selected_values' => $values_arr
                ];
            } else {
    ...
    function applyFiltersToMeta($all_filters_data)
    {
        ...
        $app->title->addSection($append_text, 1, Title::ADD_AFTER);
    }
    ...
    */
    ...
    public static function decodeDirFilterParamsFromUrl()
    {
        ...
        if ($url_filtes_cache === null){
            ...
            if ($config['use_seo_filters'] && !empty($url_filters_part)){
                ...
                //Разбор фильтра по наличию
                foreach ($url_filters as $k => $part) {
                    $part = strtolower($part);
                    if (preg_match('/^(stock_('.implode('|', self::SHORT_FILTER_STOCK_VALUES).')(_([\d_]*))?)$/i', $part, $match)) {
                        $isnum = $match[2];
                        $handbook = array_flip(self::SHORT_FILTER_STOCK_VALUES);
                        if (isset($handbook[$isnum])) {
                            $filters['bfilter']['isnum'] = $handbook[$isnum];
                            if ($handbook[$isnum] == self::FILTER_STOCK_BY_WAREHOUSES && isset($match[4])) {
                                $ids = explode('_', $match[4]);
                                $ids = array_intersect(WareHouseApi::getFilterWarehouses(), $ids);
                                $filters['bfilter']['warehouses'] = $ids;
                            }
                        }
                        unset($url_filters[$k]);
                    }
                    ...
                    if (preg_match('/^brand_(.*)$/i', $part, $match)){ //Смотрим есть ли фильтр по брендам
                        $brands = explode("_", $match[1]);
                        if (!empty($brands)){
    ...
    private static function getEncodeDirFilterParamsArrayUrlParts($bfilters = [], $pf = [], $query = "")
    {
        ...
        if (!empty($pf)) { //Фильтр по характеристикам
            if ($config['use_seo_filters']) { //ЧПУ включен
        ...
        if (isset($bfilters['isnum'])) {  //Фильтр по наличию
            if ($config['use_seo_filters']) {
                $line = 'stock_'.self::SHORT_FILTER_STOCK_VALUES[$bfilters['isnum']];
                if ($bfilters['isnum'] == self::FILTER_STOCK_BY_WAREHOUSES) {
                    asort($bfilters['warehouses']);
                    $line .= '_'.implode('_', $bfilters['warehouses']);
                }
                $filters_as_url[] = $line;
            } else {
                $params['bfilter']['isnum'] = $bfilters['isnum'];
                if ($bfilters['isnum'] == self::FILTER_STOCK_BY_WAREHOUSES
                    && !empty($bfilters['warehouses']) && is_array($bfilters['warehouses'])) {
                    $params['bfilter']['warehouses'] = $bfilters['warehouses'];
                }
            }
        }
    ...
    {
        static $encoded_filters_params = [];
        $key = json_encode([$bfilters, $pf, $query]);

        if (!isset($encoded_filters_params[$key])) {
            $encoded_filters_params[$key] = self::getEncodeDirFilterParamsArrayUrlParts($bfilters, $pf, $query);
        }

        $params = $encoded_filters_params[$key];
    ...
    public function setWarehouseFilter($warehouses, $only_join_dynamic_num = false)
    {
        if ($warehouses) {
            ...
            $this->queryObj()
                ...
                ->leftjoin(new Orm\Xstock(), "XST.product_id = `" . $this->defAlias() . "`.`id` "
                    ."AND XST.warehouse_id IN (".$warehouse_ids.")", "XST");
     ...
     * @return string
     */
    function getHavingCountSelect()
    {
        return $this->having_count_select;
    }
     ...
     * @param string $select
     */
    function setHavingCountSelect($select)
    {
        $this->having_count_select = $select;
    }
