...
use Catalog\Model\Microdata\MicrodataProductList;
...
class ListProducts extends Front
{
    ...
    const ROUTE_EXTRA_TOTAL_PRODUCTS = 'total';
    ...
    function init()
    {
        ...
        //Удаляем куки которые ломали логику переключения отображения товаров
        $this->app->headers
            ->addCookie('viewAs', '', time() - 3600, $cookie_path, '', false, false, 'viewAsPatch')
            ->addCookie('pageSize', '', time() - 3600, $cookie_path, '', false, false, 'pageSizePatch');
        ...
        if ($this->can_rank_sort && !$this->cur_sort) {
            $this->cur_sort = 'rank';
        ...
        if (($dir_id !== false) || ($is_root && $this->config['show_all_products']) || $this->url->isKey('query')) {
            if ($this->query != '' || $dir_id || ($this->config['show_all_products'] && !$this->url->isKey('query'))) { //Если есть категория или задан поисковый запрос или флаг отображать /catalog/
                ...
                if ($this->config['dependent_filters']) {
                    $filters_allowed_sorted = $prop_api->getSwitchedOffFilters($this->api,
                        $filters,
                        $bfilters,
                        $dir_id,
                        array_keys($filter_brands_values),
                ...
                $this->router->getCurrentRoute()->addExtra(self::ROUTE_EXTRA_TOTAL_PRODUCTS, $total);
                ...
                list(, $list) = EventManager::fire('listproducts.aftergetlist', [
                    'controller' => $this,
                    'list' => $list
                ])->extract();
                ...
                foreach ($path as $one_dir) {
                    ...
                    if ($one_dir['public']) {
                        $this->app->breadcrumbs->addBreadCrumb($one_dir['name'],
                            $this->url->replaceKey(['category' => $one_dir['_alias'], 'p' => null, 'filters' => null, 'bfilter' => null] +
                ...
                $this->result->addSection('all_filters_data', $all_filters_data);
                ...
                //Добавляем микроразметку для списка товаров на странице каталога товаров
                if ($config['microdata_product_list_active'] && !empty($list)) {
                    Application::getInstance()->microdata->addMicrodata(new MicrodataProductList($list));
                }
            } else {
