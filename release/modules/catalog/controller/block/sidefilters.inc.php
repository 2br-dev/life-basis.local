...
use Catalog\Model\Microdata\MicrodataCatalog;
...
class SideFilters extends \RS\Controller\StandartBlock
{
    ...
    function getParamObject()
    {
        return parent::getParamObject()->appendProperty([
            ...
            'search_min_elements' => new Type\Integer([
                'description' => t('Минимальное количество элементов, при котором отображать поиск'),
                'hint' => t('Актуально только для списковых (флажковых) фильтров')
            ])
        ]);
    }
    ...
    function actionIndex()
    {
        ...
        }elseif ($dir_id != ''){ //Если мы не в категории, но нам нужны значения из определенной категории
        ...
        $filter_warehouses = WareHouseApi::getFilterWarehouses(true);
        ...
        //Добавляем микроразметку для категории товаров
        if ($dir_id != '' && $money_array) {
            $currency = CurrencyApi::getDefaultCurrency();
            $offer_data = $money_array + ['currency' => $currency['title'], 'offer_count' => $this->router->getCurrentRoute()->getExtra('total')];
            Application::getInstance()->microdata->addMicrodata(new MicrodataCatalog(new Dir($dir_id), $offer_data));
        }
