...
class Proxy extends Front
{
    ...
    public function actionIndex()
    {
        ...
        unset($get_arr['mod_controller']);
        $url = $this->addParamsToUrl($url, $get_arr);

        if($this->url->isPost())
        {
            // Есле передна команда на исполнение
            if(isset($url_info['path']) && $url_info['path'] == '/proxyCommand')
            {
    ...
    {
        return "<script>
        let updateIframeHeight = function() {
        ...
        window.addEventListener('new-content', updateIframeHeight);
        </script>";
    }
