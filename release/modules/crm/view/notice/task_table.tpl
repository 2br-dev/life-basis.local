<table class="table">
    <tr>
        ...
        <td><a href="{$router->getAdminUrl('edit', ['id' => $task->id], 'crm-taskctrl', true)}">{$task->task_num}</a></td>
    </tr>
    <tr>
    ...
    <tr>
        <td>{t}Соисполнители{/t}</td>
        <td>
            {foreach $task->getCollaboratorUsers() as $user}
                {$user->getFio()}<br>
            {/foreach}
        </td>
    </tr>
    <tr>
        <td>{t}Наблюдатели{/t}</td>
        <td>
            {foreach $task->getObserverUsers() as $user}
                {$user->getFio()}<br>
            {/foreach}
        </td>
    </tr>
