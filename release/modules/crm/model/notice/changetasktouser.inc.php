    ...
    implements InterfaceEmail, InterfaceDesktopApp, InterfaceSms
{
    ...
    function init(Task $task, $changes_values, $changer_user_id)
    {
        ...
        $this->task = $task;
        $this->destination_users = $this->task->getUsersByRoles([
    ...
    function getRecipientUsersData()
    {
        ...
        if ($no_one || $this->task['creator_user_id'] == $this->changer_user_id) {
            ...
            foreach($this->destination_users as $user_id => $user) {
                if ($user->e_mail) {
                    $emails[$user_id] = $user->e_mail;
                }
                if ($user->phone) {
                    $phones[$user_id] = $user->phone;
                }
                $user_ids[$user_id] = $user_id;
            }
        }

        if ($no_one || isset($this->destination_users[$this->changer_user_id])) {
    ...
    {
        if (!empty($this->recipients_data['user_ids'])) {
            $notice_data = new NoticeDataDesktopApp();
            $notice_data->title = t('Изменена задача №%0', [$this->task->task_num]);

            $notice_data->destination_user_id = $this->recipients_data['user_ids'];
