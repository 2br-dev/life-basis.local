 ...
 * @property string $provider Провайдер телефонных услуг
 ...
 * @property integer $processed Звонок обработан менеджером?
 * --\--
 */
class CallHistory extends OrmObject
{
    ...
    function _init()
    {
        parent::_init()->append([
            'provider' => new Type\Varchar([
                'description' => t('Провайдер телефонных услуг'),
                'visible' => false,
            ]),
            ...
            'processed' => new Type\Integer([
                'description' => t('Звонок обработан менеджером?'),
                'allowEmpty' => false,
                'checkboxView' => [1, 0],
            ])
        ]);
    ...
    /**
     * Автоматически устанавливает флаг обработки в зависимости от наличия параметра from_call в GET
     *
     * @return bool
     */
    public static function autoUpdateProcessedFlag()
    {
        $from_call_id = \RS\Http\Request::commonInstance()->get('from_call', TYPE_INTEGER);
        if ($from_call_id) {
            $call = new static($from_call_id);
            $call['processed'] = 1;
            $call->update();
        }
    }
    ...
    /**
     * Возвращает взаимодействия, связанные с этим звонком
     *
     * @return array
     */
    public function getInteractions()
    {
        ...
        $interaction_api->addFilterByLink(LinkTypeCall::getId(), $this['id']);
        return $interaction_api->getList();
    }
}
