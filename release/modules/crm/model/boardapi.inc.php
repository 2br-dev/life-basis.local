...
class BoardApi
{
    ...
    public function moveItem(EntityList $items_api, $from, $to, $direction, $new_status_id)
    {
        ...
        if ($from_item = $items_api->getFirst()) {
            ...
            if ($to && $direction) {
                $items_api->getElement()->load($from);
    ...
    /**
     * Устанавливает сортировку элемента на первую позицию в списке в Kanban
     *
     * @param $api
     * @param $elem
     * @return void
     */
    public static function moveToFirst($api, $elem)
    {
        $api->setFilter('status_id', $elem['status_id']);
        $api->setOrder('board_sortn ASC');
        if (!$api->getElement()->id) {
            $api->setElement($elem);
        }
        if ($last_elem = $api->getFirst()) {
            $api->moveElement($elem['id'], $last_elem['id'], 'up');
        }
    }
}