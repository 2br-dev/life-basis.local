...
class CallHistoryCtrl extends Crud
{
    ...
    /**
     * Формирует состав элементов страницы со списком звонков
     *
    ...
    public function helperIndex()
    {
        ...
        $has_right_for_processed = Rights::hasRight($this, ModuleRights::CALL_HISTORY_UPDATE);
        ...
        $helper->setTable(new Table\Element([
            'Columns' => [
                ...
                new TableType\Usertpl('record_id', t('Запись'), '%crm%/telephony/tablecolumn/record.tpl', ['tdAttr' => ['align' => 'center', 'class' => 'p-relative z-0']]),
                new TableType\Yesno('processed', t('Обработан'), ['toggleUrl' => $this->router->getAdminPattern('ajaxToggleProcessed', [':id' => '@id']), 'Sortable' => SORTABLE_BOTH, 'attr' => [
                    'class' => 'toggle-switch rs-switch crud-switch '.(!$has_right_for_processed ? 'disabled' : '')
                ]]),
                ...
                new TableType\Actions('id', [
                    ...
                    new TableType\Action\DropDown([
                        [
                            'title' => t('Создать взаимодействие'),
                            'attr' => [
                                '@data-url' => $this->router->getAdminPattern('add', [':from_call' => '@id'], 'crm-interactionctrl'),
                                'class' => 'crud-add'
                            ]
                        ],
                        [
                            ...
                            'attr' => [
                                '@data-url' => $this->router->getAdminPattern('add', [':from_call' => '@id'], 'crm-dealctrl'),
                                'class' => 'crud-add'
                            ]
                        ],
                        [
                            ...
                            'attr' => [
                                '@data-url' => $this->router->getAdminPattern('add', [':from_call' => '@id'], 'crm-taskctrl'),
                                'class' => 'crud-add'
                            ]
                        ],
                        [
                            ...
                            'attr' => [
                                '@href' => $this->router->getAdminPattern('add', [':from_call' => '@id'], 'shop-orderctrl'),
                            ]
                        ],
                        [
                            'title' => t('Создать пользователя'),
                            'attr' => [
                                '@data-url' => $this->router->getAdminPattern('add', [':from_call' => '@id'], 'users-ctrl'),
                                'class' => 'crud-add'
                            ]
                        ],
                    ]),
            ...
            'Container' => new Filter\Container( [
                'Lines' =>  [
                    new Filter\Line( ['items' => [
                        ...
                        new Filter\Type\Select('processed', t('Обработан менеджером'), [
                            '' => t('Не важно'),
                            '1' => t('Да'),
                            '0' => t('Нет')
                        ])
    ...
    /**
     * Изменяет флаг обработки звонка
     *
     * @return \RS\Controller\Result\Standard
     * @throws \RS\Exception
     */
    function actionAjaxToggleProcessed()
    {
        if ($access_error = Rights::CheckRightError($this, ModuleRights::CALL_HISTORY_UPDATE)) {
        ...
        if ($action_template) {
            $action_template['processed'] = !$action_template['processed'];
    ...
    {
        $id = $this->url->get('id', TYPE_STRING);
        $is_refresh = $this->url->get('refresh', TYPE_INTEGER);
        ...
        $content = $this->view->fetch('telephony/show_call.tpl');

        if ($is_refresh) {
            return $this->result->setHtml($content);
        } else {

            $helper = $this->getHelper();
            $helper->setTopTitle(t('Звонок №{id}'));
            $helper->setForm($content);
