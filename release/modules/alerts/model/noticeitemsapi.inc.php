...
class NoticeItemsApi extends \RS\Module\AbstractModel\EntityList
{
    ...
    public static function addNoticeItem(Types\AbstractNotice $notice, $template)
    {
        ...
        if ($notice_data) {
            if ($notice_data->destination_user_id) {
                $users = (array)$notice_data->destination_user_id;
            } else {
                $users = [0];
            }
            ...
            foreach($users as $user_id) {
                ...
                $notice_item['destination_user_id'] = $user_id;
                ...
                $notice_items[] = $notice_item->insert();
            }
        }
    ...
    */
    public static function cleanOldNoticeItems()
    {
        ...
        $datetime = date('c', strtotime('-'.$config->notice_items_delete_hours.' HOURS'));
        \RS\Orm\Request::make()
            ->delete()
