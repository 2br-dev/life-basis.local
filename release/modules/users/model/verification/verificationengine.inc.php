...
class VerificationEngine extends BaseModel
{
    ...
    function __construct()
    {
        ...
        $this->setVerificationProvider(VerificationProviderManager::getCurrentProvider());
    }
     ...
     * @return $this
     */
    public function setVerificationProvider(AbstractProvider $provider)
    {
        $this->verification_session['verification_provider'] = $provider->getId();
        return $this;
    }

    /**
     * Устанавливает кастомный идентификатор сессии.
     * Нужен в случае, если у обработчика запроса нет реальной сессии.
     * Пример использования в Телеграм ботах
     *
     * @param $session_id
     * @return $this
     */
    ...
    /**
     * Возвращает либо кастомный идентификатор сессии,
     * либо стандартный системный идентификатор сессии
     *
     * @return string
     */
    ...
    {
        $this->verification_session['email'] = $email;
        return $this;
    }
    ...
    {
        return sha1($this->getSessionId().$this->getAction()->getId().\Setup::$SECRET_KEY);
    }
    ...
    public function initializeSession($skip_if_resolved = true)
    {
        ...
        if ($skip_if_resolved
            ...
            && $old_session['user_session_id'] == $this->getSessionId()
        ...
        $session['uniq'] = $uniq;
        $session['user_session_id'] = $this->getSessionId();
