...
class VerificationProviderManager
{
     ...
     *
     * @param array $first Первый элемент, добавляемый к списку
     * @param array $exclude Исключаемые провайдеры. Идентификаторы строковые
     * @return array
     * @throws Exception
     * @throws \RS\Event\Exception
     */
    public static function getProviderTitles($first = [], $exclude = [])
    {
        $result = $first;
        foreach(self::getAvailableProviders() as $name => $verification) {
            if ($verification->canSelectForPhoneVerification() && !in_array($name, $exclude)) {
