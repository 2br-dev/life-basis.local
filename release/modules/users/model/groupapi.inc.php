...
{
    const TAG_CACHE_USERS_GROUP = 'users-group';
    ...
    public function prepareModuleAccessData($module_access)
    {
        ...
        usort($list_fot_table, function($a, $b) {
    ...
    /**
     * Возвращает реальные группы пользователей
     *
     * @param bool $cache - флаг использования кэша
     * @return array|false
     */
    static function getRealUserGroups($cache = true)
    {
        if ($cache) {
            return CacheManager::obj()
                ->expire(0)
                ->tags(self::TAG_CACHE_USERS_GROUP)
                ->request(['Users\Model\GroupApi', 'getRealUserGroups'],false);
        }else {
            $_this = new static();
            $_this->setFilter(['|alias:notin' => implode(',', Tools::arrayQuote([UserGroup::GROUP_GUEST, UserGroup::GROUP_CLIENT]))]);
