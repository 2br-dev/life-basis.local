...
class Api extends EntityList
{
	...
	public function getLike($term, array $fields, $filters = [], $limit = 5)
	{
        ...
        $q->openWGroup();
        if (is_numeric($term)) {
            $q->where("U.id = '#term'", ['term' => $term]);
            $q->orderby("U.id != '#term'", ['term' => $term]);
        }
        if (count($words)==1){  
    ...
    */
    ...
    function multiUpdate(array $data, $ids = [])
    {
        ...
        if (isset($data['ban_expire'])) {
            $request = \RS\Http\Request::commonInstance();
            $data['ban_reason'] = $request->post('ban_reason', TYPE_STRING);

            if ($data['ban_expire'] == '') {
                $data['ban_expire'] = null;
