 ...
 * @property string $email Email для отправки кода
 * @property string $code_hash Хэш от кода верификации
 * @property string $code_debug Код верификации в открытом виде (для режима отладки)
...
class VerificationSession extends AbstractObject
{
    ...
    protected function _init()
    {
        $this->getPropertyIterator()->append([
            ...
            'email' => new Type\Varchar([
                'description' => t('Email для отправки кода'),
                'hint' => t('Если не задан, то будет использован email от creator_user')
            ]),
            ...
            'code_debug' => new Type\Varchar([
                'description' => t('Код верификации в открытом виде (для режима отладки)'),
                'hint' => t('Клиент будет видеть данный код, если он задан. При изменении этого поля, будет пересчитан хэш от кода верификации'),
            ]),
    ...
    public function beforeWrite($save_flag)
    {
        if ($save_flag == self::UPDATE_FLAG
            && $this->isModified('code_debug')
            && $this['code_debug'] != '')
        {
            $this['code_hash'] = $this->getCodeHash($this['code_debug']);
        }
    }
     ...
     *
     * @param bool $cache Если true, то будет использоваться локальный кэш
     * @return AbstractProvider
     * @throws Exception
     */
    public function getVerificationProvider($cache = true)
    {
        if (!$cache || !$this->provider) {
            $this->provider = clone VerificationProviderManager::getProviderByName($this['verification_provider']);
            $this->provider->setVerificationSession($this);
        }
    ...
    public function sendVerificationCode()
    {
        ...
        if ($this['verification_provider'] == Sms::getId()) {
            if ($this['phone'] == '') {
        ...
        if ($send_code_delay == 0) {
            ...
            if (!$this->config['two_factor_demo_mode']) {
                try {
                    $this->getVerificationProvider()->send($code);
                } catch(Exception $e) {
