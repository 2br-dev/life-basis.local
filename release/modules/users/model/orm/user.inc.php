...
use RS\Orm\Type;
use RS\RemoteApp\Manager as AppManager;
use RS\Site\Manager as SiteManager;
use Users\Config\ModuleRights as UsersModuleRights;
 ...
 * @property string $creator_app_id Создан через приложение
 * @property string $last_change_password_date Дата последнего изменения пароля
 * @property string $public_id Короткий публичный идентификатор пользователя, скрывающий числовой ID
...
class User extends OrmObject
{
    ...
    protected function _init()
    {
        ...
        $this->getPropertyIterator()->append([
            t('Основные'),
                ...
                'creator_app_id' => new Type\Varchar([
                    'description' => t('Создан через приложение'),
                    'hint' => t('Здесь будет выбрано приложение, если пользователь был создан через API'),
                    'list' => [function() {
                        return ['' => t('- Не выбрано -')] + AppManager::getAppTypesTitles();
                    }],
                    'maxLength' => 50,
                    'allowEmpty' => false
                ]),
                'last_change_password_date' => new Type\Datetime([
                    'appVisible' => false,
                    'description' => t('Дата последнего изменения пароля'),
                    'hint' => t('Эта дата будет использована для определения необходимости подтверждения аккаунта или смены пароля через почту'),
                ]),
                'public_id' => new Type\Varchar([
                    'description' => t('Короткий публичный идентификатор пользователя, скрывающий числовой ID'),
                    'hint' => t('Создается на время авторизации по QR-коду'),
                    'maxLength' => '15',
                    'unique' => true,
                    'visible' => false
                ]),
            ...
            t('Группы'),
                ...
                'groups' => new Type\ArrayList([
                    ...
                    'appVisible' => false,
                    'checker' => [[__CLASS__, 'checkCanSetUserToGroups']],
                ]),
    ...
    public static function checkFioField($_this, $value)
    {
        ...
        if ($config['user_one_fio_field']) {
            ...
            $fio_parts = array_filter($fio_parts);
    ...
    {
        if ($_this['changepass']) {
            if (mb_strlen((string)$value) < self::PASSWORD_LEN) {
     ...
     * @throws RSException
     */
    public static function checkCanSetUserToGroups($_this, $groups)
    {
        $error_text = null;
        if ($groups) {
            foreach ($groups as $group) {
                if ($error = Rights::CheckRightError('users', UsersModuleRights::RIGHT_SET_USERS_TO_GROUP_PREFIX . $group)) {
                    $error_text .= $error . '<br>';
                }
            }
        }
    ...
    public function beforeWrite($flag)
    {
        ...
        if (!empty($this['openpass'])) {
            $this['pass'] = self::cryptPass($this['openpass'], $this);
            $this['last_change_password_date'] = date('Y-m-d H:i:s');
            $this['public_id'] = null;
        }
     ...
     * Всегда возвращает данные в порядке: фамилия, имя, отчество.
     * Если чего-то из этого нет, то будет пустая строка
     *
    ...
    public static function explodeFio($value)
    {
        ...
        //Правильно отсортируем данные, чтобы всегда элементы шли в последовательности 0-Фамилия 1-Имя 2-Отчество
        $config = Loader::byModule(__CLASS__);
        if (!$config->canShowField('surname') && count($match) < 3) {
            array_unshift($match, ''); //Добавляем пустую фамилию
        }

        //Отрабатываем сценарий, если исключено имя
        if (!$config->canShowField('name') && count($match) < 3) {
            $match[2] = $match[1] ?? '';
            $match[1] = '';
        }
     ...
     * @return string
     */
    public static function cryptPass($password, $user = null)
    {
        ...
        if (is_callable(['\Setup', 'cryptPassword'])) {
            return \Setup::cryptPassword($password, $user);
        }
     ...
     * @return bool
     */
    public function refreshChangeDatePassword()
    {
        $this['last_change_password_date'] = date('Y-m-d H:i:s');
        return $this->update();
    }
    ...
    /**
     * Возвращает объект, описывающий ключ для удаленной авторизации.
     * Используется для создания QR-кода
     *
     * @return AuthorizationKey Объект авторизационного ключа
     */
    public function getRemoteAuthorizationKey()
    {
        return AuthorizationKey::makeByUser($this);
    }
}
