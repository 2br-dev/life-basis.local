...
class File extends ConfigObject
{
    ...
    function _init()
    {
        parent::_init()->append([
            t('Основные'),
                ...
                'password_change_interval_days' => new Type\Integer([
                    'description' => t('Кол-во дней, после которого требовать смену или подтверждения пароля через почту'),
                    'hint' => t('0 - не требовать никогда. Если значение больше нуля, то в случае, если пароль не изменялся более указанное количество дней, то пользователю нужно будет подтвердить старый или установить новый пароль, перейдя по ссылке на почте. Данная опция будет работать, только если у клиента задан Email и только если учетная запись не является административной.')
                ]),
            ...
            t('Настройки входа/регистрации'),
                ...
                'type_code_provider' => new Type\Varchar([
                    ...
                    'template' => '%users%/form/config/type_code_provider.tpl'
                ]),
                'telegram_gw_api' => new Type\Varchar([
                    'description' => t('API ключ для Telegram Gateway'),
                    'hint' => t('Данный ключ необходимо получить по адресу: gateway.telegram.org/account/api'),
                ]),
                'telegram_gw_sender_username' => new Type\Varchar([
                    'description' => t('Название канала-отправителя Telegram'),
                    'hint' => t('Актуально только для верифицированных каналов Telegram. Значение передается в параметре sender_username.')
                ]),
                'telegram_gw_alternate_provider' => new Type\Varchar([
                    'description' => t('Альтернативный провайдер отправки кода, если через Telegram не удалось отправить'),
                    'list' => [['Users\Model\Verification\VerificationProviderManager', 'getProviderTitles'], ['' => t('- Не задано -')], ['telegram-gateway']],
                ]),
                ...
                'two_factor_demo_mode' => new Type\Integer([
                    'checkboxView' => [1, 0],
                    'description' => t('Включить Демо-режим. Код из СМС будет виден прямо в поле его ввода'),
                    'hint' => t('Используйте данный режим, чтобы проверить работу двухэтапной модели авторизации, без реальной отправки СМС'),
                ]),
                'auth_by_key_enable' => new Type\Integer([
                    'description' => t('Включить авторизацию по QR-коду между устройствами'),
                    'checkboxView' => [1, 0]
                ]),
                'auth_key_lifetime' => new Type\Integer([
                    'description' => t('Срок действия QR-кода на авторизацию, в секундах'),
                ]),
                'request_confirm_subscribe_on' => new Type\Integer([
                    'description' => t('Запрашивать подтверждение на рассылку при регистрации'),
                    'checkboxView' => [1, 0]
                ]),
    ...
    public function beforeWrite($flag)
    {
        ...
        $before_config = new self();
        $before_config->load($this['site_id']);

        if ($before_config['login_fields'] != $this['login_fields']) {
    ...
    public static function getDefaultValues()
    {
        return parent::getDefaultValues() + [
                'tools' => [
                    ...
                    [
                        'url' => RouterManager::obj()->getAdminUrl(false, [], 'users-verificationctrl'),
                        'title' => t('Просмотреть верификационные сессии'),
                        'description' => t('Верификационная сессия - это запрос кода для подтверждения действия одним пользователем'),
                        'class' => ' '
                    ]
                ]
            ];
