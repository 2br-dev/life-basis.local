...
class Patches extends AbstractPatches
{
    ...
    function beforeUpdate6035()
    {
        ...
        //Устанавливаем начальное значение даты смены пароля, равное
        Request::make()
            ->update($user)
            ->set('last_change_password_date = dateofreg')
            ->where('last_change_password_date IS NULL')
            ->exec();
    }
     ...
     * Обновляет структуру БД
     */
    function beforeUpdate6011()
    {
        $user = new User();
        $user->getPropertyIterator()->append([
            t('Основные'),
            'creator_app_id' => new Type\Varchar([
                'description' => t('Этот пользователь создан через приложение'),
