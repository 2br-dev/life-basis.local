...
class Profile extends AuthorizedFront
{
    ...
    public $use_post_keys = [
        ...
        'openpass', 'current_pass', 'openpass_confirm', 'captcha', 'data', 'changepass', 'birthday', 'subscribe_on'
    ];
    ...
    /**
     * Страница - демонстрация QR-кода для авторизации на другом устройстве
     *
     * @return string
     */
    public function actionQrCode()
    {
        return $this->result->setTemplate('qrcode.tpl');
    }
        ...
        $user->getPropertyIterator()->append([
            'current_pass' => new Type\Varchar([
                ...
                'checker' => [function($user, $value) {
                    if ($user['changepass'] && $user->cryptPass($value, $user) !== $user['pass']) {
