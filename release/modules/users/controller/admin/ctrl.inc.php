...
class Ctrl extends \RS\Controller\Admin\Crud
{
    ...
    function helperIndex()
    {
        ...
        $helper->setTable(new Table\Element([
            'Columns' => [
                ...
                new TableType\StrYesno('is_company', t('Это компания'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true]),
                ...
                new TableType\Text('creator_app_id', t('Создан через приложение'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true]),
                new TableType\Usertpl('data', t('Дополнительные сведения'), '%users%/form/filter/data.tpl', ['hidden' => true]),
                ...
                new TableType\Datetime('last_change_password_date', t('Последнее изменение пароля'), ['Sortable' => SORTABLE_BOTH, 'hidden' => true]),
            ...
            'Container' => new Filter\Container( [
                                'Lines' =>  [
                                    new Filter\Line( ['items' => [
                                                            ...
                                                            new Filter\Type\Select('is_company',t('Это компания?'), [
                                                                '' => t('- Не важно -'),
                                                                '1' => t('Да'),
                                                                '0' => t('Нет')
                                                            ]),
                                    ...
                                    new Filter\Seccontainer( [
                                        'Lines' => [
                                            new Filter\Line( ['items' => [
                                                                    ...
                                                                    new Filter\Type\Select('creator_app_id', t('Создан через приложение'),
                                                                    ['' => t('Не выбрано')] + AppManager::getAppTypesTitles())
                                            ]
                                            ]),
    ...
    */
    ...
    /**
     * Отображает авторизационный QR-код текущего пользователя
     */
    function actionMyQrCode()
    {
        $helper = new CrudCollection($this);
        $helper->viewAsForm();
        $helper->setTopTitle(t('Мой QR-код'));

        $this->view->assign([
            'user' => $this->user
        ]);

        $helper->setForm($this->view->fetch('%users%/admin/qr_code_dialog.tpl'));
