...
<div class="rs-verify-code-block" data-token="{$verify_session->getToken()}">
    <div class="rs-verify-line">
        ...
        <input type="text" placeholder="{$verify_session.code_debug|default:"{t}Код{/t}"}" name="code" value="" class="form-control verify-key mb-2{if $error} is-invalid{/if}" autocomplete="one-time-code">
        ...
        {if $verify_session.send_counter > 0 && $verify_session.code_expire > time()}
            <div class="rs-verify-send-message">
                {* Здесь будет отображена надпись о том, куда отправлен код - Код отправлен на номер или email .... *}
                {$verify_session->getVerificationProvider()->getRecipientText()}
            </div>
        {/if}
