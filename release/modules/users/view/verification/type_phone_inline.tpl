...
<div class="rs-verify-code-block rs-inline"
    ...
    <div class="rs-verify-line checkout-phone">
        <div class="col">
            <input class="form-control" data-phone type="text" name="{$action->getPhoneInputName()}"
                   ...
                   {if $state != 'initialized'}readonly{/if} {$action->getPhoneInputAttrLine()} autocomplete="one-time-code">
        </div>
        ...
        <div class="rs-verify-timer-line mt-2 {if $delay_refresh_code > 0}rs-wait{/if}" data-delay-refresh-code-sec="{$delay_refresh_code}">
            {if $delay_refresh_code > 0}
                <span class="rs-verify-timer">
                    <span class="phrase">{$verify_session->getVerificationProvider()->getReSendCodeText()} <span class="rs-time">{$verify_session->formatSecond($delay_refresh_code)}</span> {t}сек.{/t}</span>
                </span>
            {/if}
    ...
    <span class="rs-verify-error {if $error}invalid-feedback{/if} d-block">{$error}</span>
</div>