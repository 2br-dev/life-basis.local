    ...
    function _init()
    {
        parent::_init()->append([
            ...
            'search_type_likeplus_spell_checker_enable' => new Type\Integer([
                'description' => t('Искать с учетом грамматических ошибок (только для like+)'),
            ...
            'search_type_likeplus_ignore_symbols' => new Type\Varchar([
                'description' => t('Игнорируемые при поиске символы (только для like+)'),
                'hint' => t('Данные символы будут считаться разделителями слов')
            ]),
            'search_type_likeplus_punto_switcher_enable' => new Type\Integer([
                'description' => t('Искать с учетом другой раскладки клавиатуры (только для like+)'),
                'hint' => t('Например, при наборе `,jkn` будет находиться `болт` и наоборот'),
                'checkboxView' => [1, 0],
            ])
        ]);
    }
