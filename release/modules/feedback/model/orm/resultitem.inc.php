...
class ResultItem extends OrmObject
{
    ...
    {
        return unserialize((string)$this['stext']);
    }
    ...
    /**
     * Удаляет объект из хранилища, вместе с прикрепленным файлом
    ...
    public function delete()
    {
        ...
        if ($result) {
            $data = $this->tableDataUnserialized();
            foreach ($data as $field) {
                if ($field['field']['show_type'] == 'file') {
                    unlink(\Setup::$ROOT.$field['field']['current_value']['real_file_name']);
                }
            }
        }
        return $result;
