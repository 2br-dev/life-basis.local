...
use PushSender\Model\Orm\PushToken;
use RS\Orm\Request;
...
class PushTokenApi extends \RS\Module\AbstractModel\EntityList
{
    ...
    /**
     * Удаляет push-token из локальной базы
     *
     * @return true
     */
    public static function unregisterUserToken($push_token)
    {
        if ($push_token) {
            $token = PushToken::loadByWhere([
                'push_token' => $push_token
            ]);

            if ($token['id']) {
                if (!$token->delete()) {
                    throw new \RS\Exception($token->getErrorsStr());
                }
            } else {
                throw new \RS\Exception(t('Push-токен не найден'));
            }
        }
    ...
    /**
     * Регистрирует push-token пользователя в локальной базе
     *
    ...
    public static function registerUserToken($push_token, $app, $user_id = null, $device = [], $update_user_id = null)
    {
        ...
        if (isset($device['uuid']) && (empty($device['uuid']) || $device['uuid'] == 'undefined')) {
            unset($device['uuid']);
        }

        if (isset($device['uuid'])){ //найдём по устройству
            $where['uuid'] = $device['uuid'];
        }

        //Пробуем загрузить предыдущий токен для этого пользователя(user_id), приложения(app), устройства(uuid),
        //чтобы не создавать лишних записей.
