...
class PushToken extends \RS\Orm\OrmObject
{
    ...
    function _init()
    {
        ...
        $this->addIndex(['model', 'platform']);

        //В рамках одного приложения у пользователя может быть много Push-токенов
        //на разных устройствах
        $this->addIndex(['user_id', 'app', 'push_token'], self::INDEX_UNIQUE);

        //В рамках одного устройства (UUID) у одного приложения может
        //быть только один Push-токен у пользователя
        $this->addIndex(['user_id', 'app', 'uuid'], self::INDEX_UNIQUE);
    }
}
