...
{
    const API_URL = 'https://fcm.googleapis.com/fcm/send'; //Cloud Messaging API (Legacy)
    const API_URL_FIREBASE  = 'https://fcm.googleapis.com/v1/projects/PROJECT_ID/messages:send';//Firebase Cloud Messaging API (V1)
    ...
    function __construct()
    {
        ...
        $url = self::API_URL;
        if ($this->config['firebase_auth_type'] === File::FIREBASE_AUTH_API) {
            $url = str_replace('PROJECT_ID', $this->config['project_id'], self::API_URL_FIREBASE);
        }
    ...
    */
    ...
    {
       if ($this->config['firebase_auth_type'] == File::FIREBASE_AUTH_SERVER_KEY && !$this->config['googlefcm_server_key']){
           throw new Exception(t('Не указан серверный ключ Google Firebase Cloud Messaging'));
       }
       return $this->config['googlefcm_server_key'];
    }
     ...
     * @throws Exception
     */
    protected function checkApiSettings()
    {
        if ($this->config['firebase_auth_type'] == File::FIREBASE_AUTH_API) {
            if (!$this->config['cloud_messaging_api']) {
                throw new Exception(t('Не указан файл с настройками Service Account'));
            }
            if (!$this->config['project_id']) {
                throw new Exception(t('Не указан Project ID'));
            }
        }
    }
    ...
    */
    function sendQuery($server_key, $platform, $push, $data)    
    {
        ...
        if ($this->config['firebase_auth_type'] === File::FIREBASE_AUTH_API) {
            $this->sendQueryApi($data);
        }else  {
            $this->sendQueryLegacy($server_key, $data);
        }
    }
     ...
     * @param array $data - массив данных
     */
    function sendQueryLegacy($server_key, $data)
    {
        ...
        $result = file_get_contents($this->api_url, false, stream_context_create($context));
        if ($result === false) {
     ...
     * @param array $data - массив данных
     */
    function sendQueryApi($data)
    {
        $response = [];
        $requests = $this->prepareRequestList($data);

        $client = new Client();
        $client->useApplicationDefaultCredentials();
        $client->setAuthConfig($this->config['__cloud_messaging_api']->getFullPath());
        $client->addScope(FirebaseCloudMessaging::FIREBASE_MESSAGING);
        $httpClient = $client->authorize();

        // Разбиваем массив на части по 50 запросов
        $requestChunks = array_chunk($requests, 50);

        $chunkIndex = 0;
        $totalChunks = count($requestChunks);

        while ($chunkIndex < $totalChunks) {
            // Создаем асинхронные запросы для каждого URL из текущей части
            $promises = [];
            foreach ($requestChunks[$chunkIndex] as $request_data) {
                $this->writeLog(t("Адрес запроса: %0", [$this->api_url]));
                $this->writeLog(t("Тело запроса к серверу:\n\r %0", [var_export($request_data, true)]));
                $promises[$this->api_url] = $httpClient->postAsync($this->api_url, ['json' => $request_data]);
            }

            // Ожидаем выполнение всех запросов из текущей части
            $results = Promise\Utils::settle($promises)->wait();

            foreach ($results as $result) {
                if ($result['state'] === 'fulfilled') {
                    $content = $result['value']->getBody()->getContents();
                    $this->writeLog(t("Получен ответ от API: %0, %1 \n\r %2", [
                        $result['value']->getStatusCode(),
                        $result['value']->getReasonPhrase(),
                        $content
                    ]));

                    $response[] = $content;
                } else {
                    $error = t("Ошибка при выполнении запроса: %0", [
        ...
        $this->last_response = implode(', ', $response);
    }
     ...
     * @return array[]
     */
    function prepareRequestList($data)
    {
        $requests  =  [];

        foreach ($data['registration_ids'] as $id) {
            $request = [];
            $request['message']['token'] = $id;
            $request['message']['notification'] = [
                'title' => $data['notification']['title'],
                'body' => $data['notification']['body'],
            ];

            $request['message']['apns'] = [
                'payload' => [
                    'aps' => [
                        'content_available' => 1,
                        'sound' => 'default'
            ...
            foreach ($data['data']as $key => $datum) {
                if ($datum) {
                    $custom_data[$key] = (string)$datum;
                }
            }

            $request['message']['data'] = $custom_data;
            $requests[] = $request;
        }
