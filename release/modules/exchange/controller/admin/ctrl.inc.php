...
class Ctrl extends \RS\Controller\Admin\Front
{
    ...
    function actionIndex()
    {
        ...
        $helper->setTopToolbar(new \RS\Html\Toolbar\Element([
            'Items' => [
                new ToolbarButton\ModuleConfig(\RS\Router\Manager::obj()->getAdminUrl('edit', ['mod' => $this->mod_name], 'modcontrol-control'), t('настройка модуля обмена данными'))
            ]
        ]));


        $helper->viewAsAny();
        $helper['form'] = $this->view->fetch('admin/exchange.tpl');
        
        $helper->setBottomToolbar(new Toolbar\Element( [
            'Items' => [
                new ToolbarButton\Button('', t('Загрузить'), ['attr' => ['class' => 'start_import btn-success']]),
            ]
        ]));
