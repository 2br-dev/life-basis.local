...
class Site extends OrmObject
{
     ...
     *
     * @param bool $idna_decode Если true, то домен будет преобразован в читаемый вид из punycode
     * @return string
     */
    public function getMainDomain($idna_decode = false)
    {
        $domains = $this->getDomainsList();
        $main_domain = $domains ? trim($domains[0]) : '';

        if ($idna_decode) {
            $main_domain = (new IdnaConvert())->decode($main_domain);
        }
