...
class RobotsTxtApi
{
    ...
    function AutoCreateSiteRobotsTxt()
    {
        ...
        $route = clone RouterManager::getRoute('sitemap-front-sitemap');
        $sitemap_url = $route
                        ->setSiteContext($this->current_site)
                        ->buildUrl(['site_id' => $this->current_site['id']], true);
    ...
    function writeHtaccess()
    {
        ...
        if ($this->isOnlyOneRobotsTxt()) return;
    ...
    {
        $domain = str_replace('.', '\.', (string)$this->getMainHost());
    ...
    {
        $postfix = $this->isOnlyOneRobotsTxt() ? '' : '-' . $this->current_site['id'];
    ...
    /**
     * Возвращает true, если на сайте должен быть только один файл robots.txt
     * Это может быть если только один мультисайт
     *
     * @return bool
     */
    function isOnlyOneRobotsTxt()
    {
        return count(SiteManager::getSiteList()) <= 1;
    }
