...
class Patches extends \RS\Module\AbstractPatches
{
    ...
    */
    ...
    function afterUpdate6017()
    {
        $site_config = new \Site\Model\Orm\Config();
        $site_config->getPropertyIterator()->append([
            'agreement_cookie' => new Type\Richtext([
                'description' => t('Соглашение на использование cookie (ссылка /cookie/)'),
            ]),
        ]);
        $site_config->dbUpdate();
    }

    function afterUpdate6012()
    {
        $site_config = new \Site\Model\Orm\Config();
        $site_config->getPropertyIterator()->append([
