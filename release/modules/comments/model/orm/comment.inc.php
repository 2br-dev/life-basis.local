...
use RS\Debug\Action\Delete;
use RS\Debug\Action\Edit;
use RS\Module\Manager;
use RS\Orm\Type;
use Photo\Model\OrmType\Photos;
use Photo\Model\Orm\Image;
use Comments\Model\PhotoType\CommentPhotos;
 ...
 * @property string $answer_message Ответ на отзыв
 * @property string $answer_dateof Дата ответа
...
class Comment extends \RS\Orm\OrmObject
{
	...
	function _init()
	{
		...
		parent::_init()->append([
            ...
            'answer_message' => new Type\Text([
                'description' => t('Ответ на отзыв'),
            ]),
            'answer_dateof' => new Type\Datetime([
                'description' => t('Дата ответа')
            ]),
        ...
        if (class_exists('\Photo\Model\OrmType\Photos')) {
            $this->getPropertyIterator()->append([
                t('Фото'),
                    'photos' => (new Photos())
                        ->setDescription(t('Фотографии'))
                        ->setLinkType(CommentPhotos::getLinkType())
            ]);
        }
	}

    /**
     * Возвращает фотографии, связанные с комментарием
     *
     * @return Image[]
     */
    public function getPhotos()
    {
        if (class_exists('Comments\Model\PhotoType\CommentPhotos')) {
            $photos = (new PhotoApi)->getLinkedImages($this['id'], CommentPhotos::getLinkType());
        }
        return $photos ?? [];
    }
	...
	public function beforeWrite($flag)
    {
        ...
        if ($this['answer_message'] != '' && !$this['answer_dateof']) {
            $this['answer_dateof'] = date('Y-m-d H:i:s');
        }
    ...
    {
        //Привязывает фотографии к комментарию
        if ($this->isModified('photos') && Manager::staticModuleExists('photo')) {
            foreach($this['photos'] as $public_uniq) {
                $photo = Image::loadByUniq($public_uniq);
                if ($photo['id'] && $photo['linkid'] != $this['id']) {
                    $photo['linkid'] = $this['id'];
                    $photo->update();
                }
            }
        }
    ...
    */
    ...
    {
        return [
            new Edit(\RS\Router\Manager::obj()->getAdminPattern('edit', [':id' => '{id}'], 'comments-ctrl')),
            new Delete(\RS\Router\Manager::obj()->getAdminPattern('del', [':chk[]' => '{id}'], 'comments-ctrl'))
        ];
    }    
