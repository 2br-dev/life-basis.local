...
class Handlers extends HandlerAbstract
{
    ...
    public static function exportGetTypes($list)
    {
        ...
        $list[] = new ExportType\Sbermegamarket\Sbermegamarket();
        return $list;
    }
    ...
    public static function ormInitCatalogProduct(Product $product)
    {
        ...
        $product->getPropertyIterator()->append([
            t('Яндекс.Маркет'),
                ...
                'yd_disabled' => new OrmType\Integer([
                    'description' => t('Скрыть товар на Яндекс.Маркете'),
                    'default' => 0,
                    'listfromarray' => [[
                        0 => t('Не скрывать'),
                        1 => t('Скрыть')
                ...
                'yd_min_order' => new OrmType\Integer([
                    'description' => t('Минимальное количество для заказа на Яндекс.макрете'),
                ]),
        ]);
    }
    ...
    public static function ormInitCatalogWarehouse(WareHouse $warehouse)
    {
        $warehouse->getPropertyIterator()->append([
            t('Основные'),
            'sber_id' => new OrmType\Integer([
                'description' => t('ID склада в СберМегаМаркете'),
                'allowEmpty' => true,
            ]),
        ]);
    }
