...
class Vkontakte extends AbstractType implements ExchangableInterface
{
    ...
    function _init()
    {
        return parent::_init()
            ->append([
                ...
                'redirect_uri' => new Type\MixedType([
                    'description' => t('Redirect URI'),
                    'visible' => true,
                    'template' => '%export%/form/exporttype/vkontake/redirect_uri.tpl'
                ]),
                ...
                '__actions__' => new Type\MixedType([
                    'description' => ' ',
                    'visible' => true,
                    'template' => '%export%/vk/vk_view_actions.tpl'
                ])
            ]);
    }
    ...
    private function addItem($group_id, $product_data)
    {
        ...
        if ($result && $result['market_item_id']) {
    ...
    private function getMarketUploadServer($group_id, $main_photo = 1)
    {
        ...
        $result = $this->vk_query->query($request_params,'market.getProductPhotoUploadServer');
     ...
     */
    private function saveMarketPhoto($group_id, $upload_response)
    {
        $request_params = [
        ...
        $result = $this->vk_query->query($request_params, 'market.saveProductPhoto');
    ...
    private function uploadPhoto(ExportProfile $profile, $url, $filepath, $main_photo = false)
    {
        ...
        if ($profile['log_level'] == self::LOG_LEVEL_QUERY) {
            ...
            $this->log->append(t('<-- Получен ответ: %result', ['result' => $upload_response]));
        }
        ...
        if ($upload_response) {
            $result = json_decode($upload_response, true);

            if (!isset($result['error'])) {
                $this->log->append(t('Изображение загружено на сервер VK'));

                if ($main_photo) {
                    $result = $this->saveMarketPhoto($profile['group_id'], $upload_response);
                } else {
                    $result = $this->saveMarketPhoto($profile['group_id'], $upload_response);
                }
        ...
        $this->log->append(t('Изображение не загружено на сервер VK. Ошибка: %error', ['error' => $result['error'] ?? 'Неизвестная ошибка']));
    ...
    private function getOfferRequestParams(ExportProfile $profile, Product $product, $offer_index, $offer, &$ext_data = [])
    {
        ...
        $request_params = [
            ...
            'dimension_width' => $dimensions_object->getWidth(ProductDimensions::DIMENSION_UNIT_MM),
            'dimension_length' => $dimensions_object->getLength(ProductDimensions::DIMENSION_UNIT_MM),
            'dimension_height' => $dimensions_object->getHeight(ProductDimensions::DIMENSION_UNIT_MM),
            'weight' => $product->getWeight($offer_index, \Catalog\Model\Api::WEIGHT_UNIT_G)
        ];
            ...
            if (isset($offer['external_data']['ext_id'])) {
                ...
                if ($result === self::EDIT_ERROR_DELETED_BY_VK) {
                    ...
                    if ($result) {
                        ...
                        $vk_id = false;
                        $this->log->append(t('Комплектация не обновлена (ID:%id)', ['id' => $offer['id']]));
                    }
                }
            } else {
                //Создаем товар
        ...
        foreach($product->getImages() as $image) {
            ...
            if ($is_main_photo || $count > 0) {
                ...
                if (isset($offer['external_data']['ext_data_array']['photos'][(int)$is_main_photo][$image_filename])) {
                    ...
                    $ext_data['photos'][(int)$is_main_photo][$image_filename] = $id;
                    ...
                    if ($uri) {
                        ...
                        if ($response && isset($response['photo_id'])) {
                            $image_ids[] = $response['photo_id'];
                            $ext_data['photos'][(int)$is_main_photo][$image_filename] = $response['photo_id'];
    ...
    public function getOauthUrl($profile_id)
    {
        ...
        $params = [
            ...
            'display' => 'page',
            'scope' => 'market, offline, photos', //134283268, //'market, offline, photos',
