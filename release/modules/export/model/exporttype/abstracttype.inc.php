...
abstract class AbstractType extends AbstractObject
{
    ...
    function getCacheFilePath()
    {
        ...
        $cache_dir = \Setup::$ROOT . \Setup::$STORAGE_DIR . '/' . 'export';
        if (!is_dir($cache_dir)) {
            mkdir($cache_dir, \Setup::$CREATE_DIR_RIGHTS, true);
        }

        return $cache_dir . '/' . 'site' . $profile['site_id'] . '_' . $profile['class'] . '_' . $profile['id'] . '.cache';
    }
     ...
     * @return array
     */
    public function getSelectedProductIds(ExportProfile $profile = null, $cache = true)
    {
        $profile = $profile ?? $this->getExportProfile();
...
//                    ->where('(is_virtual = 0 OR is_virtual IS NULL)')
     ...
     * @return OrmRequest
     */
    public function getProductsSelectionQuery(ExportProfile $profile = null)
    {
        $profile = $profile ?? $this->getExportProfile();

        $is_selected_all = !isset($profile['data']['products']) || in_array(0, $profile['data']['products']['groups'] ?? []);

        $product_ids = $is_selected_all ? [] : $this->getSelectedProductIds($profile);
        ...
        // Если не выбраны все товары
        if (!$is_selected_all) {
            if (!empty($product_ids)) {
                $query->whereIn('P.id', $product_ids);
            } else {
                // Если в профиле выбраны категории или товары, и ни один товар не найден (например, все были удалены)
                $query->whereIn('P.id', [0]);
            }
        }
        ...
        if (!empty($profile['min_cost']) || !empty($profile['max_cost'])) {
            ...
            if (!empty($profile['min_cost'])) {
                ...
                $cost_from = floor((float)$currencyApi->convertToBase($cost_from));
            ...
            if (!empty($profile['max_cost'])) {
                ...
                $cost_to = ceil((float)$currencyApi->convertToBase($cost_to));
    ...
    protected function exportOneOffer(ExportProfile $profile, \XMLWriter $writer, Product $product, $offer_id)
    {
        ...
        $offer_type = $this->getOfferTypes()[$profile['data']['offer_type']];
