...
class Yandex extends AbstractType 
{
    public function _init()
    {
        return parent::_init()->append([
            t('Основные'),
                ...
                'export_key_disabled' => new Type\Integer([
                    'description' => t('Выгружать флаг "снято с продаж" в теге "disabled"'),
                    'checkboxview' => [1, 0],
                ]),
                'no_export_step_quantity' => new Type\Integer([
                    'description' => t('Не выгружать шаг изменения количества в теге "step-quantity"'),
                    'checkboxview' => [1, 0],
                ]),
                ...
                'export_sku_in_id' => new Type\Integer([
                    'description' => t('Экспортировать артикул в атрибуте id тега offer, вместо независимого тега shop-sku'),
                    'checkboxview' => [1, 0],
                ]),
    ...
    */
    public function export()
    {
        ...
        $writer->writeAttribute('date', date('Y-m-d\\TH:i:sP'));
            $writer->startElement("shop");  
                $writer->writeElement('name', \RS\Helper\Tools::teaser(\RS\Site\Manager::getSite()->title, 20, false));  
                $writer->writeElement('company', Tools::unEntityString(\RS\Config\Loader::getSiteConfig()->firm_name));
    ...
    */
    protected function exportCurrencies(\Export\Model\Orm\ExportProfile $profile, \XMLWriter $writer)
    {
    ...
    * @param \XMLWriter $writer
    */
    protected function exportCategories(\Export\Model\Orm\ExportProfile $profile, \XMLWriter $writer)
    {
        $writer->startElement("categories");
            ...
            foreach ($groups as $group) {
                $writer->startElement("category");
                    ...
                    EventManager::fire("export.{$profile['class']}.writecategory", [
                        'profile' => $profile,
                        'writer' => $writer,
                        'group' => $group
                    ]);

                    $writer->text( Tools::unEntityString($group['export_name'] ?: $group['name']) );
