...
abstract class CommonOfferType extends AbstractOfferType
{
    ...
    protected function addCommonEspecialTags($ret)
    {
        ...
        $field->hint        = t('для выгрузки тега "dimensions" необходимо указать все 3 габарита. Если не указано, то значения для dimensions будут автоматически получены из характеристик, согласно настройкам модуля Каталог');
        $ret[$field->name]  = $field;
    ...
    public function writeOffer(ExportProfile $profile, \XMLWriter $writer, Product $product, $offer_id)
    {
        ...
        $writer->startElement("offer");
            $id = $profile->export_sku_in_id ? $product->getBarCode($offer_id) : $this->getOfferId($product, $offer_id);
            $writer->writeAttribute('id', $id);
            // Добавляем group_id, если у товара есть комплектации
            if ($product->isOffersUse() && !$profile->no_export_offers) {
            ...
            $min_quantity = (int)($product['yd_min_order'] ?? $product['min_order']);
            if ($min_quantity > 0) {
                $writer->writeElement('min-quantity', $min_quantity);
            }

            if (!$profile['no_export_step_quantity'] && $product['amount_step'] > 0) {
                $writer->writeElement('step-quantity', (int)$product['amount_step']);
            }

            if ($profile['export_key_disabled']) {
                if ($product['yd_disabled']) {
                    $writer->writeElement('disabled', 'true');
                } else {
                    $writer->writeElement('disabled', 'false');
                }
            }
            ...
            if ($profile->use_full_description) {
                ...
                $writer->writeElement('description', Tools::unEntityString($product->short_description));
            }
    ...
    protected function writeOfferProps(ExportProfile $profile, \XMLWriter $writer, Product $product, $offer_id)
    {
        if (!$profile['no_export_offers_props'] && isset($product['offers']['items'][$offer_id])) {
            ...
            foreach ((array)$offer['propsdata_arr'] as $key => $value) {
                $writer->startElement('param');
                $name_for_export = $key;
                $writer->writeAttribute('name', Tools::unEntityString($name_for_export));
