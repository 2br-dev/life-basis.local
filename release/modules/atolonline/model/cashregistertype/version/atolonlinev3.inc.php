...
class AtolOnlineV3 extends \Shop\Model\CashRegisterType\AbstractType
{
    ...
    protected function checkAuthError($response)
    {
        if ($response && $response['code']) {
            switch ($response['code']) {
                case 17:
                    $this->addError(t('Неудалось авторизоваться. Некорректная ссылка на авторизацию'));
                    break;
                case 18:
    ...
    * 
    * @param string $response - ответ сервера АТОЛ на операцию регистрации документа продажи, возврата
    * @return bool
    */
    ...
    /**
     * Делает запрос на запрос статуса чека и возвращает данные записывая их в чек, если произошли изменения
     *
    ...
    /**
    * Обрабатывает результат обработки чека
    * 
