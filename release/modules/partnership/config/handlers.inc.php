...
use Catalog\Controller\Front\Product as ProductController;
...
class Handlers extends HandlerAbstract
{
    function init()
    {
        $this
            ...
            ->bind('controller.afteraction.catalog-front-product')
    ...
    public static function ormAfterLoadShopDelivery($params)
    {
        /** @var Delivery $orm */
        $orm = $params['orm'];
        $orm['show_on_partners'] = @unserialize((string)$orm['_show_on_partners']);
    }
    ...
    /**
     * Скрываем товары недоступных категорий
     *
     * @param $params
     * @return void
     * @throws ExceptionPageNotFound
     */
    public static function controllerAfteractionCatalogFrontProduct($params)
    {
        $partner = Api::getCurrentPartner();
        if ($partner) {
            ...
            /** @var Product $product */
            $product = RouterManager::obj()->getCurrentRoute()->getExtra(ProductController::ROUTE_EXTRA_PRODUCT);
            $product_dirs = $product['xdir'] ?? [];
            $partner_dirs = $partner->getAllowFolderList();

            if (!array_intersect($product_dirs, $partner_dirs)) {
                $controller->e404(t('Такого товара не существует'));
            }
        }
    }
